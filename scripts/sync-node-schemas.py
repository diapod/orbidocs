#!/usr/bin/env python3

from __future__ import annotations

import shutil
from pathlib import Path


ROOT = Path(__file__).resolve().parents[1]
SCHEMAS_DIR = ROOT / "doc" / "schemas"
EXAMPLES_DIR = SCHEMAS_DIR / "examples"
INVALID_EXAMPLES_DIR = EXAMPLES_DIR / "invalid"
SEMANTIC_INVALID_EXAMPLES_DIR = EXAMPLES_DIR / "semantic-invalid"
GOLDEN_DIR = SCHEMAS_DIR / "golden"
MIDDLEWARE_SCHEMA_WHITELIST = (
    "capability-passport-publication-desired-state.v1.schema.json",
    "capability-passport-publication-status-list.v1.schema.json",
    "capability-passport-publication-status.v1.schema.json",
)
NODE_WORKSPACE_SENTINELS = (
    Path("Cargo.toml"),
    Path("protocol/Cargo.toml"),
    Path("schema-gate/Cargo.toml"),
)

SCHEMA_WHITELIST = (
    "configuration-common.v1.schema.json",
    "config-setting-descriptor.v1.schema.json",
    "config-source-binding.v1.schema.json",
    "config-resolution.v1.schema.json",
    "config-derivation.v1.schema.json",
    "config-describe.request.v1.schema.json",
    "config-describe.response.v1.schema.json",
    "config-explain.request.v1.schema.json",
    "config-explain.response.v1.schema.json",
    "config-change-plan.v1.schema.json",
    "config-change.plan.request.v1.schema.json",
    "config-change.plan.response.v1.schema.json",
    "config-change.commit.request.v1.schema.json",
    "config-change.commit.response.v1.schema.json",
    "config-commit-receipt.v1.schema.json",
    "config-activation.apply.request.v1.schema.json",
    "config-activation.apply.response.v1.schema.json",
    "config-activation-receipt.v1.schema.json",
    "config-launch-snapshot.v1.schema.json",
    "config-offline.request.v1.schema.json",
    "config-offline.response.v1.schema.json",
    "operator-task-common.v1.schema.json",
    "operator-task-profile.v1.schema.json",
    "operator-task-local-binding.v1.schema.json",
    "operator-task-readiness.v1.schema.json",
    "operator-task-offer-draft.v1.schema.json",
    "operator-task-experiment-candidate.v1.schema.json",
    "operator-task-experiment-plan.v1.schema.json",
    "operator-task-experiment-result.v1.schema.json",
    "operator-task-conformance-report.v1.schema.json",
    "inference-provider-registry.v1.schema.json",
    "node-identity.v1.schema.json",
    "node-advertisement.v1.schema.json",
    "node-succession.v1.schema.json",
    "peer-handshake.v1.schema.json",
    "capability-advertisement.v1.schema.json",
    "capability-passport-present.v1.schema.json",
    "capability-passport-revocation.v1.schema.json",
    "capability-passport.v1.schema.json",
    "capability-proof-presentation-batch.v1.schema.json",
    "capability-authorization-policy.v1.schema.json",
    "seed-capability-registration.v1.schema.json",
    "federation-root.v1.schema.json",
    "federation-service-endorsement.v1.schema.json",
    "federation-service-endorsement-revocation.v1.schema.json",
    "seed-directory-query-attestation.v1.schema.json",
    "capability-schema-present.v1.schema.json",
    "capability-schema.v1.schema.json",
    "signal-marker-envelope.v1.schema.json",
    "signal-marker.v1.schema.json",
    "coi-declaration.v1.schema.json",
    "exception-record.v1.schema.json",
    "emergency-signal.v1.schema.json",
    "reputation-signal.v1.schema.json",
    "agora-authority-policy.v1.schema.json",
    "agora-public-rejection.v1.schema.json",
    "reputation-snapshot.v1.schema.json",
    "nym-certificate.v1.schema.json",
    "nym-authorship-proof.v1.schema.json",
    "nym-succession.v1.schema.json",
    "association-room-proposal.v1.schema.json",
    "room.v1.schema.json",
    "room-membership.v1.schema.json",
    "room-event.v1.schema.json",
    "room-policy.v1.schema.json",
    "room-live-message.v1.schema.json",
    "room-live-message.v2.schema.json",
    "room-live-message.v3.schema.json",
    "room-live-message.v4.schema.json",
    "room-inference-assertion.v1.schema.json",
    "room-contribution-inference.v1.schema.json",
    "room-access-denial.v1.schema.json",
    "room-floor-policy.v1.schema.json",
    "room-floor-lease.v1.schema.json",
    "room-moderation-intent.v1.schema.json",
    "room-moderation-audit.v1.schema.json",
    "room-relay-endpoint.v1.schema.json",
    "room-relay-delivery.v1.schema.json",
    "room-relay-sender-key-distribution.v1.schema.json",
    "room-relay-sealed-delivery.v1.schema.json",
    "room-membership-attestation.v1.schema.json",
    "room-membership-attestation-request.v1.schema.json",
    "room-attestation-audit.v1.schema.json",
    "whisper-interest.v1.schema.json",
    "whisper-signal.v1.schema.json",
    "whisper-trace.v1.schema.json",
    "whisper-trace-consent-request.v1.schema.json",
    "whisper-trace-publish-request.v1.schema.json",
    "whisper-trace-publish-result.v1.schema.json",
    "whisper-threshold-reached.v1.schema.json",
    "weak-signal-finding.v1.schema.json",
    "weak-signal-web-review.v1.schema.json",
    "whisper-redaction-prepare-request.v1.schema.json",
    "whisper-redaction-prepare-response.v1.schema.json",
    "procurement-offer.v1.schema.json",
    "procurement-contract.v1.schema.json",
    "procurement-receipt.v1.schema.json",
    "gateway-policy.v1.schema.json",
    "escrow-policy.v1.schema.json",
    "settlement-policy-disclosure.v1.schema.json",
    "ledger-account.v1.schema.json",
    "gateway-receipt.v1.schema.json",
    "community-pool-disbursement.v1.schema.json",
    "participant-bind.v1.schema.json",
    "participant-capability-limits.v1.schema.json",
    "node-operator-binding.v1.schema.json",
    "client-instance-attachment.v1.schema.json",
    "client-instance-detachment.v1.schema.json",
    "client-instance-recovery.v1.schema.json",
    "peer-status.v1.schema.json",
    "question-envelope.v1.schema.json",
    "response-envelope.v1.schema.json",
    "service-offer.v1.schema.json",
    "service-offer-relay.v1.schema.json",
    "topic-taxonomy.v1.schema.json",
    "topic-resolution.v1.schema.json",
    "corpus-reasoning-query.v1.schema.json",
    "corpus-reasoning-bid.v1.schema.json",
    "corpus-reasoning-answer.v1.schema.json",
    "corpus-reasoning-bid-state.v1.schema.json",
    "corpus-reasoning-chair-control-policy.v1.schema.json",
    "corpus-reasoning-room-policy.v1.schema.json",
    "corpus-reasoning-room-policy.v2.schema.json",
    "corpus-reasoning-room-policy.v3.schema.json",
    "corpus-reasoning-room-invite.v1.schema.json",
    "corpus-reasoning-turn-proposal.v1.schema.json",
    "corpus-turn-order-offer.v1.schema.json",
    "corpus-turn-order-decision.v1.schema.json",
    "corpus-reasoning-experiment-proposal.v1.schema.json",
    "corpus-reasoning-experiment-review.v1.schema.json",
    "corpus-reasoning-experiment-review.v2.schema.json",
    "corpus-deliberation-review-claims.v1.schema.json",
    "corpus-reasoning-experiment-review.v3.schema.json",
    "corpus-reasoning-experiment-regeneration.v1.schema.json",
    "corpus-reasoning-chair-experiment-decision.v1.schema.json",
    "inquirium.candidate-plan.v1.schema.json",
    "inquirium-resource-profile.v1.schema.json",
    "inquirium-federated-resource-profile.v1.schema.json",
    "operator-resource-envelope.v1.schema.json",
    "operator-resource-envelope-revocation.v1.schema.json",
    "capability-derived.v1.schema.json",
    "operator-guard-hook.v1.schema.json",
    "operator-attention-budget.v1.schema.json",
    "federated-envelope-declaration.v1.schema.json",
    "federated-envelope-declaration.v2.schema.json",
    "node-extension-posture.v1.schema.json",
    "node-extension-federation-publication.v1.schema.json",
    "node-extension-posture-evaluation.v1.schema.json",
    "nse-hook-offer.v1.schema.json",
    "daemon.nse-offer-resolution-trace.v1.schema.json",
    "nse-middleware-evidence.v1.schema.json",
    "nse-hook-decision.v1.schema.json",
    "nse-policy-table.v1.schema.json",
    "nse-select-turn-order-table.v1.schema.json",
    "operator-extension-refusal-code.v1.schema.json",
    "operator-extension-refusal.v1.schema.json",
    "operator-experiment-package.v1.schema.json",
    "operator-extension-activation.v1.schema.json",
    "operator-extension-conformance-report.v1.schema.json",
    "operator-extension-conformance-run.v1.schema.json",
    "operator-extension-conformance-run-result.v1.schema.json",
    "operator-extension-loose-import.v1.schema.json",
    "operator-extension-import-receipt.v1.schema.json",
    "operator-extension-inspection.v1.schema.json",
    "operator-effective-policy-inspection-input.v1.schema.json",
    "operator-effective-policy-inspection.v1.schema.json",
    "operator-extension-revocation.v1.schema.json",
    "operator-extension-safe-mode-action.v1.schema.json",
    "operator-extension-session-activation.v1.schema.json",
    "operator-extension-session-deactivation.v1.schema.json",
    "operator-extension-transition.v1.schema.json",
    "communication-boundary-registry.v1.schema.json",
    "component-communication-observation.v1.schema.json",
    "component-communication-report.v1.schema.json",
    "component-trace-policy.v1.schema.json",
    "component-trace-session.v1.schema.json",
    "component-trace-gap.v1.schema.json",
    "component-trace-record.v1.schema.json",
    "component-trace-manifest.v1.schema.json",
    "component-trace-recording-list.v1.schema.json",
    "component-trace-read-model.v1.schema.json",
    "limit-classification.v1.schema.json",
    "enum-classification.v1.schema.json",
    "dispatch-classification.v1.schema.json",
    "semantic-registry-binding.v1.schema.json",
    "semantic-registry-inspection.v1.schema.json",
    "dator-dispatch-entry.v1.schema.json",
    "corpus-semantic-entry.v1.schema.json",
    "corpus-thematic-profile.v1.schema.json",
    "arca-strategy-entry.v1.schema.json",
    "agent-semantic-entry.v1.schema.json",
    "agent.inference-flow-binding.v1.schema.json",
    "agent.inference-flow-inspection.v1.schema.json",
    "agent.inference-passage-input.v1.schema.json",
    "agent.inference-passage-invoke.v1.schema.json",
    "agent.inference-passage-invoke.response.v2.schema.json",
    "agent.inference-passage-product.v1.schema.json",
    "agent.inference-passage-product.v2.schema.json",
    "agent.inference-terminal-selection.v1.schema.json",
    "agent.inference-terminal-selection.v2.schema.json",
    "agent.inference-passage-trace.v1.schema.json",
    "agent.inference-passage-trace.v2.schema.json",
    "agent.outcome.v1.schema.json",
    "agent.outcome.v2.schema.json",
    "corpus-agent-answer-draft.v1.schema.json",
    "corpus-agent-answer-draft.v2.schema.json",
    "corpus-reasoning-answer.v2.schema.json",
    "corpus-agent-answer.publish.request.v2.schema.json",
    "agent.controller-execution-provenance.v1.schema.json",
    "agent.supplied-input-manifest.v1.schema.json",
    "agent.external-runtime.profile.v1.schema.json",
    "agent.external-runtime.binding.v1.schema.json",
    "agent.external-runtime.turn-request.v1.schema.json",
    "agent.external-runtime.event.v1.schema.json",
    "agent.external-runtime.product.v1.schema.json",
    "agent.external-runtime.product.v2.schema.json",
    "agent.external-runtime.turn-outcome.v1.schema.json",
    "agent.external-runtime.turn-outcome.v2.schema.json",
    "inference-provenance-common.v1.schema.json",
    "inference-execution-posture.v1.schema.json",
    "inference-consumption-policy.v1.schema.json",
    "inference-policy.evaluate.request.v1.schema.json",
    "inference-policy.evaluate.response.v1.schema.json",
    "service-dispatch-result.v2.schema.json",
    "service-order.result.prepare.request.v1.schema.json",
    "inference-execution-provenance.v1.schema.json",
    "inquirium.result-provenance.v1.schema.json",
    "middleware-result-provenance.v1.schema.json",
    "middleware-observed-result.v1.schema.json",
    "inquirium.response.v2.schema.json",
    "inquirium.assistant.disclosure.v1.schema.json",
    "inquirium.assistant.turn.request.v2.schema.json",
    "inquirium.terminal-outcome.v1.schema.json",
    "inquirium-operation-descriptor.v1.schema.json",
    "dator.dispatch-entry-deactivation.v1.schema.json",
    "dator.dispatch-entry-deactivation-result.v1.schema.json",
    "corpus-reasoning-role-assignment.v1.schema.json",
    "corpus-reasoning-instruction-overlay.v1.schema.json",
    "corpus-reasoning-inference-flow-binding.v1.schema.json",
    "service-order.v1.schema.json",
    "service-order-dispatch-request.v1.schema.json",
    "service-order-result.v1.schema.json",
    "service-order-result.v2.schema.json",
    "offer-catalog-fetch-request.schema.json",
    "offer-catalog-fetch-response.schema.json",
    "classification.v1.schema.json",
    "memarium-host-api.v1.schema.json",
    "memarium-blob.v1.schema.json",
    "artifact-delivery-envelope.v1.schema.json",
    "artifact-delivery-result.v1.schema.json",
    "artifact-delivery-status.v1.schema.json",
    "artifact-object-pointer.v1.schema.json",
    "portable-artifact-package.v1.schema.json",
    "artifact-source.v1.schema.json",
    "artifact-extraction-profile.v1.schema.json",
    "artifact-location-advice.v1.schema.json",
    "artifact-acquisition-record.v1.schema.json",
    "middleware-channel-hello.v1.schema.json",
    "middleware-channel-accepted.v1.schema.json",
    "middleware-channel-frame.v1.schema.json",
    "middleware-channel-host-capability-call.v1.schema.json",
    "middleware-channel-hello.v2.schema.json",
    "middleware-channel-accepted.v2.schema.json",
    "middleware-channel-frame.v2.schema.json",
    "middleware-channel-host-capability-call.v2.schema.json",
    "middleware-channel-call-result.v1.schema.json",
    "middleware-channel-request-cancel.v1.schema.json",
    "middleware-channel-heartbeat.v1.schema.json",
    "middleware-channel-session-shutdown.v1.schema.json",
    "middleware-component-contract.v1.schema.json",
    "middleware-module-http-request.v1.schema.json",
    "middleware-module-http-response.v1.schema.json",
    "deferred-operation.v1.schema.json",
    "deferred-operation-status.v1.schema.json",
    "interaction-broker-watch.v1.schema.json",
    "interaction-broker-wait.request.v1.schema.json",
    "interaction-broker-wait.outcome.v1.schema.json",
    "interaction-broker-probe.v1.schema.json",
    "operator-consent.request.v1.schema.json",
    "operator-consent-decision.v1.schema.json",
    "operator-consent-binding.v1.schema.json",
    "sensorium-workbench.consent-descriptor.v1.schema.json",
    "sensorium-os.consent-descriptor.v1.schema.json",
    "sensorium-os.action-catalog-delta.v1.schema.json",
    "sensorium-os.action-catalog-sidecar.v1.schema.json",
    "sensorium-relative-path-address.v1.schema.json",
    "sensorium-command-profile.v1.schema.json",
    "sensorium-patch-policy.v1.schema.json",
    "sensorium-action-semantics.v1.schema.json",
    "sensorium-command-intent.v1.schema.json",
    "sensorium-pty-resource-caps.v1.schema.json",
    "sensorium-workbench-environment.v1.schema.json",
    "sensorium-virt-backend-capabilities.v1.schema.json",
    "sensorium-virt-environment-plan.v1.schema.json",
    "sensorium-virt-image-manifest.v1.schema.json",
    "sensorium-virt-recovery-record.v1.schema.json",
    "sensorium-virt-guest-frame.v1.schema.json",
    "sensorium-virt.host.request.v1.schema.json",
    "sensorium-virt-vfkit-deployment-report.v1.schema.json",
    "sensorium-virt-cloud-hypervisor-deployment-report.v1.schema.json",
    "story-012-vfkit-full-system-report.v1.schema.json",
    "story-012-cloud-hypervisor-full-system-report.v1.schema.json",
    "story-012-powerdns-full-system-report.v1.schema.json",
    "story-012-powerdns-critique-gated-full-system-report.v1.schema.json",
    "story-012-powerdns-discovery-full-system-report.v1.schema.json",
    "corpus-experiment-authority-events.v1.schema.json",
    "story-012-delegated-adaptive-executor-profile.v1.schema.json",
    "story-012-delegated-adaptive-executor-report.v1.schema.json",
    "story-012-delegated-adaptive-executor-report.v2.schema.json",
    "sensorium-terminal-session.v1.schema.json",
    "sensorium-terminal-command.v1.schema.json",
    "sensorium-terminal-input.v1.schema.json",
    "sensorium-terminal-event.v1.schema.json",
    "sensorium-interface-remote-feed-request.v1.schema.json",
    "sensorium-terminal-screen-snapshot.v1.schema.json",
    "sensorium-file-snapshot.v1.schema.json",
    "sensorium-file-read-result.v1.schema.json",
    "sensorium-workbench-patch.v1.schema.json",
    "sensorium-workbench-patch-apply-result.v1.schema.json",
    "sensorium-workbench-patch-stage-result.v1.schema.json",
    "sensorium-workbench-outcome.v1.schema.json",
    "sensorium-workbench-error-codes.v1.schema.json",
    "sensorium-operational-context.v1.schema.json",
    "bounded-http-fetch-request.v1.schema.json",
    "bounded-http-fetch-result.v1.schema.json",
    "bounded-http-fetch-artifact-read-request.v1.schema.json",
    "bounded-http-fetch-artifact-read-result.v1.schema.json",
    "bounded-http-fetch-error-codes.v1.schema.json",
    "bounded-http-fetch-operator-snapshot.v1.schema.json",
    "sensorium-web-source.v1.schema.json",
    "sensorium-web-extraction-request.v1.schema.json",
    "sensorium-web-document-blocks.v1.schema.json",
    "sensorium-web-extraction-result.v1.schema.json",
    "sensorium-web-document-snapshot.v1.schema.json",
    "sensorium-web-durable-source.v1.schema.json",
    "sensorium-web-refresh-status.v1.schema.json",
    "sensorium-web-operator-snapshot.v1.schema.json",
    "sensorium-web-latest-state.v1.schema.json",
    "sensorium-interface-resource.v1.schema.json",
    "sensorium-interface-descriptor.v1.schema.json",
    "sensorium-interface-actuation-descriptor.v1.schema.json",
    "sensorium-interface-actuation-status.v1.schema.json",
    "sensorium-interface-control-request.v1.schema.json",
    "sensorium-interface-control-status.v1.schema.json",
    "sensorium-interface-control-lease.v1.schema.json",
    "sensorium-interface-actuation-grant-scope.v1.schema.json",
    "sensorium-interface-invoke-request.v1.schema.json",
    "sensorium-interface-invoke-receipt.v1.schema.json",
    "sensorium-interface-terminal-input.v1.schema.json",
    "sensorium-interface-terminal-resize.v1.schema.json",
    "sensorium-interface-terminal-signal.v1.schema.json",
    "sensorium-interface-led-set.v1.schema.json",
    "sensorium-interface-frame.v1.schema.json",
    "sensorium-interface-read-request.v1.schema.json",
    "sensorium-interface-read-result.v1.schema.json",
    "sensorium-interface-status.v1.schema.json",
    "sensorium-interface-subscribe-request.v1.schema.json",
    "sensorium-interface-subscription-command.v1.schema.json",
    "sensorium-interface-subscription-status.v1.schema.json",
    "vendor-media-type-registry.v1.schema.json",
    "inquirium.model-package.manifest.v1.schema.json",
    "inquirium.model-package.source-trust.v1.schema.json",
    "inquirium.model-package.operator-endorsement.v1.schema.json",
    "inquirium.model-package.provider-key-endorsement.v1.schema.json",
    "inquirium.model-package.install-receipt.v1.schema.json",
    "inquirium.model-package.active-profile.v1.schema.json",
    "inquirium.model-package.install-plan.v1.schema.json",
    "inquirium.local-model-acceptance-report.v1.schema.json",
    "inac-control.v1.schema.json",
    "notification.v1.schema.json",
    "notification-create.v1.schema.json",
    "notification-action.v1.schema.json",
    "notification-action-result.v1.schema.json",
    "notification-state-changed.v1.schema.json",
    "notification-allow.v1.schema.json",
    "notification-delivery-policy.v1.schema.json",
    "local-contact.v1.schema.json",
    "contact-claim.v1.schema.json",
    "contact-lookup-result.v1.schema.json",
    "contact-request.v1.schema.json",
    "contact-attestation-request.v1.schema.json",
    "contact-attestation-result.v1.schema.json",
    "membership-invitation.v1.schema.json",
    "membership-sponsorship.v1.schema.json",
    "membership-acceptance.v1.schema.json",
    "participant-entry-profile.v1.schema.json",
    "participant-effective-limits.v1.schema.json",
    "surface-access-policy.v1.schema.json",
    "message-envelope.v1.schema.json",
    "messaging.passport-issued.v1.schema.json",
    "messaging.passport-revoked.v1.schema.json",
    "messaging.retention-decided.v1.schema.json",
    "messaging.crisis-marked.v1.schema.json",
    "capability-passport-lookup.v1.schema.json",
    "local-recipient-mailbox-resolve.v1.schema.json",
    "service-ca-material.v1.schema.json",
    "service-ca-revocation.v1.schema.json",
    "causal-context.v1.schema.json",
    "execution-receipt.v1.schema.json",
    "replication-summary.v1.schema.json",
    "replication-delta-request.v1.schema.json",
    "replication-delta-batch.v1.schema.json",
    "replication-apply-report.v1.schema.json",
    "scoped-claim-request.v1.schema.json",
    "scoped-claim-presentation.v1.schema.json",
    "scoped-claim-type-registry.v1.schema.json",
    "trace-event.v1.schema.json",
    "trace-link.v1.schema.json",
    "orbiplex-acceptance-topology.v1.schema.json",
    "orbiplex-acceptance-storage-policy.v1.schema.json",
    "federation-scenario.v1.schema.json",
    "federation-node.v1.schema.json",
    "federation-run.v1.schema.json",
)

EXAMPLE_WHITELIST = (
    "fixture.config-setting-descriptor.json",
    "fixture.config-source-binding.json",
    "fixture.config-resolution.json",
    "fixture.config-derivation.json",
    "fixture.config-describe.request.json",
    "fixture.config-describe.response.json",
    "fixture.config-explain.request.json",
    "fixture.config-explain.response.json",
    "fixture.config-change-plan.json",
    "fixture.config-change.plan.request.json",
    "fixture.config-change.plan.response.json",
    "fixture.config-change.commit.request.json",
    "fixture.config-change.commit.response.json",
    "fixture.config-commit-receipt.json",
    "fixture.config-activation.apply.request.json",
    "fixture.config-activation.apply.response.json",
    "fixture.config-activation-receipt.json",
    "fixture.config-launch-snapshot.json",
    "fixture.config-offline.request.json",
    "fixture.config-offline.response.json",
    "fixture.operator-task-profile.json",
    "fixture.operator-task-local-binding.json",
    "fixture.operator-task-readiness.json",
    "fixture.operator-task-offer-draft.json",
    "fixture.operator-task-experiment-candidate.json",
    "fixture.operator-task-experiment-plan.json",
    "fixture.operator-task-experiment-result.json",
    "fixture.operator-task-conformance-report.json",
    "accepted.middleware-observed-result.v1.json",
    "fixture.agent-supplied-input-manifest.v1.json",
    "bootstrap.node-identity.json",
    "seed-wss.node-advertisement.json",
    "vector-signed.node-advertisement.json",
    "basic.node-succession.json",
    "bootstrap.hello.peer-handshake.json",
    "bootstrap.ack.peer-handshake.json",
    "base-node.capability-advertisement.json",
    "network-ledger.capability-passport-present.json",
    "sealer-access.capability-passport-present.json",
    "memarium-space-access.capability-passport-present.json",
    "offer-catalog.seed-capability-registration.json",
    "orbiplex-main.federation-root.json",
    "org-threshold.federation-root.json",
    "bootstrap-inline-endorsement.federation-root.json",
    "seed-directory.federation-service-endorsement.json",
    "seed-directory.federation-service-endorsement-revocation.json",
    "mixed.capability-proof-presentation-batch.json",
    "capability.seed-directory-query-attestation.json",
    "memarium-declassify.capability-passport-present.json",
    "community-key-access.capability-passport-present.json",
    "p071-workbench.capability-authorization-policy.json",
    "audio-transcription.capability-schema-present.json",
    "audio-transcription.capability-schema.json",
    "participant-broadcast.signal-marker-envelope.json",
    "privacy-redacted.signal-marker.json",
    "no-conflict.coi-declaration.json",
    "temporary-routing-override.exception-record.json",
    "blackout-failover.exception-record.json",
    "emergency-deferred-review.exception-record.json",
    "blackout-correlated.emergency-signal.json",
    "degraded-trust-peer-corroborated.emergency-signal.json",
    "participant-governance-inaction.reputation-signal.json",
    "node-relay-unreliable.reputation-signal.json",
    "nym-helpful-participation.reputation-signal.json",
    "default.agora-authority-policy.json",
    "public-policy-denial.agora-public-rejection.json",
    "community-trusted.reputation-snapshot.json",
    "selected-responder.question-envelope.json",
    "single-responder.procurement-offer.json",
    "contract-without-receipt.response-envelope.json",
    "host-ledger-escrow.procurement-contract.json",
    "arbiter-confirmed.procurement-contract.json",
    "host-ledger-settled.procurement-receipt.json",
    "arbiter-confirmed.procurement-receipt.json",
    "pl-main.gateway-policy.json",
    "pl-main.escrow-policy.json",
    "gateway-payout-freeze.settlement-policy-disclosure.json",
    "access-condition-violation.settlement-policy-disclosure.json",
    "prepaid-participant.ledger-account.json",
    "community-pool.ledger-account.json",
    "inbound-topup.gateway-receipt.json",
    "ubc-subsidy.community-pool-disbursement.json",
    "bound-over-live-session.participant-bind.json",
    "limited-procurement-offer.participant-capability-limits.json",
    "primary-operator.node-operator-binding.json",
    "operator-remote-screen.client-instance-attachment.json",
    "user-requested.client-instance-detachment.json",
    "replace-device.client-instance-recovery.json",
    "warm.peer-status.json",
    "federation-local-procurement.question-envelope.json",
    "federation-local-pseudonymous.question-envelope.json",
    "human-mediated.response-envelope.json",
    "ola-redaction.service-offer.json",
    "cpp-reasoning.service-offer.json",
    "orbiplex-core.topic-taxonomy.json",
    "resolved.topic-resolution.json",
    "ambiguous.topic-resolution.json",
    "unresolved.topic-resolution.json",
    "cpp-sort.corpus-reasoning-query.json",
    "accept.corpus-reasoning-bid.json",
    "decline.corpus-reasoning-bid.json",
    "fish-water.corpus-reasoning-answer.json",
    "active.corpus-reasoning-bid-state.json",
    "private.corpus-reasoning-room-policy.json",
    "moderated.corpus-reasoning-chair-control-policy.json",
    "private.corpus-reasoning-room-policy-v2.json",
    "private.corpus-reasoning-room-policy-v3.json",
    "expert.corpus-reasoning-room-invite.json",
    "expert-contribution.corpus-reasoning-turn-proposal.json",
    "fixture.corpus-turn-order-offer.json",
    "fixture.corpus-turn-order-decision.json",
    "fixture.operator-policy.corpus-turn-order-decision.json",
    "local-agent.corpus-reasoning-experiment-proposal.json",
    "revised.corpus-reasoning-experiment-review.json",
    "requested.corpus-reasoning-experiment-review.v2.json",
    "accepted.corpus-reasoning-experiment-review.v2.json",
    "accepted.corpus-deliberation-review-claims.v1.json",
    "accepted.corpus-reasoning-experiment-review.v3.json",
    "revised-followup.corpus-reasoning-experiment-review.v2.json",
    "solver.corpus-reasoning-experiment-regeneration.json",
    "admitted.corpus-reasoning-chair-experiment-decision.json",
    "workbench-experiment.inquirium.candidate-plan.json",
    "basic.service-offer-relay.json",
    "casualfeeders-breakfast-research.service-order.json",
    "basic.offer-catalog-fetch-request.json",
    "basic.offer-catalog-fetch-response.json",
    "personal-local.classification.json",
    "quarantined-ingress.classification.json",
    "personal-to-community-one-shot.classification.json",
    "community-to-public-persistent.classification.json",
    "write-entry.memarium-host-api.json",
    "inline-note.memarium-blob.json",
    "basic.whisper-redaction-prepare-request.json",
    "draft-ready.whisper-redaction-prepare-response.json",
    "email-dispatched.whisper-trace.json",
    "email-content.whisper-trace.json",
    "image-held.whisper-trace.json",
    "email-content.whisper-trace-consent-request.json",
    "email-dispatched.whisper-trace-publish-request.json",
    "accepted.whisper-trace-publish-result.json",
    "filesystem-retaliation.weak-signal-finding.json",
    "fixture.weak-signal-web-review.json",
    "declassify.memarium-host-api.json",
    "revoked.memarium-host-api.json",
    "private-whisper.artifact-delivery-envelope.json",
    "contact-request-via-contact-lookup.artifact-delivery-envelope.json",
    "basic.artifact-object-pointer.json",
    "basic.middleware-channel-hello.json",
    "basic.middleware-channel-accepted.json",
    "request.middleware-channel-frame.json",
    "basic.middleware-channel-host-capability-call.json",
    "failed.middleware-channel-call-result.json",
    "basic.middleware-channel-request-cancel.json",
    "basic.middleware-channel-heartbeat.json",
    "basic.middleware-channel-session-shutdown.json",
    "basic.middleware-component-contract.json",
    "artifact-acquisition-fixture.middleware-component-contract.json",
    "basic.middleware-module-http-request.json",
    "basic.middleware-module-http-response.json",
    "accepted.artifact-delivery-result.json",
    "fixture.portable-artifact-package.json",
    "fixture.artifact-source.json",
    "https.artifact-source.json",
    "whole-resource.artifact-extraction-profile.json",
    "exact.artifact-location-advice.json",
    "https.artifact-location-advice.json",
    "unchanged.artifact-acquisition-record.json",
    "receipts-created.artifact-acquisition-record.json",
    "receipt.artifact-acquisition-record.json",
    "extraction.artifact-acquisition-record.json",
    "staged.artifact-acquisition-record.json",
    "running.artifact-delivery-status.json",
    "basic.interaction-broker-watch.json",
    "file-exists.interaction-broker-wait.request.json",
    "satisfied.interaction-broker-wait.outcome.json",
    "terminal-alive.interaction-broker-probe.json",
    "production.sensorium-operational-context.json",
    "fixture.bounded-http-fetch-request.json",
    "fixture.bounded-http-fetch-result.json",
    "fixture.bounded-http-fetch-artifact-read-request.json",
    "fixture.bounded-http-fetch-artifact-read-result.json",
    "fixture.bounded-http-fetch-error-codes.json",
    "fixture.bounded-http-fetch-operator-snapshot.json",
    "fixture.limit-classification.json",
    "fixture.enum-classification.json",
    "fixture.dispatch-classification.json",
    "fixture.semantic-registry-binding.json",
    "fixture.semantic-registry-inspection.json",
    "fixture.dator-dispatch-entry.json",
    "fixture.corpus-semantic-entry.json",
    "fixture.corpus-thematic-profile.v1.json",
    "fixture.arca-strategy-entry.json",
    "fixture.agent-semantic-entry.json",
    "fixture.agent.inference-flow-binding.json",
    "fixture.agent.inference-flow-inspection.json",
    "fixture.agent.inference-passage-input.json",
    "fixture.agent.inference-passage-invoke.json",
    "fixture.agent.inference-passage-product.json",
    "fixture.agent.inference-terminal-selection.json",
    "fixture.agent.inference-passage-trace.json",
    "fixture.agent.external-runtime.profile.json",
    "fixture.agent.external-runtime.binding.json",
    "fixture.agent.external-runtime.turn-request.json",
    "fixture.agent.external-runtime.event.json",
    "fixture.agent.external-runtime.product.json",
    "fixture.agent.external-runtime.turn-outcome.json",
    "fixture.inference-execution-posture.json",
    "local.inference-consumption-policy.v1.json",
    "missing.inference-policy.evaluate.request.v1.json",
    "missing.inference-policy.evaluate.response.v1.json",
    "unknown.service-order-result.v2.json",
    "bound.service-dispatch-result.v2.json",
    "bound.corpus-agent-answer-draft.v2.json",
    "invalid/stripped.corpus-agent-answer-draft.v2.json",
    "bound.service-order.result.prepare.request.v1.json",
    "exact.inquirium.assistant.disclosure.v1.json",
    "preview.inquirium.assistant.turn.request.v2.json",
    "execute.inquirium.assistant.turn.request.v2.json",
    "household.inference-provider-registry.v1.json",
    "fixture.inference-execution-provenance.json",
    "fixture.inquirium-operation-descriptor.json",
    "fixture.batch-embed.inquirium-operation-descriptor.json",
    "fixture.image-generate.inquirium-operation-descriptor.json",
    "fixture.image-edit.inquirium-operation-descriptor.json",
    "fixture.train-adapt.inquirium-operation-descriptor.json",
    "fixture.dator.dispatch-entry-deactivation.json",
    "fixture.dator.dispatch-entry-deactivation-result.json",
    "fixture.inquirium-resource-profile.json",
    "fixture.inquirium-federated-resource-profile.json",
    "fixture.operator-resource-envelope.json",
    "fixture.operator-resource-envelope-revocation.json",
    "fixture.capability-derived.json",
    "fixture.operator-guard-hook.json",
    "fixture.operator-attention-budget.json",
    "fixture.federated-envelope-declaration.json",
    "fixture.federated-envelope-declaration.v2.json",
    "fixture.node-extension-posture.json",
    "fixture.node-extension-federation-publication.json",
    "fixture.node-extension-posture-evaluation.json",
    "fixture.nse-hook-offer.json",
    "fixture.daemon.nse-offer-resolution-trace.json",
    "fixture.nse-middleware-evidence.json",
    "fixture.nse-hook-decision.json",
    "fixture.nse-policy-table.json",
    "fixture.nse-select-turn-order-table.json",
    "fixture.operator-extension-refusal-code.json",
    "fixture.operator-extension-refusal.json",
    "fixture.operator-experiment-package.json",
    "artifact-acquisition-fixture.operator-experiment-package.json",
    "fixture.semantic-only.operator-experiment-package.json",
    "fixture.operator-extension-activation.json",
    "fixture.semantic-only.operator-extension-activation.json",
    "fixture.operator-extension-conformance-report.json",
    "fixture.operator-extension-inspection.json",
    "fixture.operator-effective-policy-inspection.json",
    "fixture.operator-effective-policy-inspection-input.json",
    "fixture.operator-extension-revocation.json",
    "fixture.operator-extension-safe-mode-action.json",
    "fixture.operator-extension-session-activation.json",
    "fixture.operator-extension-session-deactivation.json",
    "fixture.operator-extension-transition.json",
    "fixture.communication-boundary-registry.json",
    "fixture.component-communication-observation.json",
    "fixture.component-communication-report.json",
    "fixture.component-trace-policy.json",
    "fixture.component-trace-session.json",
    "fixture.component-trace-gap.json",
    "fixture.component-trace-record.json",
    "fixture.component-trace-manifest.json",
    "fixture.component-trace-recording-list.json",
    "fixture.component-trace-read-model.json",
    "fixture.sensorium-web-source.json",
    "fixture.sensorium-web-extraction-request.json",
    "fixture.sensorium-web-document-blocks.json",
    "fixture.sensorium-web-extraction-result.json",
    "fixture.sensorium-web-document-snapshot.json",
    "fixture.sensorium-web-durable-source.json",
    "fixture.sensorium-web-refresh-status.json",
    "fixture.sensorium-web-operator-snapshot.json",
    "fixture.sensorium-web-latest-state.json",
    "basic.sensorium-relative-path-address.json",
    "cargo-test.sensorium-command-profile.json",
    "observation.sensorium-command-profile.json",
    "fixture.sensorium-patch-policy.json",
    "fixture.sensorium-action-semantics.json",
    "basic.sensorium-command-intent.json",
    "laptop.sensorium-pty-resource-caps.json",
    "local.sensorium-workbench-environment.json",
    "fixture.sensorium-virt-backend-capabilities.json",
    "fixture.sensorium-virt-environment-plan.json",
    "fixture.sensorium-virt-image-manifest.json",
    "ready.sensorium-virt-recovery-record.json",
    "handshake.sensorium-virt-guest-frame.json",
    "request.sensorium-virt-guest-frame.json",
    "fixture.sensorium-virt-host-request.json",
    "invoke.sensorium-virt-host-request.json",
    "passed.sensorium-virt-vfkit-deployment-report.json",
    "passed.sensorium-virt-cloud-hypervisor-deployment-report.json",
    "passed.story-012-vfkit-full-system-report.json",
    "passed.story-012-cloud-hypervisor-full-system-report.json",
    "passed.story-012-powerdns-full-system-report.json",
    "passed.story-012-powerdns-critique-gated-full-system-report.json",
    "passed.story-012-powerdns-discovery-full-system-report.json",
    "delegated.story-012-delegated-adaptive-executor-profile.json",
    "partial.story-012-delegated-adaptive-executor-report.json",
    "running.sensorium-terminal-session.json",
    "cargo.sensorium-terminal-command.json",
    "operator.sensorium-terminal-input.json",
    "stdout.sensorium-terminal-event.json",
    "terminal.sensorium-interface-remote-feed-request.json",
    "viewport.sensorium-terminal-screen-snapshot.json",
    "tree.sensorium-file-snapshot.json",
    "inline.sensorium-file-read-result.json",
    "unified.sensorium-workbench-patch.json",
    "applied.sensorium-workbench-patch-apply-result.json",
    "staged.sensorium-workbench-patch-stage-result.json",
    "completed.sensorium-workbench-outcome.json",
    "active.sensorium-interface-subscription-status.json",
    "published.sensorium-interface-status.json",
    "renew.sensorium-interface-subscription-command.json",
    "temperature.sensorium-interface-descriptor.json",
    "temperature.sensorium-interface-frame.json",
    "temperature.sensorium-interface-read-request.json",
    "temperature.sensorium-interface-read-result.json",
    "temperature.sensorium-interface-subscribe-request.json",
    "push-inline.inac-control.json",
    "refused.inac-control.json",
    "invite-only.contact-claim.json",
    "invitation-required.contact-lookup-result.json",
    "no-match.contact-lookup-result.json",
    "basic.local-contact.json",
    "basic.contact-attestation-request.json",
    "issued.contact-attestation-result.json",
    "community-basic.membership-invitation.json",
    "community-basic.membership-sponsorship.json",
    "probationary.membership-acceptance.json",
    "newcomer.participant-entry-profile.json",
    "newcomer.participant-effective-limits.json",
    "default.surface-access-policy.json",
    "basic.room.json",
    "grant-member.room-membership.json",
    "close.room-event.json",
    "closed-private.room-policy.json",
    "basic.room-live-message.json",
    "reply.room-live-message-v2.json",
    "signed.room-live-message.v4.json",
    "declared.room-live-message.v4.json",
    "signed.room-inference-assertion.v1.json",
    "deny.room-access-denial.json",
    "moderated.room-floor-policy.json",
    "assigned.room-floor-lease.json",
    "mute.room-moderation-intent.json",
    "admitted.room-moderation-audit.json",
    "member.room-relay-endpoint.json",
    "federation.room-relay-endpoint.json",
    "basic.room-relay-delivery.json",
    "basic.room-relay-sender-key-distribution.json",
    "basic.room-relay-sealed-delivery.json",
    "member.room-membership-attestation.json",
    "member-self.room-membership-attestation-request.json",
    "issued.room-attestation-audit.json",
    "refused.room-attestation-audit.json",
    "messaging.contact-request.json",
    "basic.message-envelope.json",
    "issued.messaging.passport-issued.json",
    "public-seed-directory.service-ca-material.json",
    "public-seed-directory.service-ca-revocation.json",
    "basic.causal-context.json",
    "completed.execution-receipt.json",
    "revocations.replication-summary.json",
    "revocations.replication-delta-request.json",
    "revocations.replication-delta-batch.json",
    "partial.replication-apply-report.json",
    "room-membership.scoped-claim-request.json",
    "room-membership.scoped-claim-presentation.json",
    "initial.scoped-claim-type-registry.json",
    "scheduler.trace-event.json",
    "scheduler.trace-link.json",
    "home-lab.orbiplex-acceptance-storage-policy.json",
    "story-012-physical-transport.federation-scenario.json",
    "accepted.corpus-reasoning-role-assignment.json",
    "accepted.corpus-reasoning-instruction-overlay.json",
    "accepted.corpus-reasoning-inference-flow-binding.json",
    "passed.inquirium.local-model-acceptance-report.json",
    "bielik.inquirium.model-package.provider-key-endorsement.json",
)

INVALID_EXAMPLE_WHITELIST = (
    "absent-without-target.config-source-binding.json",
    "incomplete-without-aggregate.config-derivation.json",
    "active-without-component.config-explain.request.json",
    "outcome-without-result.config-describe.response.json",
    "active-without-generation.config-activation-receipt.json",
    "admitted-without-evidence.config-offline.request.json",
    "embedded-digest.config-setting-descriptor.json",
    "shell-command.operator-task-experiment-candidate.json",
    "unknown-action.operator-task-experiment-candidate.json",
    "interface-manage.operator-task-experiment-candidate.json",
    "candidate-digest.operator-task-experiment-candidate.json",
    "candidate-capability.operator-task-experiment-candidate.json",
    "candidate-effect-class.operator-task-experiment-candidate.json",
    "candidate-hil.operator-task-experiment-candidate.json",
    "candidate-binding-identity.operator-task-experiment-candidate.json",
    "absolute-path.operator-task-profile.json",
    "hidden-egress.operator-task-profile.json",
    "missing-digest.operator-task-profile.json",
    "embedded-secret.operator-task-profile.json",
    "restated-portable-fact.operator-task-local-binding.json",
    "activation-generation.operator-task-local-binding.json",
    "mutating-observation.operator-task-experiment-plan.json",
    "uncontained-class.operator-task-experiment-plan.json",
    "blocked-without-code.operator-task-readiness.json",
    "publishable-not-runnable.operator-task-readiness.json",
    "unknown-refusal.operator-task-readiness.json",
    "missing-stage.operator-task-readiness.json",
    "destroyed-without-confirmation.operator-task-experiment-result.json",
    "raw-transcript.operator-task-experiment-result.json",
    "signed-draft.operator-task-offer-draft.json",
    "missing-map-digest.operator-task-conformance-report.json",
    "absolute-path.sensorium-patch-policy.json",
    "parent-escape.sensorium-patch-policy.json",
    "glob-target.sensorium-patch-policy.json",
    "unanchored-pattern.sensorium-patch-policy.json",
    "malformed-mode.sensorium-patch-policy.json",
    "no-operations.sensorium-patch-policy.json",
    "escaped-anchor.sensorium-patch-policy.json",
    "interface-manage.sensorium-action-semantics.json",
    "inferred-rule.sensorium-action-semantics.json",
    "unknown-effect-mode.sensorium-command-profile.json",
    "windows-path.operator-task-profile.json",
    "observe-as-mutation.operator-task-experiment-plan.json",
    "defaulted-observation.operator-task-experiment-plan.json",
    "mutation-without-hil.operator-task-experiment-plan.json",
    "runnable-with-blocked-stage.operator-task-readiness.json",
    "publishable-without-publication.operator-task-readiness.json",
    "verified-without-pass.operator-task-experiment-result.json",
    "refusal-on-success.operator-task-experiment-result.json",
    "refused-step-without-code.operator-task-experiment-result.json",
    "pending-with-confirmation.operator-task-experiment-result.json",
    "missing-sidecar.middleware-observed-result.v1.json",
    "missing-redaction.weak-signal-web-review.json",
    "global-bypass.orbiplex-acceptance-storage-policy.json",
    "missing-risk.orbiplex-acceptance-storage-policy.json",
    "shared-intermediate.corpus-reasoning-inference-flow-binding.json",
    "inline-instruction.corpus-reasoning-inference-flow-binding.json",
    "unknown-field.operator-effective-policy-inspection.json",
    "plain-ws.room-relay-endpoint.json",
    "member-placement.room-relay-sealed-endpoint.json",
    "session-bearer.room-relay-delivery.json",
    "unscoped-delegation.room-event.json",
    "missing-storage-ref.node-identity.json",
    "inline-secret.node-identity.json",
    "no-endpoints.node-advertisement.json",
    "missing-new-proof.node-succession.json",
    "ack-without-reference.peer-handshake.json",
    "no-core-caps.capability-advertisement.json",
    "org-without-custody.federation-root.json",
    "unknown-field.federation-root.json",
    "unsigned.federation-root.json",
    "bad-inline-endorsement.federation-root.json",
    "official-flag.federation-root.json",
    "missing-enabled.federation-root.json",
    "bad-seed-directory-tls-pin.federation-root.json",
    "missing-grants.capability-authorization-policy.json",
    "wrong-kind.signal-marker-envelope.json",
    "raw-with-operations.signal-marker.json",
    "conflict-without-category.coi-declaration.json",
    "high-risk-without-approvals.exception-record.json",
    "critical-without-monitoring-metrics.exception-record.json",
    "failed-test-active.exception-record.json",
    "system-requester-with-non-system-id.exception-record.json",
    "wrong-policy-id.exception-record.json",
    "deferred-emergency-without-review-by.exception-record.json",
    "bad-source-node-id.emergency-signal.json",
    "unknown-trigger-class.emergency-signal.json",
    "tc5-without-flag.emergency-signal.json",
    "tc5-flag-on-non-tc5.emergency-signal.json",
    "nym-procedural-governance-inaction.reputation-signal.json",
    "zero-weight.reputation-signal.json",
    "neutral-polarity.reputation-signal.json",
    "participant-kind-with-node-id.reputation-signal.json",
    "council-emitter-without-council-id.reputation-signal.json",
    "org-root-without-custody.agora-authority-policy.json",
    "detailed-public-rejection.agora-public-rejection.json",
    "runtime-authority.reputation-snapshot.json",
    "negative-price.procurement-offer.json",
    "no-confirmation-paid.procurement-contract.json",
    "settled-without-payee.procurement-receipt.json",
    "corpus-reasoning-role-assignment-widening.json",
    "missing-content.corpus-reasoning-turn-proposal.json",
    "unknown-field.corpus-reasoning-turn-proposal.json",
    "wrong-turn-id-prefix.corpus-reasoning-turn-proposal.json",
    "preselected-target.corpus-turn-order-offer.json",
    "missing-plan-digest.corpus-reasoning-experiment-proposal.json",
    "unknown-field.corpus-reasoning-experiment-proposal.json",
    "mismatched-plan-artifact.corpus-reasoning-experiment-proposal.json",
    "missing-replacement.corpus-reasoning-experiment-review.json",
    "unknown-field.corpus-reasoning-experiment-review.json",
    "missing-correction.corpus-reasoning-experiment-review.v2.json",
    "correction-on-accept.corpus-reasoning-experiment-review.v2.json",
    "unknown-field.corpus-reasoning-experiment-review.v2.json",
    "v2-verdict-in-v1.corpus-reasoning-experiment-review.json",
    "unknown-field.corpus-deliberation-review-claims.v1.json",
    "unnamespaced-profile.corpus-deliberation-review-claims.v1.json",
    "missing-claim-envelope.corpus-reasoning-experiment-review.v3.json",
    "missing-solver-turn.corpus-reasoning-experiment-regeneration.json",
    "unknown-field.corpus-reasoning-experiment-regeneration.json",
    # `mismatched-plan.corpus-reasoning-chair-experiment-decision.json` is a
    # signature-integrity negative (the reviewed-plan ref is re-pointed after
    # signing, so only the Ed25519 signature is invalid). The Node schema gate
    # rejects it via the semantic `decision.validate()`; the doc validator is
    # pure JSON Schema and cannot verify signatures, so it is not synced here.
    "ambient-execution.inquirium.candidate-plan.json",
    "missing-edge-target.inquirium.candidate-plan.json",
    "corpus-reasoning-instruction-overlay-ambient.json",
    "gateway-policy-with-participant-operator.gateway-policy.json",
    "escrow-policy-without-operator.escrow-policy.json",
    "incident-without-basis.settlement-policy-disclosure.json",
    "manual-review-without-decision-basis.settlement-policy-disclosure.json",
    "community-pool-without-council-controller.ledger-account.json",
    "inbound-topup-without-fee-breakdown.gateway-receipt.json",
    "without-basis.community-pool-disbursement.json",
    "missing-via-node.participant-bind.json",
    "mixed-participant-and-nym.question-envelope.json",
    "private-scope-public-delivery.question-envelope.json",
    "selected-responder-missing-federation.question-envelope.json",
    "human-flag-without-human-origin.response-envelope.json",
    "corpus-provider-missing-taxonomy.service-offer.json",
    "min-price-above-max.corpus-reasoning-query.json",
    "accept-without-offer.corpus-reasoning-bid.json",
    "counter-without-offer.corpus-reasoning-bid.json",
    "decline-with-offer.corpus-reasoning-bid.json",
    "non-did-taxonomy-issuer.service-offer.json",
    "blocked-core-messaging.participant-capability-limits.json",
    "missing-scopes.membership-sponsorship.json",
    "guest-with-broadcast.participant-entry-profile.json",
    "unknown-surface.surface-access-policy.json",
    "missing-participant-bind.client-instance-attachment.json",
    "missing-participant-bind.client-instance-detachment.json",
    "missing-detachment-ref.client-instance-recovery.json",
    "direct-personal-to-public.classification.json",
    "one-shot-with-expires.classification.json",
    "persistent-without-expires.classification.json",
    "public-with-full-subjects.classification.json",
    "success-without-draft.whisper-redaction-prepare-response.json",
    "inline-without-consent.whisper-trace.json",
    "digest-only-with-content.whisper-trace.json",
    "direct-with-hops.whisper-trace.json",
    "raw-source.weak-signal-finding.json",
    "review-not-required.weak-signal-finding.json",
    "grant-without-grants.room-membership.json",
    "mediated-policy-with-live.room-policy.json",
    "live-message-with-session-bearer.room-live-message.json",
    "duplicate-reply.room-live-message-v2.json",
    "stripped.room-live-message.v4.json",
    "both-claims.room-inference-assertion.v1.json",
    "reinstate-without-supersedes.room-access-denial.json",
    "unknown-mode.room-floor-policy.json",
    "expiry-on-mute.room-moderation-intent.json",
    "widened.corpus-reasoning-chair-control-policy.json",
    "over-budget.inquirium.local-model-acceptance-report.json",
    "bad-purpose.inquirium.model-package.provider-key-endorsement.json",
    "unknown-field.inquirium.model-package.provider-key-endorsement.json",
    "missing-policy-ref.corpus-reasoning-room-policy-v2.json",
    "missing-binding.corpus-reasoning-room-policy-v3.json",
    "empty-entries.node-extension-posture-evaluation.json",
    "inac-push-missing-artifact.inac-control.json",
    "raw-handle.contact-lookup.artifact-delivery-envelope.json",
    "unsupported-mode.contact-lookup.artifact-delivery-envelope.json",
    "missing-store-ref.artifact-object-pointer.json",
    "mixed-layout.portable-artifact-package.json",
    "traversal.portable-artifact-package.json",
    "noncanonical-base64.portable-artifact-package.json",
    "zero-generation.artifact-source.json",
    "secret-url.artifact-source.json",
    "secret-query.artifact-source.json",
    "extended-secret-query.artifact-source.json",
    "empty-inac-component.artifact-source.json",
    "admit-known-without-table.artifact-source.json",
    "open-field.artifact-extraction-profile.json",
    "zero-candidates.artifact-extraction-profile.json",
    "missing-profile.artifact-location-advice.json",
    "secret-url.artifact-location-advice.json",
    "secret-query.artifact-location-advice.json",
    "extended-secret-query.artifact-location-advice.json",
    "unknown-refusal.artifact-acquisition-record.json",
    "wrong-retry-class.artifact-acquisition-record.json",
    "unchanged-without-freshness.artifact-acquisition-record.json",
    "receipts-created-without-receipts.artifact-acquisition-record.json",
    "unknown-field.middleware-channel-hello.json",
    "zero-epoch.middleware-channel-accepted.json",
    "response-with-request-id.middleware-channel-frame.json",
    "caller-override.middleware-channel-host-capability-call.json",
    "path-shaped.middleware-channel-host-capability-call.json",
    "succeeded-with-failure.middleware-channel-call-result.json",
    "missing-target.middleware-channel-request-cancel.json",
    "unknown-field.middleware-channel-heartbeat.json",
    "missing-deadline.middleware-channel-session-shutdown.json",
    "imperative-federated.middleware-component-contract.json",
    "host-local-compensation.middleware-component-contract.json",
    "mismatched-disposer.middleware-component-contract.json",
    "traversal.middleware-module-http-request.json",
    "query-in-path.middleware-module-http-request.json",
    "header-injection.middleware-module-http-response.json",
    "traversal.interaction-broker-wait.request.json",
    "traversal.sensorium-relative-path-address.json",
    "latest-state-replay.sensorium-interface-descriptor.json",
    "unknown-class.sensorium-operational-context.json",
    "codepoint-overflow.sensorium-operational-context.json",
    "utf8-byte-overflow.sensorium-operational-context.json",
    "mixed-body.bounded-http-fetch-result.json",
    "unbounded.bounded-http-fetch-request.json",
    "wildcard-action.bounded-http-fetch-request.json",
    "oversized-read.bounded-http-fetch-artifact-read-request.json",
    "missing-body.bounded-http-fetch-artifact-read-result.json",
    "unknown-code.bounded-http-fetch-error-codes.json",
    "leaky.bounded-http-fetch-operator-snapshot.json",
    "missing-evidence.limit-classification.json",
    "multibyte-owner.limit-classification.json",
    "pending-boundary.limit-classification.json",
    "normative-with-evidence.limit-classification.json",
    "forbidden-federated.limit-classification.json",
    "no-merge-boundary.limit-classification.json",
    "override-forbidden-boundary.limit-classification.json",
    "override-unclassified.limit-classification.json",
    "path-traversal.limit-classification.json",
    "unknown-field.limit-classification.json",
    "unknown-disposition.enum-classification.json",
    "missing-fingerprint.dispatch-classification.json",
    "operator-binding.semantic-registry-binding.json",
    "unknown-status.semantic-registry-inspection.json",
    "unknown-role.corpus-semantic-entry.json",
    "authority-widening.corpus-thematic-profile.v1.json",
    "unnamespaced.corpus-thematic-profile.v1.json",
    "widening.arca-strategy-entry.json",
    "unknown-field.agent-semantic-entry.json",
    "empty-profiles.agent.inference-flow-binding.json",
    "incomplete-package.agent.inference-flow-inspection.json",
    "inline-prompt.agent.inference-passage-input.json",
    "caller-product.agent.inference-passage-invoke.json",
    "unretained-artifact.agent.inference-passage-product.json",
    "publication-authority.agent.inference-terminal-selection.json",
    "raw-content.agent.inference-passage-trace.json",
    "provider-field.agent.external-runtime.profile.json",
    "unbounded.agent.external-runtime.binding.json",
    "progress-prose.agent.external-runtime.event.json",
    "inference-field.agent.external-runtime.product.json",
    "missing-product.agent.external-runtime.turn-outcome.json",
    "withheld-provider-ref.inference-execution-posture.json",
    "missing-unknown-policy.inference-consumption-policy.v1.json",
    "stripped.service-order-result.v2.json",
    "non-verifiable-requirement.inference-execution-posture.json",
    "missing-ack.inquirium.assistant.turn.request.v2.json",
    "preview-ack.inquirium.assistant.turn.request.v2.json",
    "reference-below-threshold.inference-execution-provenance.json",
    "open-operation.inquirium-operation-descriptor.json",
    "unknown-status.dator.dispatch-entry-deactivation-result.json",
    "bad-profile-ref.inquirium-resource-profile.json",
    "unknown-limit.inquirium-resource-profile.json",
    "missing-axis.inquirium-federated-resource-profile.json",
    "bad-runtime.inquirium-federated-resource-profile.json",
    "wrong-version.inquirium-resource-profile.json",
    "missing-signature.operator-resource-envelope.json",
    "missing-signature.operator-resource-envelope-revocation.json",
    "revision-gap.operator-resource-envelope.json",
    "unknown-field.operator-resource-envelope-revocation.json",
    "empty-components.capability-derived.json",
    "widen.operator-guard-hook.json",
    "unsupported-pair.operator-guard-hook.json",
    "approve.operator-attention-budget.json",
    "empty-windows.operator-attention-budget.json",
    "missing-posture.federated-envelope-declaration.json",
    "empty-bindings.federated-envelope-declaration.v2.json",
    "unknown-attestation.node-extension-posture.json",
    "unknown-code.operator-extension-refusal.json",
    "prompt.daemon.nse-offer-resolution-trace.json",
    "unknown-feature.nse-middleware-evidence.json",
    "production.operator-extension-session-activation.json",
    "package-owned-priority.operator-experiment-package.json",
    "unbound-semantic-entry.operator-experiment-package.json",
    "empty.operator-experiment-package.json",
    "required-fallback.operator-extension-activation.json",
    "malformed-signature.operator-extension-activation.json",
    "missing-signature.operator-extension-revocation.json",
    "missing-package.operator-extension-safe-mode-action.json",
    "foreign-package.operator-extension-safe-mode-action.json",
    "unknown-field.operator-extension-session-deactivation.json",
    "secret-digest.component-communication-observation.json",
    "missing-session.component-communication-observation.json",
    "context-without-digest.component-communication-observation.json",
    "host-evidence.component-communication-report.json",
    "cursor-order.component-trace-gap.json",
    "unknown-kind.component-trace-record.json",
    "terminal-without-final-resume.component-trace-session.json",
    "wrong-offer-digest.nse-hook-decision.json",
    "unknown-field.nse-policy-table.json",
    "duplicate-role.nse-select-turn-order-table.json",
    "missing-generation.sensorium-web-source.json",
    "empty-profile-version.sensorium-web-source.json",
    "missing-fetch.sensorium-web-extraction-request.json",
    "heading-without-level.sensorium-web-document-blocks.json",
    "success-without-representation.sensorium-web-extraction-result.json",
    "missing-provenance.sensorium-web-document-snapshot.json",
    "missing-retention.sensorium-web-durable-source.json",
    "terminal-without-completion.sensorium-web-refresh-status.json",
    "leaky.sensorium-web-operator-snapshot.json",
    "unknown-isolation.sensorium-virt-backend-capabilities.json",
    "unknown-network.sensorium-virt-environment-plan.json",
    "missing-sbom.sensorium-virt-image-manifest.json",
    "quarantined-without-reason.sensorium-virt-recovery-record.json",
    "zero-chunks.sensorium-virt-guest-frame.json",
    "legacy-operation.sensorium-virt-guest-frame.json",
    "unknown-operation.sensorium-virt-host-request.json",
    "unknown-guest-operation.sensorium-virt-host-request.json",
    "non-patch-transfer.sensorium-virt-host-request.json",
    "missing-patch-transfer.sensorium-virt-host-request.json",
    "over-budget.sensorium-virt-vfkit-deployment-report.json",
    "over-budget.sensorium-virt-cloud-hypervisor-deployment-report.json",
    "duplicate-check.story-012-vfkit-full-system-report.json",
    "duplicate-check.story-012-cloud-hypervisor-full-system-report.json",
    "duplicate-check.story-012-powerdns-full-system-report.json",
    "duplicate-check.story-012-powerdns-critique-gated-full-system-report.json",
    "duplicate-check.story-012-powerdns-discovery-full-system-report.json",
    "direct-authority.story-012-delegated-adaptive-executor-report.json",
    "one-shot-with-subscription.sensorium-interface-read-request.json",
    "ready-with-reason.sensorium-interface-status.json",
    "snapshot-with-payload-and-artifact.sensorium-interface-frame.json",
    "terminal-one-shot.sensorium-interface-read-result.json",
    "missing-bytes.sensorium-terminal-event.json",
    "input-content.sensorium-terminal-event.json",
    "status-with-output.sensorium-terminal-event.json",
    "over-limit.sensorium-interface-remote-feed-request.json",
    "invitation-without-route.contact-lookup-result.json",
    "missing-signature.message-envelope.json",
    "memarium-blob-without-signature.memarium-blob.json",
    "no-scope.service-ca-material.json",
    "missing-endorser.federation-service-endorsement.json",
    "unsupported-artifact.capability-proof-presentation-batch.json",
    "missing-signature.seed-directory-query-attestation.json",
    "too-many-parents.causal-context.json",
    "unknown-field.causal-context.json",
    "payload.execution-receipt.json",
    "bad-cursor.replication-summary.json",
)

SEMANTIC_INVALID_EXAMPLE_WHITELIST = (
    "observing-patch.sensorium-action-semantics.json",
    "duplicate-triple.sensorium-action-semantics.json",
    "foreign-operation.sensorium-action-semantics.json",
    "duplicate-target.sensorium-patch-policy.json",
    "lookaround-pattern.sensorium-patch-policy.json",
    "unknown-candidate.corpus-turn-order-decision.json",
    "mismatched-source.sensorium-web-latest-state.json",
    "mismatched-summary.inference-execution-provenance.json",
)

GOLDEN_WHITELIST = (
    "artifact-acquisition-identity-v1.golden.json",
    "artifact-acquisition-refusal-v1.golden.json",
    "golden-room-projection-input.json",
    "golden-room-projection-output.json",
    "middleware-channel-v1.golden.json",
    "middleware-channel-v2.golden.json",
    "sensorium-actuation-relative-path.v1.json",
)

SCHEMA_GATE_AGORA_SCHEMA_WHITELIST = (
    "seed-capability-registration.v1.schema.json",
    "service-offer.v1.schema.json",
    "topic-taxonomy.v1.schema.json",
    "topic-resolution.v1.schema.json",
    "corpus-reasoning-bid.v1.schema.json",
    "corpus-reasoning-answer.v1.schema.json",
    "corpus-reasoning-chair-control-policy.v1.schema.json",
    "corpus-reasoning-room-policy.v1.schema.json",
    "corpus-reasoning-room-policy.v2.schema.json",
    "corpus-reasoning-room-invite.v1.schema.json",
    "corpus-reasoning-turn-proposal.v1.schema.json",
    "corpus-reasoning-experiment-proposal.v1.schema.json",
    "room.v1.schema.json",
    "room-membership.v1.schema.json",
    "room-event.v1.schema.json",
    "room-policy.v1.schema.json",
    "room-live-message.v1.schema.json",
    "room-live-message.v2.schema.json",
    "room-access-denial.v1.schema.json",
    "room-floor-policy.v1.schema.json",
    "room-floor-lease.v1.schema.json",
    "room-moderation-intent.v1.schema.json",
    "room-moderation-audit.v1.schema.json",
    "room-relay-endpoint.v1.schema.json",
    "room-relay-delivery.v1.schema.json",
    "room-relay-sender-key-distribution.v1.schema.json",
    "room-relay-sealed-delivery.v1.schema.json",
    "room-membership-attestation.v1.schema.json",
    "room-membership-attestation-request.v1.schema.json",
    "room-attestation-audit.v1.schema.json",
    "whisper-trace.v1.schema.json",
)


def copy_files(files: tuple[str, ...], source_dir: Path, target_dir: Path) -> None:
    target_dir.mkdir(parents=True, exist_ok=True)
    for name in files:
        source = source_dir / name
        target = target_dir / name
        target.parent.mkdir(parents=True, exist_ok=True)
        shutil.copy2(source, target)


def copy_shared_schemas(schema_target: Path) -> None:
    shared_source = SCHEMAS_DIR / "_shared"
    if not shared_source.exists():
        return
    shared_target = schema_target / "_shared"
    shared_target.mkdir(parents=True, exist_ok=True)
    for source in shared_source.rglob("*.json"):
        relative = source.relative_to(shared_source)
        target = shared_target / relative
        target.parent.mkdir(parents=True, exist_ok=True)
        shutil.copy2(source, target)


def resolve_node_root(node_src: str) -> Path:
    """Resolve and validate the Node workspace root before writing contracts."""
    node_root = (ROOT / node_src).resolve()
    if node_root.parts[-2:] == ("protocol", "contracts"):
        raise ValueError(
            "--node-src must name the Node workspace root, not its protocol/contracts directory"
        )
    missing = [
        str(relative)
        for relative in NODE_WORKSPACE_SENTINELS
        if not (node_root / relative).is_file()
    ]
    if missing:
        raise ValueError(
            f"--node-src does not name a Node workspace root; missing: {', '.join(missing)}"
        )
    return node_root


def main() -> int:
    import argparse

    parser = argparse.ArgumentParser()
    parser.add_argument("--node-src", default="../node")
    args = parser.parse_args()

    try:
        node_root = resolve_node_root(args.node_src)
    except ValueError as error:
        parser.error(str(error))
    contracts_root = node_root / "protocol" / "contracts"
    schema_target = contracts_root / "schemas"
    example_target = contracts_root / "examples"
    invalid_target = example_target / "invalid"
    semantic_invalid_target = example_target / "semantic-invalid"
    golden_target = contracts_root / "golden"
    schema_gate_agora_target = node_root / "schema-gate" / "contracts" / "schemas" / "agora"
    middleware_schema_target = node_root / "middleware" / "schemas"

    copy_files(SCHEMA_WHITELIST, SCHEMAS_DIR, schema_target)
    copy_shared_schemas(schema_target)
    copy_files(SCHEMA_GATE_AGORA_SCHEMA_WHITELIST, SCHEMAS_DIR, schema_gate_agora_target)
    copy_files(MIDDLEWARE_SCHEMA_WHITELIST, SCHEMAS_DIR, middleware_schema_target)
    copy_files(EXAMPLE_WHITELIST, EXAMPLES_DIR, example_target)
    copy_files(INVALID_EXAMPLE_WHITELIST, INVALID_EXAMPLES_DIR, invalid_target)
    copy_files(
        SEMANTIC_INVALID_EXAMPLE_WHITELIST,
        SEMANTIC_INVALID_EXAMPLES_DIR,
        semantic_invalid_target,
    )
    copy_files(GOLDEN_WHITELIST, GOLDEN_DIR, golden_target)
    shutil.copy2(
        SCHEMAS_DIR / "registries" / "vendor-media-types.v1.json",
        contracts_root / "vendor-media-types.v1.json",
    )

    return 0


if __name__ == "__main__":
    raise SystemExit(main())
