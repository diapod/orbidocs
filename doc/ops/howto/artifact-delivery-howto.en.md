...
This HOWTO keeps the operational reference, configuration details, schema map, routing rules, and sequential examples. The shorter [Artifact Delivery FAQ](../faq/artifact-delivery-faq.en.md) is the conceptual entry point.
...
- The daemon exposes `POST /v1/host/capabilities/artifact.delivery.send`,
  ...
  `GET /v1/artifact-delivery/admissions`, and
...
- The daemon can register `matrix-mailbox` and `object-store-indirect` adapters
  for sealed mailbox control delivery and larger object-store-backed payloads
...
- `matrix_mailbox` enables and configures Matrix mailbox transport: homeserver,
  access token, room alias prefix, event type, event payload limit, retention,
  peer artifact cache, and sealed-payload policy.
- `object_store` enables the daemon-owned object store, root directory, maximum
  object size, and retention.
- `object_store_indirect` enables pointer-based indirect delivery for large
  payloads and selects its control adapter, fetch base URL, and threshold.
...
- `in_process` registers `ad-host` built acceptors such as `inac.push`,
  ...
  `corpus.query`, `corpus.answer`, `capability-passport-present.accept`,
  `capability-proof-presentation-batch.accept`, and
  `federation-service-endorsement.install`; daemon-owned code supplies only the
  effect shims.
- `memarium_blob_custody` selects the local target-space policy for accepted
  `memarium-blob.v1` custody facts. The sender does not choose this space in the
  MVP.

`artifact_delivery_profiling` controls lightweight operator/status counters for
payload materialization and transport profiling.

`artifact_delivery_observers` enables metadata-only read-only observers, such as
the tracing observer for delivery/admission completion events. Observer failures
do not affect delivery outcomes.
...
- [`artifact-object-pointer.v1`](../../schemas-gen/schemas/artifact-object-pointer.v1.md)
  is the object-store indirect control artifact for larger payloads. The pointer
  is not authority; receivers verify digest, size, expiry, and policy before
  admission.
...
- [`contact-lookup-result.v1`](../../schemas-gen/schemas/contact-lookup-result.v1.md)
  is consumed by the host Contact Catalog lookup provider when an envelope uses
  `selector/kind = "contact-lookup"`.
...
- `matrix-mailbox` resolves to a Matrix mailbox target for a node id when the
  adapter is enabled.
- `configured-default` resolves through `artifact_delivery.defaults`.
- `group` expands through `artifact_delivery.groups`.
- `capability-first` and `capability-many` resolve through the host capability
  lookup provider.
- `participant`, `org`, and `routing-subject` resolve through host subject/org
  lookup providers.
- `contact-lookup` resolves through a host Contact Catalog provider and is
  currently constrained to lookup-safe `contact/index-value` values, supported
  lookup modes, `selector/purpose = "contact-request/messaging"`, and
  `max/nodes = 1`.
- `object-store-indirect` resolves to an indirect object-store target for a node
  id when the object-store adapter and control adapter are enabled.
...
adapter schemes are Agora publish, INAC direct, Matrix mailbox,
object-store indirect, and node-local loopback where configured. The stage
success policy (`all`, `any`, or `quorum`) decides whether a stage succeeded.
If it fails and `on/failure` points to another stage, the runtime follows that
fallback stage.
