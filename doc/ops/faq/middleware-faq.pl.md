...
- `annotate` - dodaj lokalne metadane i kontynuuj.
...
- `allow` - jedyna decyzja `middleware-decision.v1` dopuszczona przez chain
  `audit`; obserwacja nie może zmieniać funkcjonalnego wyniku dispatchu.
...
Dla ścieżek peer-message i audit host wysyła `PeerMessageInvokeRequest`. Ten sam
endpoint może otrzymywać wywołania `inbound-peer` i `audit`:
