...
| FR-004 | The MVP deployment MUST accept a `topic/key` of `"opinions/url"` as a default catch-all bucket for URL opinions without requiring a per-domain or per-community split. | Fact | Story-008 Step 3 notes |
