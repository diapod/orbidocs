...
| FR-013 | The relay MUST record a full action trace for each accepted record covering: capability lookup, sign request, ingest, readback. This trace MUST be inspectable without re-running the flow.  | Fact | Story-008 AC-7 | implemented |
...
| NFR-005 | The local trace for a published opinion SHOULD be available under a single trace namespace (e.g. `trace/agora`) and SHOULD correlate capability lookup, sign request, ingest, and readback by a common correlation identifier.  | Inference | Operability | implemented |
