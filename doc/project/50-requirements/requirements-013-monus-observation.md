...
| FR-007 | When an outgoing `whisper-signal.v1` was materially prepared by Monus, the publication path MUST preserve `source/class = monus-derived` or `monus-sensorium-derived` as appropriate.  | Fact | Proposal 013 + Proposal 022 | todo |
...
| ID | Requirement | Type | Source | Status |
|---|---|---|---|---|
| NFR-001 | The Monus integration surface SHOULD preserve stratyfikacja between local observation, host capability grants, and bounded social-signal publication.  | Fact | Project values + Proposal 022 | todo |
