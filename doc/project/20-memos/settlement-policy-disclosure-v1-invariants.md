...
7. `effective/from <= effective/until` SHOULD be enforced by consumers when
   `effective/until` is present.
8. `incident/*` events MUST carry at least one formal anchor:
   `case/ref`, `exception/ref`, or non-empty `basis/refs`.
9. This artifact MAY snapshot `operator/org-ref` and `serving/node-id` even though
...
10. `settlement-policy-disclosure.v1` does not replace `exception-record.v1`; it
