...
- Inquirium moved from a mostly conceptual organ to an implemented substrate slice. Proposal 063 now has a first `generate` vertical through `inquirium-core`, daemon `inquirium.generate`, JSON-e Flow ingress/preflight, NSE runtime selection, deterministic stub runtime, classification-aware request validation, metadata-only trace records, and the direct data-plane lease/batch-embed pilot for lease-backed work. Proposal 064's runtime-adapter recommendations are mostly implemented across model-runtime catalog v0.2, runtime-candidate routing, HTTP/stdio adapters, remote provider adapters, embedding contracts, durable lease APIs, deferred `batch.embed`, and verified artifact descriptors; conformance report storage/runner and broader operation-specific surfaces remain open. Proposal 066 now has a local-only assistant turn capability, host-capped assistant output, principal-scoped transcript fallback with local-control excision markers, idempotent turn replay, metadata-only assistant trace/feed, and a render-only Node UI affordance under `/admin/inquirium/assistant`; it remains below MVP because Memarium-backed transcript storage, baseline-assistant conformance, context-source grants, operator-question widgets, escalation policy, feedback, and agentic effect governance are still open.
...
| [Proposal 063: Inquirium as a Model Inquiry Organ](../40-proposals/063-inquirium-model-inquiry-organ.md) | `false` | `false` | `false` | `80` |
...
| [Proposal 066: Inquirium Assistant Channel](../40-proposals/066-inquirium-assistant-channel.md) | `false` | `false` | `false` | `60` |
