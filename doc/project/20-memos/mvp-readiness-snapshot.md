...
- Inquirium is now promoted to Solution 044 as an MVP-capable bounded-inquiry
  ...
  items.
- Proposal 073 introduces Agent as the bounded stateful orchestration organ above Inquirium. It remains intentionally post-MVP, but the node-local lifecycle foundation is now durable: `agent-core` owns substrate-free lifecycle/fork/step/effect contracts, `agent-host` owns pure step decisions and operator-profile admission, and the daemon exposes local-control-only `spawn/fork/suspend/resume/status/stop/effect.propose` dispatch. Memarium Personal facts are the source of truth for session seeds, lifecycle idempotency, state, initial steps, budget traces, and effect proposals/outcomes; startup rebuilds a disposable projection through bounded ordered pages, repairs only the latest missing lifecycle-state projection, and reports disabled Memarium as explicit ephemeral durability. Forks enforce monotone narrowing, leaf/depth/child/concurrency bounds, and parent budget reservation. Inquirium budget charges are recovered exactly, a bounded scheduler reaps expired sessions, and HIL effect proposals use globally owned refs plus bounded transition histories before an admitted Workbench request can bind one Sensorium directive. P073 now freezes removed/tightened-profile recovery as immutable admission-snapshot reconstruction plus a current-policy gate and quarantine; implementation remains a tracked task. The real controller executor, bounded memory projection, generic AD effect bridge, explicit module lifecycle grants, consumer/Room chair bindings, Agent-owned lease lifecycle, and fully populated per-step model/instruction/effect traces remain open.
...
| [Proposal 073: Agent — Bounded Stateful Orchestration Organ](../40-proposals/073-agent-orchestration-organ.md) | `false` | `false` | `false` | `63` |
