...
- Inquirium moved from a mostly conceptual organ to an implemented substrate slice.
  ...
  protection, and shared daemon artifact-output intent execution).
...
- Proposal 025/Seed Directory capability catalog is hard-MVP ready but still post-MVP incomplete: official-service status now rests on `federation-service-endorsement.v1`, root-pack endorsement revocations are applied at startup, the unified `/revocations` feed carries `{artifact_family, artifact}` entries for passport and endorsement revocations, operator/AD endorsement install is scoped and conflict-aware, and `capability-passport-present.v1` can refresh the local passport cache through AD with metadata-only presentation facts. Remaining work is broader sovereign issuance UX, endorsement-revocation schema freeze/fixtures, and mixed-batch presentation hardening.
...
| [Proposal 025: Seed Directory as Capability Catalog](../40-proposals/025-seed-directory-as-capability-catalog.md) | `true` | `true` | `false` | `97` |
...
| [Proposal 076: Federation Identity and Network Selector](../40-proposals/076-federation-identity-and-network-selector.md) | `true` | `true` | `false` | `90` |
