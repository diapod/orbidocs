...
- Proposal 080 is now complete through P080-018. The bounded shared `channel_json` executor has opt-in bundled cohorts for Dator, Arca, the Python Inquirium adapters, Sensorium OS, Sensorium Workbench, and Offer Catalog. Story-005 now proves host-owned `runtime/ref` and model-binding invocation over the channel plus stop/non-routable/restart; Story-009 and strict Story-010 also pass while intentional product/network listeners remain explicit. P080 remains at 90% because P080-019 still owns the default switch and host-only port removal, and P080-020 still owns the explicit legacy `http_local_json` policy.
...
| [Proposal 080: Multiplexed Middleware Channel Executor](../40-proposals/080-multiplexed-middleware-channel-executor.md) | `false` | `true` | `false` | `90` |
