...
registered with positive and negative vectors and Node Schema Gate coverage. The readiness estimates of the new
...
The first optional typed Reviewer-claim profile under P074-033 is runtime-complete,
but P074-033 itself remains partial. The
2026-08-31 passage
...
domains. Reviewer-contract selection is now task-profile-owned rather than
...
distinct Orbiplex Agent backed by the pinned Codex Python SDK and local App Server
over supervised JSONL `stdio`. The restarted passage
`federation-run:story-012-physical-two-host-three-node-codex-reviewer:20260901T031054Z`
continued the same durable Reviewer session after restarting `node-c`; the later
fresh passage
`federation-run:story-012-physical-two-host-three-node-codex-reviewer:20260901T032223Z`
used new run and provider-session roots and then independently proved restart,
rejoin, and cleanup. Both passed all ten Story assertions, exact DNS results,
typed host-owned review claims, complete communication traces, solver/Chair model
post-effect revalidation, external-Agent status/revocation revalidation, and no
provider identity in Room/Corpus contracts. Their aggregates have SHA-256
`930a590f6c68ffa7cb08c91a16bd7addb465a17662eb89b2343cc4e4ae6c81da` and
`8e2ae6dedb32109de95acf36cef1df97a91de109cc461c5207752f6f0defd253`.
...
grammars, deterministic replay, and a ten-pair real-model calibration. The measured
...
The macOS Seatbelt profile now permits only the exact host-created middleware
...
use. Supervised package components can now reuse P080's exact dependency and effect
...
A bounded fake-process smoke proves the structural boundary. A separate opt-in
macOS arm64/Metal suite now proves the complete lifecycle with a pinned,
...
- The 2026-09-01 post-MVP P089 foundation and first concrete profile add a provider-neutral External Agent
  Runtime path without changing Agent, Corpus, or Room identity. Six canonical
  `agent.external-runtime.*.v1` contracts and their Node mirrors are Schema-Gated;
  pure core accounting rejects unbounded reservations and missing, malformed, or
  overflowing measured usage; every terminal `authoritative` or `host-measured`
  batch requires matching usage without silently downgrading the profile, while
  `turn-outcome` conditionals locally require their `status` discriminator; and a
  private bounded driver port has a deterministic fake
  covering malformed, reordered, stale, slow, oversized, crashed, cancelled, and
  `unknown` behavior. Agent/Memarium remains the durable source of truth for
  binding, fenced session/checkpoint, pre-I/O reservation, dispatch, normalized
  observations, terminal selection, product, settlement, and replay. BDO exposes
  canonical bounded poll/cancel state, while Replay Scheduler owns reconciliation
  and durable cancellation intent without a private queue. Absolute turn deadlines
  bound driver I/O; a stopped Agent rejects late provider output while product-free
  cancellation cleanup retains the full ambiguous charge. Typed tool and operator
  requests traverse the existing inert Agent effect/HIL/receipt boundary or return
  a typed refusal, and operator admission requires a real mediation receipt. Fake
  acceptance proves separate Room-attested participant and
  Chair Agents, restart and exact replay without reinvocation, ordinary
  `agent.outcome.v1`, and inert Corpus answer-draft admission without provider
...
- The 2026-09-04 P090 foundation now has accepted semantics, a repository-wide
  ...
  core for conservative legacy migration, boundary-relative comparison,
  bounded canonical identity, joins, redaction, and lineage. Schema Gate covers
  ...
  complete P090-004 through P090-006a. The next producer slice now admits
  explicit provider-neutral execution characteristics for active runtimes,
  keeps missing facts `unknown`, and commits exact response-bound sidecars into
  durable traces for successful direct generation, embedding, classification,
  ...
  runtime completion or yet enforce every acceptance promotion path.
...
| [Proposal 094: Operator Task Packs for Bounded Problem Solving](../40-proposals/094-operator-task-packs-for-bounded-problem-solving.md) | `true` | `false` | `false` | `15` |
...
| [Classification](../60-solutions/018-classification/018-classification.md) | `true` | `true` | `false` | `100` |
