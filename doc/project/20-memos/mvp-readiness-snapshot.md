...
- Inquirium is now promoted to Solution 044 as an MVP-capable bounded-inquiry
  ...
  items.
- Proposal 073 introduces Agent as the bounded stateful orchestration organ above Inquirium. It remains intentionally post-MVP, but the node-local foundation is now substantially implemented: `agent-core` owns substrate-free lifecycle, fork, step, memory, binding, and effect contracts, including capability-grant identifier validation and binding-response memory-policy consistency; `agent-host` owns pure step decisions, operator-profile admission, and an explicit fail-closed summary-producer allowlist; and the daemon exposes lifecycle, proposal, FlowNode binding, bounded controller-run, and contextual effect-dispatch capabilities. Local control is administrative authority, while module calls require explicit bounded JSON-e Flow `agent_grants`, Agent ownership, and target-capability allowlists. Memarium Personal facts remain the source of truth; startup performs bounded replay, partial-bundle repair, immutable admission-snapshot recovery, current-policy quarantine cascading through descendants, and terminal/expired-owner lease reconciliation. The real HTTP process smoke covers spawn/status/binding/controller/suspend/resume/fork/stop plus durable recovery after `SIGKILL`. Memory projection combines pinned facts, registered externally produced summaries, and recall refs without prompt content in traces; removing producer authority quarantines only affected sessions. Generic admitted effects use the official Sensorium Core or Artifact Delivery surfaces and operation-bound Agent leases; completed effects, stop, controller completion, reaping, recovery, and TTL bound lease cleanup. Remaining post-MVP work is active Inquirium/effect/child execution inside the controller passage, unused child-budget/resource-cost reconciliation, operator proposal/status and content-addressed outcome projections, Capability Registry-derived summary-producer eligibility intersected with the host allowlist, 30-day-default audit-preserving lease compaction, and later Corpus-chair/assistant bindings.
...
| [Proposal 073: Agent — Bounded Stateful Orchestration Organ](../40-proposals/073-agent-orchestration-organ.md) | `false` | `false` | `false` | `67` |
