...
| `interaction-broker.wait` | `host/interaction-broker.wait` | koordynacja hosta | host-owned ograniczony wait nad zarejestrowanymi źródłami obserwacji | daemon interaction broker | częściowe | Waits są control-plane coordination z deadline i idempotency; runtime daemona istnieje dla waitów wspartych deferred-operation, a live source-provider adapters i grant-context admission pozostają częściowe. |
| `interaction-broker.watch` | `host/interaction-broker.watch` | koordynacja hosta | host-owned ograniczony watch/replay cursor nad zarejestrowanymi źródłami obserwacji | daemon interaction broker | częściowe | Watch replay windows są ograniczone przez liczbę/czas i niosą wyłącznie metadata-safe events; daemon utrwala watch resources, a live provider delivery i retention-backed replay pozostają częściowe. |
| `interaction-broker.probe` | `host/interaction-broker.probe` | koordynacja hosta | host-owned aktywny probe postępu, żywotności, stanu pliku albo obecności artefaktu | daemon interaction broker | częściowe | Probes tworzą diagnostykę albo outcomes; daemon zapisuje bounded probe resources, a efektowna remediation pozostaje po stronie właścicielskiego connectora/operator path. |
