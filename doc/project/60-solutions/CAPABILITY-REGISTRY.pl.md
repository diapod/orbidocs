...
| `sensorium.interface.manage` | `sensorium/interface.manage` | sterowanie hosta | lokalne dla źródła publikowanie, zawieszanie, wycofywanie oraz administracja grantami i revocation | Sensorium Interfaces runtime | nie; tylko host-local | Zaimplementowana capability nie jest reklamowana i nie kwalifikuje się do Passportu. Wymagane pozostają action-specific policy, uwierzytelnione związanie callera, niezmienne fakty zarządcze i rekonstrukcja po restarcie. |
