...
| `corpus.provider` | `app/corpus.provider` | rola aplikacyjnego rozumowania | topic-scoped provider rozumowania Corpus uprawniony do odbioru `corpus-reasoning-query.v1` i zwracania `corpus-reasoning-bid.v1` | przyszłe middleware providera Corpus / oferta publikowana przez Dator | planowane | Capability autoryzuje operacyjną rolę providera; kompetencja tematyczna pozostaje w polach rozszerzenia Corpus w `service-offer.v1`, nigdy w capability id. |
...
| `sensorium.workbench.terminal` | `sensorium/workbench.terminal` | lokalna aktuacja | ograniczona capability PTY/sesji dla Sensorium Workbench | Sensorium Workbench connector | planowane | Powierzchnia efektów wysokiego ryzyka; obecny connector wystawia tylko fail-closed gates z limitami i diagnostyką odmów do czasu store, recovery i adversarial vectors. |
