...
| `room.membership-query` | `app/room.membership-query` | zapytanie aplikacyjne | authority do żądania signer-backed atestacji członkostwa albo grantów w Room | Room primitive / daemon room host | planowane | Pierwsza implementacja to daemon query surface wsparty lokalnym signerem; middleware nie mintują atestacji bezpośrednio. |
