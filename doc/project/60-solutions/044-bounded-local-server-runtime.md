...
**Based on:** `node/code-audit-2.md` A01
**Date:** 2026-04-29
...
1. Should `shutdown_drain_ms` be measured from stop signal to last handler
   return, or from accept-loop exit to last worker return? (MVP records the
   worker drain interval.)
2. Should a later async runtime add a handler deadline that the server enforces (e.g., via
   `set_read_timeout` already applied, but no preemption)?
3. Should the peer supervisor's session worker lifecycle also share a richer
