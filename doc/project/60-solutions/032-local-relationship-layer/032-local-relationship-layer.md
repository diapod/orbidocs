...
1. `relationship-class.v1` retention horizon inherits from the host profile when
   `retention/profile-ref` is absent. A present ref is only a pointer to a
   host-owned Local Relationship retention profile; unknown refs fail closed at
   class write/update, reserved class overrides are disabled in the default host
   policy, and inline retention windows/deletion rules are not part of the class
   contract.
...
| S032-M2 | Pure core | done | `node/local-relationship-core` provides schema-shaped types, validators, reducers, active group resolution, relationship-derived predicate evaluation, owner-scoped membership keys, shared `route_key` canonicalization for contact/route identity matching, and read-model filters without I/O or daemon coupling. |
| S032-M3 | Storage + daemon API | done | Daemon owns `LocalRelationshipStore`, opens `<data-dir>/storage/local-relationships.sqlite`, replays sealed Pseudonym Vault `local-relationship` records at startup, seeds default `untrusted`/`contacts`/`friends`/`trusted` classes from the pure core when the vault is writable, rejects reserved-class archival with `cannot-archive-reserved-class`, permits operator metadata edits on reserved classes, exposes local control API, and advertises guarded host capabilities. SQLite projection carries `transactions`, `events`, `current_classes`, `current_memberships`, `current_nym_bindings`, `predicates`, `predicate_class_ids`, `decisions`, and predicate approval audit facts; projection cells are sealed with the daemon sealer AEAD backend under `local-relationship-projection:v1`, and lookup columns use sealer-derived keyed HMAC indexes. Control writes are vault-first: prepare record, append sealed Pseudonym Vault entry as commit point, then apply projection. Projection failures after vault commit mark `pending-vault-rebuild`; production projection-only mutation helpers are no longer part of the non-test API. Hard-MVP regression tests cover vault rebuild equivalence and plaintext projection leakage. |
| S032-M4 | Operator UI + trust requirements | done | Node UI exposes `/admin/local-relationships` for class, membership, predicate, evaluation, decision-audit, and package trust queue workflows. It distinguishes the four reserved tiers visually, submits predicate requirements as `required/class-ids[]`, validates that at least one class is selected before daemon submission, uses a typed `trusted` secondary-confirmation modal with bounded lifetime before approving predicates that include the top trust tier, and records predicate approval state as `pending`, `approved`, or `rejected`. Rejection requires a reason and writes a vaulted `local-relationship-predicate-approval.local.v1` audit record. Middleware package manifests can declare `trust_requirements[]`; signed package readiness blocks startup until approval, reports `predicate-rejected` after rejection, and shows latest/history approval context in the operator queue. |
| S032-M5 | Messaging canonical integration | done | Daemon `contact-request.accept` appends canonical `contacts` membership and pairwise messaging nym binding after local contact creation. `messaging-service` receives the primary operator `owner/ref` from daemon supervision, reads active `contacts` through `local-relationship.membership.latest`, writes canonical membership through `local-relationship.membership.append`, and fails closed if host/owner configuration is missing. User-mode wizard readiness checks this owner/ref path before entering the app and stores pseudonymous-only contactability without creating a global messaging nym. Daemon tests prove owner-scoped group resolution: owner A membership does not satisfy owner B. Story-010 acceptance tooling now has repeatable `--multi-operator-scope` coverage for contradictory owner-scoped memberships. `status = blocked` is consumed by the schema-scoped `contact-request.v1` AD/INAC preflight as the canonical local deny signal for public contact-request delivery. The pre-release `contacts_membership` cache, bootstrap migration endpoint, and `contacts.membership-changed.v1` compatibility fact have been removed. Story-010 strict `ad-smoke` passes over this canonical-only path. |
| S032-M6 | AD integration + Contact Catalog cleanup + hardening | done | Artifact Delivery has a host-composed dynamic group selector hook and daemon wiring to resolve relationship classes for the primary local operator into ordinary AD recipient selectors when `contact/ref` is directly AD-routable (`node:`, `participant:`, `routing:`), or when a `local-contact:` record carries a local `routing-subject/id` or participant `remote/subject`. Relationship membership/candidate contracts admit directly routable refs while pairwise nym binding contracts remain local-contact scoped. Tests assert empty dynamic groups do not fall back, unroutable relationship groups fail closed, and relationship candidates do not bypass AD outbound authority. Host predicate evaluation verifies available node-operator-binding evidence from the daemon binding store and redacts candidate/evidence detail before returning to middleware. Contact Catalog private-state cleanup is documented. Story-010 acceptance tooling now has one repeatable relationship runner plus a local CI wrapper covering strict canonical delivery, relationship-group delivery, and multi-operator owner-scope checks. AD config validation permits late-bound group references, so Local Relationship groups do not need static placeholder groups. The daemon identity store also imports verified `remote-disclosed` `node-operator-binding.v1` evidence with provenance, rejects inactive or tampered imports, and refuses to overwrite local-issued binding records. Predicate evidence verification rejects bindings revoked by the current revocation view, and Seed Directory revocation sync durably marks matching remote-disclosed binding records as revoked with revocation id and timestamp. |
...
| S032-D3 | Public/federated protocol capability | deferred | Post-MVP only; the MVP boundary remains local-authenticated host API. A federated relationship capability needs a separate threat model, disclosure policy, remote audit semantics, and revocation/freshness contract before implementation. |
| S032-D4 | Richer multi-operator UX | deferred | Post-MVP operator layer for filters, comparison views, owner-scope explanations, and safer bulk review. Current UI remains sufficient for hard-MVP class, membership, predicate, package trust queue, and decision audit workflows. |
| S032-D5 | Relationship cardinality performance profile | deferred | Post-MVP profiling under realistic relationship cardinalities, including projection rebuild time, HMAC index lookup behavior, pairwise nym history growth, and group expansion cost. Current hard-MVP tests prove correctness and fail-closed behavior, not production-scale envelopes. |
...
| S032-04 | Implement `node/local-relationship-core` (pure) | done | Types, validation, projection reducer, active group resolver, predicate evaluator, read-model filters, and shared `route_key` canonicalization for relationship/contact route matching. No I/O. |
| S032-05 | Implement `LocalRelationshipStore` in daemon | done | Store and projection exist with Solution 028-shaped `transactions`, `events`, and `current_*` tables; startup replay from Pseudonym Vault works. Projection payload cells are sealed by the node sealer AEAD backend, and lookup columns are keyed HMAC indexes. Control writes are vault-first and projection failures mark `pending-vault-rebuild`. Regression coverage proves vault rebuild equivalence and absence of plaintext relationship values in projection files. |
| S032-06 | Local-authenticated host capabilities + API surface | done | Control API and guarded host capabilities exist for membership append/latest, group resolve, predicate evaluate, class/membership/nym/predicate management, and decision reads. Direct middleware membership reads remain forbidden; class/predicate management stays control-plane only for hard-MVP. |
| S032-07 | Canonical Messaging integration | done | `messaging-service` reads point membership status from Local Relationship and writes canonical membership through `local-relationship.membership.append`; missing host/owner configuration fails closed. Story-010 strict `ad-smoke` covers the canonical-only path, and Story-010 tooling now has repeatable `--multi-operator-scope` owner isolation coverage. Broader multi-operator UI polish is post-MVP. |
...
| S032-09 | AD `resolve_group` resolver | done | AD group resolution is host-composed through Local Relationship classes and still performs normal outbound/admission authority checks after candidate expansion. Current tests cover dynamic override, empty-group fail-closed, direct `node:`/`participant:`/`routing:` refs, `local-contact:` route/participant mapping, unroutable member failure, and candidate-not-authority. Story-010 tooling now has `--relationship-group-delivery` to promote the accepted contact to `friends` and verify group resolution; the dedicated relationship acceptance runner executes this together with strict canonical and multi-operator variants. |
| S032-10 | Contact Catalog cleanup | done | Proposal/Solution/runbook state that Contact Catalog owns public/federable route-set discovery only; raw handles stay in Local Contact Store and classes/memberships/predicates live in Local Relationship Layer. Future UI wording sweeps remain ordinary regression hardening. |
| S032-11 | Operator UI: class management + membership inspection | done | `/admin/local-relationships` supports class upsert, membership append, predicate registration/evaluation, decision audit, and package trust queue review. Polished filters remain post-MVP UX work rather than a hard-MVP blocker. |
| S032-12 | Replay equivalence + privacy regression tests | done | Pure reducer replay/property coverage and vault unknown-kind roundtrip coverage exist; daemon projection reload, vault rebuild equivalence, and plaintext DB scans cover Local Relationship projection secrecy, and vault-first writes mark projection failures as rebuildable. Broader performance profiling remains post-MVP. |
| S032-13 | Default class seeds for `untrusted`, `contacts`, `friends`, `trusted` (PGP-style trust gradation) | done | Daemon seeds all four reserved classes via `relationship-class-changed.v1` from `default_class_definitions()` when an operator participant and writable vault are available. `untrusted` seed has empty `grant-allowlist`/`suggested-defaults`; `trusted` seed includes `secondary-confirmation` in `verification/required` and a broader `grant-allowlist` (custody, delegation). Reserved classes cannot be archived through the daemon API, but their metadata can be edited by the operator. |
...
| S032-15 | Read-model level enforcement | done | Pure read-model filters exist. Middleware predicate evaluation returns redacted decision data rather than host-internal candidates; direct middleware membership listing remains unavailable. Operator queue and projection privacy tests cover the hard-MVP read surfaces. Broader audit-log privacy sweeps remain post-MVP hardening. |
| S032-16 | Host policy evaluator for predicates | done | Pure evaluator is callable through daemon control API and the redacted `local-relationship.predicate.evaluate` host capability. Decisions are appended to the canonical local-relationship log. Host capability responses omit internal candidates and redact evidence refs. Richer dispatch-path policy composition remains a post-MVP extension. |
| S032-17 | Middleware package manifest `trust_requirements` declaration mechanism | done | Parser and readiness gate are implemented for signed middleware packages: a package with unapproved trust requirements is blocked until the referenced predicate is approved, and a rejected predicate blocks with `predicate-rejected`. Predicate approval/rejection is stored as a vaulted local audit record and exposed through the package trust queue with latest/history context. |
| S032-18 | Node-operator-binding evidence integration | done | Pure evaluator requires a node-operator-binding evidence ref and rejects mismatches. Daemon predicate evaluation verifies available binding evidence against the local node-operator-binding store before invoking core and fails closed by clearing unverified hints. The identity control surface now imports verified `remote-disclosed` node-operator-binding evidence with provenance, rejects inactive or tampered bindings, and refuses to overwrite local-issued binding records. Predicate evidence verification rejects imported bindings revoked by the current revocation view, and Seed Directory revocation sync durably marks matching remote-disclosed binding records as revoked with revocation id and timestamp. |
| S032-19 | `owner/ref` multi-operator query scoping | done | Pure membership keys and group resolution are owner-scoped; daemon injects the primary operator owner into supervised messaging; tests verify that memberships for owner A do not satisfy owner B; Story-010 tooling now has repeatable `--multi-operator-scope` coverage. Richer multi-operator UI surfaces remain post-MVP polish. |
...
| S032-22 | Public protocol capability for federated consumers | deferred | Not in this iteration; local-authenticated host API only. A federated relationship capability needs a separate threat model, disclosure policy, remote audit semantics, and revocation/freshness contract before implementation. |
| S032-23 | Registry-driven pairwise nym context kinds | done | `context/kind` is now registry-driven and seeded with `messaging`, `ad-direct`, `agora-topic`, and `inquirium-session`. The schema contract exposes the registered enum, `local-relationship-core` validates pairwise nym facts/projections against the seed registry, and unknown context kinds fail closed in tests. |
| S032-24 | Relationship retention profile inheritance | done | `relationship-class.v1` now treats absent `retention/profile-ref` as inheritance of the explicit `local-relationship/default` profile and validates any present ref as a narrow host-owned profile pointer. `local-relationship-core` rejects inline `retention/*` lifecycle policy fields, daemon class writes and vault replay fail closed for unknown profile refs, reserved class overrides are disabled by default, store status reports `all-profiles-resolved` only when every ref is known, Node UI displays inherited/explicit retention refs, and tests cover inheritance, replay refusal, unknown-profile refusal, reserved override refusal, and known custom profile admission. |
| S032-25 | Richer multi-operator UX layer | deferred | Post-MVP operator layer for filters, comparison views, owner-scope explanations, and safer bulk review. Current UI remains sufficient for hard-MVP class, membership, predicate, package trust queue, and decision audit workflows. |
| S032-26 | Relationship cardinality performance profile | deferred | Post-MVP profiling under realistic relationship cardinalities, including projection rebuild time, HMAC index lookup behavior, pairwise nym history growth, and group expansion cost. Current hard-MVP tests prove correctness and fail-closed behavior, not production-scale envelopes. |
