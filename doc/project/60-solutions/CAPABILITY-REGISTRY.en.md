...
| `room.membership-query` | `app/room.membership-query` | application query | authority to request signer-backed Room membership or grant attestations | Room primitive / daemon room host | planned | The first implementation is a daemon query surface backed by the local signer; middleware does not mint attestations directly. |
