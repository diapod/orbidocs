...
| `sensorium.interface.manage` | `sensorium/interface.manage` | host control | source-local publication, suspension, withdrawal, grant, and revocation administration | Sensorium Interfaces runtime | no; host-local only | The implemented capability is non-advertisable and Passport-ineligible. Action-specific policy, authenticated caller binding, immutable management facts, and restart reconstruction remain mandatory. |
