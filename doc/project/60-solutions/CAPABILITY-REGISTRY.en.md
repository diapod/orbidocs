...
| `messaging-receive` | `app/messaging-receive` | application consent | narrow recipient-issued authority allowing one sender subject to deliver messages to one accepted route | messaging middleware / Contact Catalog contact-request acceptor | planned | Used by Story 010 as the concrete passport minted after accepting a contact request. It does not grant friend-class capabilities. |
| `messaging-send` | `app/messaging-send` | application authorship | participant-side authority to sign and enqueue outbound messaging envelopes for a local messaging client | messaging middleware / Node signing host | planned | Signing delegation uses `signing/messaging-send`; receive consent remains a separate `messaging-receive` passport. |
