...
| `corpus.provider` | `app/corpus.provider` | application reasoning role | topic-scoped Corpus reasoning provider eligible to receive `corpus-reasoning-query.v1` and return `corpus-reasoning-bid.v1` | future Corpus provider middleware / Dator-published service offer | planned | The capability authorizes the operational provider role; topic expertise remains in `service-offer.v1` Corpus extension fields, never in the capability id. |
...
| `sensorium.workbench.terminal` | `sensorium/workbench.terminal` | local actuation | bounded PTY/session capability for Sensorium Workbench | Sensorium Workbench connector | planned | High-risk effect surface; must use explicit Workbench policy, caps, audit, cleanup, and refusal diagnostics. |
| `sensorium.workbench.file` | `sensorium/workbench.file` | local actuation | bounded file snapshot/read surface under leased workspace roots | Sensorium Workbench connector | planned | Uses shared actuation path rules; it is not ambient filesystem authority. |
| `sensorium.workbench.patch` | `sensorium/workbench.patch` | local actuation | bounded patch application surface under leased workspace roots | Sensorium Workbench connector | planned | Patch execution requires provenance, digest checks, policy gates, and operator approval unless an explicit lease permits it. |
| `sensorium.workbench.env` | `sensorium/workbench.env` | local actuation | bounded environment/sandbox lifecycle surface for Workbench sessions | Sensorium Workbench connector | planned | Environment grants are narrower than general Sensorium authority and must expose cleanup/recovery status. |
