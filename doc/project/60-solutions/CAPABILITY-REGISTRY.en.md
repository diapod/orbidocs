...
| `sensorium.workbench.terminal` | `sensorium/workbench.terminal` | local actuation | bounded PTY/session capability for Sensorium Workbench | Sensorium Workbench connector | planned | High-risk effect surface; current connector exposes only fail-closed gates with caps/refusal diagnostics until store, recovery, and adversarial vectors are wired. |
