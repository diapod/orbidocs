...
The carrier-neutral pull-batch core, durable host runtime, Sensorium and Workbench
source adapters, local host-capability surface, authenticated direct-peer surface,
local SSE adapter, and WSS-backed Room `latest-state` projection are implemented.
...
The resource is not a connector, transport, store, or capability. Sensorium and
Workbench own source facts. Interaction Broker owns bounded source observation.
...
Sensorium observations and Workbench terminal representations need a shared way
to be deliberately exposed without giving consumers access to connector
credentials, terminals, or private provider stores. Implementing each exposure
as a separate stream API would duplicate authorization, cursor, replay,
classification, revocation, and backpressure semantics.
...
Workbench terminal screen, and a bounded terminal event batch structurally
comparable while preserving their distinct schemas and delivery semantics.
...
Interaction Broker source-provider adapter
  bounded source watch/probe mechanics and provider-specific adaptation
...
1. Collect read-next latency, no-change rate, batch occupancy, and revocation-lag
   metrics under representative local and peer workloads.
