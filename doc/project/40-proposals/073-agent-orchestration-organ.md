...
| `agent-assistant-surface` | Let the assistant (P066) drive an Agent under explicit capability and human-in-loop. | `deferred` | Assistant stays advise-only baseline; agentic escalation requires capability + operator approval; no ambient initiation. The frozen acceptance default requires no second operator question merely to render a valid, same-session `assistant-response-draft`; rendering grants neither effect nor publication authority. Admission and durable recovery share an explicit fail-closed guard that names the still-unimplemented escalation-authority and output-acceptance contracts. Tests prove an attempted assistant-channel binding appends no durable facts. |
