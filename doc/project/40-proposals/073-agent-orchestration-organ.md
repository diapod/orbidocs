...
| :--- | :--- | :--- | :--- |
| `agent-core-crate` | Create thin `agent-core` contract crate with Agent/controller/budget/grant DTOs and the lifecycle state machine. | `todo` | DTOs validate spec, reject unknown fields, fail-closed classification; state machine enforces legal transitions; depends only on classification + inquirium-core + serde/error. |
| `agent-dep-direction-lint` | Add a dependency-direction lint so `agent-core` cannot import substrate. | `todo` | CI fails if `agent-core` imports daemon/model-runtime/HTTP/async/SQLite crates, mirroring `inquirium-core`. |
