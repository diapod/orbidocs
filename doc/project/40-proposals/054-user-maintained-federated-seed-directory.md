...
| P054-08 | Add multi-directory client query policy | done | Daemon-owned Seed Directory consumers use `preferred-directory`, `quorum`, or `weighted-trust` policy through one source eligibility and merge layer implemented in the `seed-directory-client` host-service crate; embedded local directory counts in quorum/weighted only by explicit opt-in. |
...
| P054-13 | Reuse P081 signed bounded replication profiles across trusted directories | done | Capability and revocation lanes now reuse the P081 profile/fact core while retaining Seed Directory source trust and multi-directory policy. The direct hard-MVP profile binds `origin/node-id` to the authenticated source, verifies the source batch signature and every domain artifact separately, persists accepted facts through the existing store, and refusal-tests source spoofing, digest mismatch, rollback, restart resume, and revocation dominance. |
