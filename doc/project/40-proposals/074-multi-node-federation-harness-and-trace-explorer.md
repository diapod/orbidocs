...
5. The selected Qwen2.5-Coder 3B reviewer subsequently passed two fresh,
   ...
   `node-c`: 5/5 complete-plan acceptances and 5/5 repairable corrections. The
   respective observed p95/max durations were 156,195 ms and 156,728 ms. Bounded
   reviewer-only correction remains recovery for this symmetrically qualified
   binding; it is not a mechanism for coercing a verdict.
...
3. Design and review the generic interrupted-run checkpoint/replay boundary
   before implementing P074-008 and the optional exploration posture P074-022;
   Story-owned ephemeral state must not leak into the harness core.
...
| P074-008 | Add an any-host resumable local/SSH orchestrator | partial | `federation_harness_runtime.py` and the bounded host agent share one local/SSH control interface, parallel preflight, checked planning, owner-first leases, host-local execution fences, an idempotent hash-chained owner ledger, sanitized child environments, supervised PID-reuse-safe processes, and bounded private evidence transfer. Physical identity accepts only the expected hostname/FQDN or an expected non-loopback DNS address owned by the local kernel; SSH carries control/evidence and never product traffic. Unit coverage includes competing/stale leases, ledger conflicts, path escape, partial-host aggregation, shell noise, clock skew, host identity, host-only locator stripping, and no forwarding. Retained deterministic and real-model physical runs have now proved host-local model roots, vfkit lifecycle, direct product traffic, restart, evidence, cleanup, and lease release. It remains `partial` only because a generic interrupted-run resume/replay boundary has not yet been implemented and exercised; P074-022 is the separately optional retain-for-exploration posture. |
...
| P074-022 | Add an explicit resumable exploration passage distinct from acceptance evidence | todo | Define a non-default `exploration-retain` execution posture that may preserve healthy daemons, model runtimes, VM workspaces, and supervised control sessions after a repairable harness or calibration failure. The owner ledger must record a typed checkpoint, retained component identities, completed and ambiguous effects, lease status, configuration/topology/profile digests, and the exact next resumable step. Resume must revalidate process identity and health, model and image target bytes, storage policy, repository revision, clock bounds, Room membership, grants, leases, and every authority expiry; it must restart only invalidated strata and must refuse changed digests, expired or widened authority, uncertain effect completion, lost supervision, or a checkpoint after cleanup. A separate bounded cleanup command must release the lease and remove all retained private state. This posture is diagnostic only: promotion still requires a fresh default run from preflight through post-effect revalidation and cleanup, with no retained checkpoint as acceptance evidence. Before implementation, review which Story-owned values can be reconstructed from durable facts and which ephemeral Room/Sensorium/VM states require an explicit replay boundary, so the generic harness does not absorb Story semantics. Depends on P074-008 and P074-021; it does not gate the first clean P074-010 report. |
...
| P074-025 | Qualify a hardware-appropriate physical reviewer binding for `node-c` | done | P074-014's Qwen2.5-Coder 7B remains immutable correctness evidence but exceeded the current two-core Broadwell timing envelope. The selected Qwen2.5-Coder 3B Q4_K_M candidate uses native GBNF plus host semantic validation, a conservative 6 GiB host minimum, and an exact Reviewer plus Facilitator qualification. Production-shaped work exposed that narrative-first grammar anchored verdicts on the pre-effect `NXDOMAIN`; placing `verdict` first for non-revision reviews fixed the representation without weakening the JSON contract or host authority. On the actual two-core Broadwell `node-c`, two fresh no-retry populations then passed 5/5 complete-plan acceptances and 5/5 repairable corrections, with observed p95/max durations of 156,195 ms and 156,728 ms. The retained 2026-08-28 P074-010 run admitted this exact binding, produced the real reviewer product, and passed its post-effect revalidation. Exactly one fresh reviewer-only correction turn is allowed only after this symmetric qualification and over the same immutable proposal and host facts; a second invalid review fails closed, and the host never synthesizes or coerces `accept`. |
