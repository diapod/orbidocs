...
3. Freeze and implement the typed Reviewer-claim contract under P074-033 before
   using P074-029 for the exact three-host P074-022 closure. P074-030 through
   P074-032 retain the completed two-host diagnostic comparison without
   weakening the three-host evidence requirement. Story-owned ephemeral state
   remains in the thin Story adapter.
...
| P074-033 | Replace Reviewer prose adjudication with typed host-checked claims | todo | Freeze a versioned, closed Reviewer-claim vocabulary before implementation. A claim must identify its subject (`proposal`, `pre-effect-snapshot`, `package-reference`, or `post-effect-observation`), closed predicate, polarity, exact directive or resource token, and host-owned evidence reference; explanatory prose remains inert and cannot change the verdict. The host compares these values with the same immutable facts that authorize the effect, rejects unknown, absent, contradictory, duplicated, or unsupported claims, and never supplies a default, coerces a verdict, or converts narrative text into authority. Update the Story-owned response contract and runtime constraint where supported, prompt/framing, parser, qualification identity, private report shape, and focused correction populations. Retain the current prose variants as migration/refusal fixtures, then remove `_review_claims_proposal_omits_directive` and equivalent natural-language verdict gates only after the typed path passes fresh no-retry Reviewer qualification and the full Story acceptance contract. This task depends on P074-027 and P074-031 and gates the P074-029 adapter qualification; it does not invalidate the already retained P074-032 diagnostic evidence. |
