...
Resolved 2026-09-01 (profile-scoped two-host evidence):

1. Shared physical placement remains a fact, not an automatic synonym for weak
   evidence. The generic `physical-two-host-three-node` profile keeps its
   conservative `diagnostic-not-promotable` default, and every
   `exploration-retain` passage remains diagnostic. A Story profile may request a
   `profile-defined` ceiling only by binding an explicit claim ref and exact
   logical-node, physical-host, and shared-failure-domain cardinalities into the
   checked scenario and exported aggregate.
2. Promotion is limited to that exact claim. A passing Codex-backed Story-012
   profile may prove real two-host transport, three distinct logical Node and
   Agent identities, Room participation, interrupted/resumed execution, cleanup,
   fresh execution, and Story assertions. It cannot satisfy
   `physical-three-host-full-system`, independent three-host failure tolerance,
   or the exact three-host closure clauses of P074-008/P074-022.
3. The first such profile places `node-b` and the Codex-backed Reviewer `node-c`
   on `self.local` and `node-a` on `turbo.local`. This placement is development
   evidence only; the later `cyc.local` run changes topology, not the Agent,
   runtime, Room, typed-review, or Story contracts.
...
Implementation evidence recorded 2026-09-03 (three-host Codex Reviewer):

1. The exact `story-012-physical-three-host-three-node-codex-reviewer` scenario
   placed Chair `node-a`, Solver `node-b`, and Reviewer `node-c` on three
   distinct physical hosts. Its checked aggregate claim is
   `acceptance-claim:story-012-three-physical-host-three-node-codex-reviewer-v1`;
   there is no shared physical failure domain. The Codex-backed Reviewer remains
   an ordinary durable Agent and Room participant with its own identity,
   reservation, product, session, trace, and node-local evidence. This is online
   provider-backed interoperability evidence, not offline or local-model
   qualification for the Reviewer.
2. The retained restarted passage
   `federation-run:story-012-physical-three-host-three-node-codex-reviewer:20260903T181349Z`
   restarted `node-c` after its first review and resumed the same durable
   Reviewer-Agent/runtime session. It passed all ten Story assertions. Its
   Schema-Gate-valid aggregate and paired Story report have SHA-256 values
   `4441c9a85742ef0facce73e58fbe8016a2b177404a9350f727067fa067441c61`
   and
   `75740180f08605647d048917ee66fb8e87736a964f72c82f2e5ea272e4cbc826`.
3. The independent fresh passage
   `federation-run:story-012-physical-three-host-three-node-codex-reviewer:20260903T185604Z`
   created new run and provider-session roots, completed five real deliberation
   cycles and five model-authored experiments without fallback, and passed all
   ten Story assertions in 240,693 ms of deliberation and 437,538 ms total. Its
   Schema-Gate-valid aggregate and paired Story report have SHA-256 values
   `7375be7d79d7dc750ec40d98d07954b9e5730e5db645d34d86ec742d03f6f408`
   and
   `b965c4bc3ad2f6da22314671741a908b19427abd47fcb0c2408afcdeb17baf11`.
   The retained evidence includes exact A/B/C DNS results, complete no-drop/no-gap
   communication traces, model and external-Agent post-effect revalidation,
   vfkit cleanup, all lease releases, and zero redundant peer-session workers on
   every host.
4. These passages complete P074-029. The fresh zero-redundancy observation also
   completes P074-038's live convergence evidence. They do not complete the
   generic P074-022 `exploration-retain` fault/resume exercise or the injected
   transient Room WSS reset required by P074-037, and they do not retire the
   compatible v2 prose path still keeping P074-033 partial.
...
3. Complete P074-033 by migrating or retiring the compatible critique-gated v2 prose
   path. The bounded Codex-backed Reviewer adapter and its exact three-host
   Story-012 claim are now runtime-complete under P074-029; this does not by itself
   complete P074-022's separate exploration-retain interrupt/resume contract.
   Story-owned ephemeral state remains in the thin Story adapter.
4. Complete P074-040 before adding another promotable acceptance profile: move the
   closed profile-claim admission set out of the generic federation-run schema and
   into a separately versioned, locally admitted registry without weakening the
   current posture-to-claim checks.
5. Complete P074-011 for immutable shared inputs, shared VMM workspace roles,
   executable/build-hook staging, and retained udev/autofs/remount evidence.
6. Decide whether to finish the Bielik-specific P074-012 admission branch or
   retain it as non-gating alternative evidence beside the completed P074-026
   profile.
7. Add the optional Matrix smoke profile under P074-006 after the generic Story
   010 consumer is stable.
8. Implement the optional macOS signing/firewall hardening in P074-020 without
...
| P074-038 | Converge simultaneous peer handshakes without replacing a live canonical session worker | done | Inbound/outbound races now converge on the deterministic Node-id-selected physical socket; canonical replacement inherits the shared pending-response registry, closes and joins the superseded worker outside the registry lock, and identity-fences stale cleanup. Failed current workers still release their slot for ordinary targeted recovery. Candidate ranking excludes the exact local Node id, ordinary same-identity handshakes fail before replay/quality mutation, and only the signed `seed-directory-address-verification` challenge reaches the listener. That exact probe is terminated immediately after reachability evidence instead of entering the peer store or spawning a durable session worker. Focused tests cover direction choice, replacement, pending-response preservation, redundant shutdown, self-candidate and self-handshake refusal, exact/near-miss probe shapes, and later recovery. In the fresh green P074-029 run, concurrent Seed Directory, Corpus fanout, and Room-invite traffic passed without acknowledged INAC loss; all three retained host logs contain zero `peer_session_worker_redundant` events and all communication traces closed without drops or gaps. This remains transport liveness only and adds no retry, timeout widening, or message/capability relaxation. Depends on the peer-runtime session registry, P074-009, and P074-029. |
| P074-039 | Fail fast on selected external Agent runtime admission before expensive physical Story bootstrap | done | The reusable physical Story harness exposes an optional provider-neutral callback after host preflight and host-private path materialization but before its first build, model start, or vfkit effect. It records only a typed pass/refusal step and releases the acquired run lease on refusal. The first Story-012 consumer runs the exact materialized `node-c` Codex interpreter/helper/workspace/model environment through a bounded probe, verifies pinned SDK/App Server/helper identity and the current operator session, accepts only one exact canonical redacted result, denies tools, creates no provider thread, and retains no qualification artifact. Missing configuration, malformed/extra/noncanonical output, helper drift, an unsupported platform, and runtime unavailability fail closed; absence of a callback preserves the existing bootstrap path. Focused suites pass on macOS and GNU/Linux. Live `cyc.local` execution first returned the expected `external-runtime-unavailable` refusal for its expired operator session; after ordinary operator reauthentication the same probe passed with the exact Linux x86_64 SDK/App Server/runtime/helper pins, `deny-all`, and `session/created=false`. This readiness diagnostic does not replace durable P089 qualification or Agent-owned execution evidence. Depends on P074-016, P074-029, P074-035, and P089-008c. |
| P074-040 | Move promotable acceptance-claim admission out of the generic federation-run schema | todo | Restore `profile-claim/ref` in `federation-run.v1` to a bounded namespaced syntax contract and introduce a separate versioned, locally admitted registry mapping each claim ref to its required host posture, network profile, report profile, evidence boundary, and permitted evidence ceiling. The harness must validate the selected claim against that registry before aggregate emission; the registry is policy input, not run-authored self-attestation. Schema Gate and harness tests must reject unknown claims, posture/profile/boundary mismatches, absent admission, and attempts to use a diagnostic claim for promotion. Migrate both current Story-012 Codex Reviewer claims before removing their literals from the generic schema, preserve validation of retained reports, and keep Story vocabulary out of the generic contract. This stratification is required before claim number three is admitted. Depends on P074-003, P074-029, and P074-034. |
...
| P074-029 | Add a Codex-backed Reviewer adapter for the physical `node-c` profile | done | P089-008 supplies the bounded pinned adapter and P089-008c first proved it as a real `node-c` Reviewer Agent in the explicit two-host/three-node Story profile. The `story-012-physical-three-host-three-node-codex-reviewer` scenario moves that unchanged Agent contract to independently admitted `cyc.local` and binds `acceptance-claim:story-012-three-physical-host-three-node-codex-reviewer-v1` to three distinct physical hosts. The restarted passage `federation-run:story-012-physical-three-host-three-node-codex-reviewer:20260903T181349Z` resumed the same durable Reviewer-Agent/runtime session after restarting `node-c`; its aggregate and Story-report SHA-256 values are `4441c9a85742ef0facce73e58fbe8016a2b177404a9350f727067fa067441c61` and `75740180f08605647d048917ee66fb8e87736a964f72c82f2e5ea272e4cbc826`. The later fresh run `federation-run:story-012-physical-three-host-three-node-codex-reviewer:20260903T185604Z` used a new run and provider session, passed five real cycles and five model-authored experiments without fallback in 240,693 ms of deliberation and 437,538 ms total, then independently proved restart/rejoin, revocation, post-effect revalidation, trace closure, vfkit cleanup, and lease release. Its Schema-Gate-valid aggregate and Story-report SHA-256 values are `7375be7d79d7dc750ec40d98d07954b9e5730e5db645d34d86ec742d03f6f408` and `b965c4bc3ad2f6da22314671741a908b19427abd47fcb0c2408afcdeb17baf11`. Both passed all ten Story assertions and exact A/B/C DNS results. The Agent owns its Room identity, durable session, finite reservation, typed product, signed review, communication trace, restart evidence, and node-local report; tools and approvals remain denied, provider-native state remains absent from Room/Corpus contracts, and host facts own claim adjudication. This is online Codex-backed interoperability, not local-model or offline acceptance; the GGUF binding remains separate evidence. P074-022's generic exploration-retain interrupt/resume exercise remains independent. Depends on P089-008, P074-008, P074-017, P074-021, P074-022, P074-027, and P074-033. |
...
| P074-033 | Replace Reviewer prose adjudication with typed host-checked claims | partial | The generic `corpus-deliberation-review-claims.v1` envelope and signed `corpus-reasoning-experiment-review.v3` are canonical, mirrored into Node, Schema-Gate admitted, and validated by the pure Corpus core. They carry opaque subject, predicate, object, modality, evidence, disposition, and next-move refs plus bounded semantically inert commentary; they install no PowerDNS or universal domain ontology. Story 012 owns the first closed `review-claim-profile:story-012-powerdns-v1` catalog-select specialization. The host projects immutable admissible claims from the same plan, policy, lifecycle, and observation facts used for effect admission, and accepts only exact catalog entries and a profile-legal next move. Unknown combinations, contradictions, duplicates, missing evidence, unsupported subjects, or narrative attempts to select disposition/effect/test fail closed. Typed correction state, signed v3 lineage, report projection, qualification identity, and focused malformed/correction tests are implemented. Reviewer-contract selection is now an explicit task-profile property independent of runtime topology, every current discovery transport uses v3, prompt and bench helpers require an explicit mode, and the bench defaults to typed JSON. The compatible critique-gated v2 path still uses prose adjudication, including `_review_claims_proposal_omits_directive`; therefore the replacement criterion is not yet complete. P074-031 supplied the fresh 5/5 no-retry whole-JSON Reviewer qualification because MLX 0.30.2 has no native grammar or JSON-Schema boundary; this retained measurement used the typed path despite the previously weaker generic bench default. The full run `federation-run:story-012-physical-two-host-three-node-real-model:20260831T193333Z` then passed with three real runtimes, four model-authored experiments, four typed envelopes with empty inert commentary, no fallback, exact `a`/`b`/`c` DNS assertions, closed traces, cleanup, 701,996 ms of deliberation, and 958,375 ms total. Its paired Story report has SHA-256 `759145ed615bf6d4a823d30b501f655c159456691ee0bbc25f39a063867fe3e7`. The real passage used the direct valid typed path; correction retry remains focused-test evidence. Closure requires migrating or retiring the v2 prose path and deleting its NL adjudicator after its compatibility decision. That typed MLX run retains `diagnostic-not-promotable`; the separate P074-034 Codex claim is `profile-defined` only for its exact declared two-host/three-node boundary. Generic profile admission remains P069-DOMAIN-005 and the real third-host Codex exercise remains P074-029. Depends on completed P069-CLAIM-001, P074-027, and P074-031. |
| P074-034 | Add an explicit promotable `physical-two-host-three-node-codex-reviewer` Story-012 claim without inflating it into three-host evidence | done | The checked scenario and aggregate admit only `acceptance-claim:story-012-two-physical-host-three-node-codex-reviewer-v1`, binding three logical Nodes, two physical hosts, and the declared `node-b`/`node-c` shared failure domain. Default shared-host and exploration evidence remain `diagnostic-not-promotable`; only this exact claim exports `profile-defined`. Canonical `federation-run.v1` currently encodes the closed V1 Story-012 claim set as an enum and Schema Gate rejects invented or posture-mismatched claims, so this evidence ceiling is not merely a Python-harness convention. This deliberate short-term closure now exposes Story vocabulary in the generic run contract; P074-040 owns migration to a separately admitted registry before another claim is added. The restarted passage `federation-run:story-012-physical-two-host-three-node-codex-reviewer:20260901T031054Z` restarted `node-c` after the first Codex review, continued the same durable Agent session, passed six deliberation cycles and five experiments, and completed cleanup; its aggregate and Story report SHA-256 values are `930a590f6c68ffa7cb08c91a16bd7addb465a17662eb89b2343cc4e4ae6c81da` and `a9dc132807c37c3931f15dcd5659f62e15df599451fa7d25e80e8909e5e9b8bf`. The later fresh run `federation-run:story-012-physical-two-host-three-node-codex-reviewer:20260901T032223Z` created new run and provider-session roots, passed five cycles/five experiments, restarted and rejoined `node-c`, then cleaned every resource; its aggregate and Story report SHA-256 values are `8e2ae6dedb32109de95acf36cef1df97a91de109cc461c5207752f6f0defd253` and `95b81360d8a22fa67779de66ab69c9bc969305fd816cea8ded01ee8bcedd4965`. Both runs passed all ten Story assertions, exact DNS results, typed claims, communication traces, model post-effect revalidation on `node-a`/`node-b`, external-Agent status/revocation revalidation on `node-c`, distinct evidence ownership, and no provider leakage into Room/Corpus contracts. This does not close the three-host clauses of P074-008/P074-022. Depends on P074-008, P074-016, P074-021, P074-029, P074-030, and P074-033. |
