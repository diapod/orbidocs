...
| P074-008 | Add an any-host resumable local/SSH orchestrator | partial | `federation_harness_runtime.py` and the bounded host agent now share one local/SSH control interface, run parallel host/platform/revision/topology/profile/root/clock preflight, invoke the checked Rust planner, acquire the owner lease before identical host-local execution fences, retain an idempotent append-only hash-chained owner ledger, sanitize child environments, bind effects to a live lease and host-owned cwd, and fence PID reuse. Strict preflight rejects symlinked, group/world-writable, and known removable/automatic authority roots without changing permissions; selected shared/removable model storage still requires P074-011. Unit coverage proves competing and stale lease refusal, idempotent/conflicting ledger behavior, path escape refusal, partial-host aggregation, shell-noise refusal, clock skew, host-only locator stripping, and absence of SSH forwarding. Live read-only preflight on 2026-08-27 confirmed one common repository HEAD and the three configured slots, while the full runner correctly refused before effects until the remote checkout locators are configured. A clean synchronized physical run with retained node evidence, real-model per-slot root injection, and vfkit-specific checks remain before `done`. Depends on P074-007. |
| P074-009 | Prove direct transport across three physical hosts | partial | The Story-012-shaped deterministic profile now composes the generic checked plan and any-host executor, binds its exact three-assertion contract before effects, builds the existing P070 deployment binary per host, binds a host-TLS Room relay to the planned topology endpoint on `node-a`, compares DNS identity case-insensitively, publishes four `room-live-message.v2` deliveries from `node-b`, receives the exact replay under observation-only authority on `node-c`, gracefully restarts the relay, reconnects at the exact checkpoint, records redacted assertion results, removes private credential/key material, and retains a Schema-Gate-validated redacted `federation-run.v1` manifest. Unit evidence covers topology/profile and assertion drift surfaces, stale and competing leases, partial-host preflight, clock skew, PID reuse, path escape, and absence of SSH forwarding. A local runtime check passed four-frame TLS publish/observe with one exact checkpoint. Live three-host invocation on 2026-08-27 still refused before effects after host-local locator configuration because the two remote synchronized revisions do not yet contain the uncommitted host agent. The clean retained physical report remains before `done`. Depends on P074-008 and the relevant P056/P070 deployment contracts. |
...
| P074-015 | Freeze the host-local Node checkout locator | partial | Keep repository paths out of the reusable topology. Resolve the remote control agent through an explicit host-local `ORBIPLEX_ACCEPTANCE_NODE_ROOT`, require an absolute canonical Git worktree whose agent path is below that root, compare the resulting revision/profile evidence across hosts, and remove this locator from every product child environment and retained aggregate. The local executor may bind its own current checkout directly. The variable, refusal behavior, installed-agent execution, and host-local configuration on all three hosts are present. Live diagnostics on 2026-08-27 found that the two remote checkouts still lack the uncommitted agent file, so synchronized revision evidence remains before `done`. Supports P074-008. |
