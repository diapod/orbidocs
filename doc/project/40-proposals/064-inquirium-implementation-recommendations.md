...
| `inq-http-adapter-instance-handles` | Key HTTP lifecycle handles by adapter instance while invoking by selected runtime candidate. | `done` | `node/model-runtime-http` accepts `RuntimeInvocationContext`, supports one HTTP adapter instance serving multiple runtime candidates, and rejects caller override of host-owned model keys. Daemon `inquirium.generate` now invokes HTTP adapter instances through this context, and Story 005 verifies the path with the opt-in simulator runtime. |
...
| `inq-embedding-contracts` | Add explicit direct and batch embedding request/response contracts. | `done` | `node/inquirium-core` owns `inquirium.embed.{request,response}.v1` and `inquirium.batch-embed.{request,response}.v1` DTOs with validation for schema, operation, dimensions, leases, vector shape, finite vector values, and stable slash-scoped artifact/lease refs. `node/model-runtime` re-exports these DTOs for runtime-facing compatibility. |
...
| `inq-cost-budget-enforcement` | Make token and cost budgets enforced contracts with metering and a typed exhaustion outcome. | `in-progress` | `node/inquirium-host` now owns the pure generate budget snapshot: runtime default/ceiling, adapter `max-output-tokens`, effective ceiling, safety-critical denial, trace projection, and prompt-token estimate. `node/daemon` supplies host-local rail state and adapter limits as data, injects default or host-ceiling `max_tokens` into adapter parameters for safety-critical paths, returns typed `budget_exceeded` denials when the effective output-token budget exceeds the selected ceiling or when safety-critical generate has no host-owned ceiling, and records request/default/effective token budget, runtime ceiling, adapter ceiling, effective ceiling, safety-critical flag, caller-turn token estimate, and cost tier in generate trace diagnostics while omitting unset optional fields. Remaining work: per-session counters, provider-reported output token/cost metering, repair/cache spend, and stronger assembled-prompt token accounting. |
...
| `inq-effect-intent-vocabulary` | Define returned-value effect intents such as `GenerateDecision`, `TraceFact`, `TranscriptWriteIntent`, and `BudgetCharge`. | `in-progress` | `node/inquirium-host` now owns the first typed returned-value effect intent vocabulary: `TraceFactIntent`, `TranscriptWriteIntent`, expanded `BudgetCharge`, and the `InquiriumEffectIntent` sum type, all as serializable data with validation, required idempotency/correlation handles for effect execution, final-charge operation/runtime invariants, and no daemon write ports. The daemon still executes the existing trace/transcript/budget effects through its current composition-root paths; the remaining work is to have operation decisions return and the daemon consume these intents as the common effect execution path. |
| `inq-effect-intent-execution-path` | Make returned-value effect intents the active daemon execution path across Inquirium. | `todo` | Follow-up from the resolved Effect Intent Consumption Boundary decision. Define operation-level decision values for generate, embed/batch-embed where relevant, and assistant turn that return response/denial plus effect intents; then have the daemon execute trace, transcript, budget, artifact, and other write effects from those intents through existing stores. Do not add write ports to `inquirium-host`; daemon remains the effect authority. Completion requires tests proving idempotency/correlation handles are consumed, duplicate replay does not double-write, and old daemon-local ad hoc write assembly has been removed or narrowed to an interpreter for the intents. |
