...
| :--- | :--- | :--- | :--- |
| `inq-execution-posture-contract` | Bind runtime/profile admission and preflight disclosure to canonical `inference-execution-posture.v1`. | `partial` | `inquirium-host` has a pure conservative derivation path over explicit host-owned context, and shared requirements reject observation-only `unknown`/`derived` evidence bases. Runtime/profile preflight exposure bound to current generation, validity, and invocation scope plus its negative integration fixtures remain open. The value grants no inference, egress, context, file lease, or effect authority. |
