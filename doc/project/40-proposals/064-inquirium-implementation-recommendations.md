...
| `inq-io-rails` | Make input/output rails explicit boundary data independent of the model. | `in-progress` | `node/daemon` now owns `inquirium.io_rails` and enforces the first deterministic host rail slice around `inquirium.generate`. Rails are ordered marker data, configurable by distributor/operator config, not regex code: input rails deny explicit secret/credential assignment markers before routing or selected-runtime invocation, while PII/self-harm material in input remains admissible for analysis/citation; output rails inspect normalized text, structured string leaves, and the response envelope defensively for secret, PII, self-harm, and content-policy markers after model return. Rail decisions carry safe `io_rail/policy_ref` diagnostics, and unsafe output preserves usage/cost diagnostics for accountability while suppressing releasable content. Remaining work: schema-aware redaction, egress-class limits, signed/versioned evaluator critics, and tests proving provider-native rails are only optimization hints and never remove host-critical checks. |
