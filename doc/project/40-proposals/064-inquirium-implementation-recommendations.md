...
| `inq-python-remote-provider-adapters` | Add first middleware-hosted remote provider adapters while preserving adapter-instance/runtime-candidate stratification. | `done` | The OpenAI adapter maps neutral generation and host-authorized JSON Schema to Responses; the Anthropic adapter maps the same neutral contract and schema subset to Messages. Both share `node/middleware-modules/lib/inquirium_adapter`, expose neutral and chat-compatible endpoints, read secrets from env/file config, normalize provider-constrained JSON into the neutral structured envelope, and have fake-provider coverage. Host validation and rails still run after normalization. `model-runtime-http` also verifies one managed adapter instance serving two runtime bindings. |
...
| `inq-baseline-profile-renderer` | Turn an installed local OpenAI-compatible runtime into explicit baseline-assistant operator configuration without adding a proxy. | `done` | `node/tools/inquirium-baseline-profile.py` renders the validated reference catalog for an unmanaged loopback Ollama endpoint or a managed local `llama-server`; it rejects non-loopback endpoints, verifies executable/model files, records the model SHA-256 digest in binding/candidate metadata, and deliberately leaves binary/model installation to the distributor or operator. It can atomically activate the reviewed profile with mode `0600` under a canonically contained Node data directory and report the active baseline runtime refs; daemon health and conformance remain runtime authority. Unit tests cover loopback denial, executable checks, digest binding, activation, invalid status, and symlink escape refusal. |
| `inq-feedback-training-grants` | Verify separate operator authority before feedback becomes adaptation material. | `done` | `inquirium-core` defines closed training-grant issue and revoke requests; daemon local control persists exact participant/feedback-bound expiring grants, archives participant-bound idempotent revocations, and rejects candidate-marked feedback unless the referenced grant exists, is current, not revoked, and matches both bindings. Future dataset-manifest builders and `train.adapt` admission must re-run the same authorization before consuming candidate data. |
...
| `inq-host-service-stratum` | Extract host-owned Inquirium orchestration into an `inquirium-host` service stratum while keeping daemon effects at the composition root. | `in-progress` | `inquirium-host` owns pure generate budget/prompt/adapter decisions, schema normalization, I/O rails, embed/classify/rerank admission and binding predicates, assistant output/transcript planning, reusable config including monotone assistant escalation policy, returned-value effect intents, operation decision wrappers, complete assembled-prompt token estimation, and `INQUIRIUM_HOST_API_VERSION`. Daemon still owns file/env loading, catalog iteration, supervision, transport, persistence, trace sinks, HTTP, effect execution, and grant authority. Dependency direction is guarded by core/host substrate lints plus a trace-consumer lint that prevents non-daemon assistant trace readers from importing Memarium runtime internals; future operation families should place pure decisions here before daemon wiring. |
...
| `inq-io-schema-contract` | Host-owned Output Schema Contract: broadcast a `schema/ref`, enforce it at the strongest available tier, validate, pre-parse, and bound repair (see *I/O Contract*). | `in-progress` | `inquirium-core` carries optional bounded output contracts and a narrow registered JSON Schema subset with canonical digests. `inquirium-host` resolves the strongest tier, compacts admitted schemas for provider use, normalizes output, enforces host byte/shape/schema checks, and returns typed invalid projections. OpenAI and Anthropic adapters compile the subset into native structured-output fields and normalize provider JSON, but native selection requires adapter support, selected runtime/model `supports_json_schema`, and per-schema `allow_native_enforcement`; host rails and semantic validation are never skipped. Dynamic refs/recursion/combinators, grammar caching, and bounded repair remain future layers. |
