...
| `inq-host-service-stratum` | Extract host-owned Inquirium orchestration into an `inquirium-host` service stratum while keeping daemon effects at the composition root. | `in-progress` | `inquirium-host` owns pure generate budget/prompt/adapter decisions, schema normalization, I/O rails, embed/classify/rerank admission and binding predicates, assistant output/transcript planning, reusable config, returned-value effect intents, operation decision wrappers, and `INQUIRIUM_HOST_API_VERSION`. Daemon still owns file/env loading, catalog iteration, supervision, transport, persistence, trace sinks, HTTP, effect execution, and grant authority. Dependency direction is guarded; future operation families should place pure decisions here before daemon wiring. |
