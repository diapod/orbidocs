...
| `inq-execution-provenance-host-derivation` | Derive realized provenance at the host boundary from selected runtime/adapter/model facts and the strongest available execution evidence. | `partial` | The real Agent-passage `generate` host path stamps completed dispatch plus exact runtime/model/model-snapshot bindings and explicitly uses unknown locality/egress when no admitted realized characteristic exists. Transport and runtime names do not establish locality. Pure observation constructors also cover pre-dispatch refusal, but integration of admitted characteristics and every denial/operation remains open. |
| `inq-execution-provenance-result-carriage` | Carry realized provenance through every Inquirium terminal result and result-bearing trace/effect intent. | `partial` | The Agent-passage `generate` response has a canonical V2 schema and cannot drop the exact result provenance sidecar. Direct generate, embed/batch, classify, rerank, summarize, transform, image, train-adapt, assistant outcomes, traces, and effect intents remain to migrate. |
| `inq-execution-provenance-persistence` | Preserve provenance through deferred completion, artifact publication, deterministic cache, replay, and redacted projections. | `partial` | The representative Agent-passage `generate` path writes V2 product/trace facts before V1 compatibility, repairs interrupted bundles, and replays after restart without reinvocation or duplicate charge. Direct Inquirium cache, artifact, deferred completion, effect-intent, and redacted projections remain open. |
| `inq-execution-provenance-acceptance` | Prove the complete local, non-local, mixed, unknown, redacted-provider, cache, artifact, and replay matrix. | `partial` | Unit and Schema Gate tests prove unknown direct execution, mixed parent composition, exact restart replay, and stripping/substitution refusal through a representative higher-layer Agent consumer. The retained runtime matrix — local no-egress, remote sidecar/HTTP, provider-hidden, ambiguous, pre-dispatch, cache, artifact, deferred, and complete no-drop inventory — remains open. |
