...
the requested `schema/ref` and stays within host JSON shape limits. It does not
yet resolve schema documents, compile grammars, or perform repair attempts.
...
| `inq-io-schema-contract` | Host-owned Output Schema Contract: broadcast a `schema/ref`, enforce it at the strongest available tier, validate, pre-parse, and bound repair (see *I/O Contract*). | `in-progress` | `node/inquirium-core` now carries optional `output_contract` on `inquirium.generate.request.v1` with `schema/ref`, requested enforcement (`auto`/`native`/`primed`/`validated`), bounded JSON byte/depth/array/object limits, and a repair-policy shape that is restricted to `max_attempts = 0` until repair is implemented. `node/daemon` forwards the contract to adapters, accepts only structured output with the matching `schema/ref`, bounds the returned JSON value before downstream use without materializing an oversized serialized buffer, and returns `GenerateOutcome::Invalid` with JSON Pointer-like violations when the adapter returns missing, mismatched, or oversized structured output. This foundation slice deliberately does not yet include a schema registry, semantic JSON Schema validation, grammar artifact caching, or an automatic repair loop; trace records redacted projection metadata, not structured values. |
