...
4. Admit either a distributor-signed manifest or a host-computed manifest with a
   ...
   not author the question shape or approval scope. A federation-endorsed
   distributor key remains scoped to the exact `provider/ref` retained by the
   host-owned release catalog; only an explicitly manual trust root is
   host-wide.
...
by the hard-MVP readiness calculation. The 2026-09-06 code and regression audit
confirmed closure of the previously reopened remote-provider adapter and
embedding claims (`P089-013`) and raw-file data-plane eligibility (`P089-012`).
Their three rows below are now `done`; provider integration evidence uses real
managed adapters with controlled fake providers, not live external services.
This closes existing-slice correctness gaps, not additive Proposal 090 work.
Post-MVP local-model package productization
...
| :--- | :--- | :--- | :--- |
| `inq-execution-posture-contract` | Bind runtime/profile admission and preflight disclosure to canonical `inference-execution-posture.v1`. | `done` | `inquirium-host` derives conservative posture and exposes exact runtime/profile/invocation scope through `posture_exposure`; configuration and registry fingerprints, generation and half-open validity are checked independently. P090-003a provides authenticated operator HTTP inspection; P090-004a rechecks optional invocation policy before model I/O. Core tests cover changed runtime/model/registry and mismatched generation, `process_provider_registry_and_scoped_posture_cross_real_http` proves missing/matched/wrong-generation exposure through a running daemon, and `generate_invocation_preflight_refuses_missing_disallowed_and_stale_posture` covers fail-closed invocation. Requirements reject observation-only `unknown`/`derived` bases. The declaration grants no inference, egress, context, file lease or effect authority; broader disclosure UX and producer coverage remain separate tasks. |
| `inq-execution-provenance-contract` | Bind Inquirium to the canonical provider-neutral `inference-execution-provenance.v1` value and pure composition rules. | `done` | `inquirium-core` consumes the canonical contract and owns `inquirium.result-provenance.v1`, an immutable sidecar bound to exact canonical response bytes. Inline/ref validation, explicit unknown and pre-dispatch forms, monotone join semantics, Schema Gate stripping refusal, and separation from `classification.v1` and provider-native session data are tested. Operation coverage is tracked separately below. |
| `inq-execution-provenance-host-derivation` | Derive realized provenance at the host boundary from selected runtime/adapter/model facts and the strongest available execution evidence. | `partial` | Daemon configuration admits explicit provider-neutral characteristics per active runtime, validates their evidence and registered provider refs at startup, and supplies exact runtime/model/model-snapshot bindings to the result descriptor. Missing characteristics remain `unknown`; transport names, endpoint placement, and catalog locality never establish realized execution. Successful direct operations and typed pre-I/O `generate` refusal use the shared constructor. P090-004a binds failed/ambiguous generate/embed outcomes without inventing successful runtime characteristics; P090-004b additionally retains classify/rerank/image HTTP terminal evidence and snapshots image execution before publication. P090-004b3 now binds training success/HTTP failure and bounded batch execution under the supervisor lock. P090-004c covers the currently admitted ten-operation refusal matrix, explicit resource-profile/lease/grant checks and HTTP response/ambiguous-dispatch evidence. Generalized producer profiles remain separate; the common producer gate does not promote the full host-derivation inventory. |
| `inq-execution-provenance-result-carriage` | Carry realized provenance through every Inquirium terminal result and result-bearing trace/effect intent. | `partial` | The Agent-passage `generate` response has a canonical V2 schema and cannot drop the exact result provenance sidecar. Direct successful `generate`, `embed`, `classify`, `rerank`, `summarize`, `transform`, image-generate, and image-edit responses commit exact sidecars in their metadata-only traces before success is returned. P090-004a carries bound generate/embed results and sanitized terminal failures, adds the shared inquirium.response.v2 envelope and parent-preserving summarize/transform translation. P090-004b exposes explicit V2 selection for eight synchronous operations without silent downgrade and carries image execution ancestry through publication or refusal. P090-004b3/004b4 add exact batch/training source commits, artifact-bound sidecars and durable image recovery. Unbound generic artifact effects refuse; broader operation terminals remain open. P090-007a/007b implement the scoped Assistant response/trace/transcript path, including finalized terminal publication and fenced missing completion. H-1–H-14 review keeps observed image terminals in typed/V2 errors if their commit fails; restart still refuses uncommitted reinference and does not claim missing bytes durable. P090-004c retains exact sanitized terminal/provenance pairs in operation traces and preserves the original observation across all eight synchronous trace sinks, image byte/commit faults and deferred result-object refusal. General result-bearing effect carriage remains outside this scoped checkpoint. |
| `inq-execution-provenance-persistence` | Preserve provenance through deferred completion, artifact publication, deterministic cache, replay, and redacted projections. | `partial` | The representative Agent-passage `generate` path writes V2 product/trace facts before V1 compatibility, repairs interrupted bundles, and replays after restart without reinvocation or duplicate charge. Direct-operation success traces are durable and fail closed if the sidecar cannot be committed. P090-004a also verifies current embedding cache L1/SQLite restart with original evidence, zero current usage, and corrupt/stripped/substituted entry refusal. P090-004b3/004b4 add the owned batch/training/image journals, exact artifact publication, source-image ancestry and restart without adapter I/O. The 24-case worker matrix separates pending/running/publishing projections from source authority; a failed artifact projection can retry its committed sidecar. Owner-scoped keyset recovery drains past refused pages without touching other components. P090-004c also retains a separate owner-journal failure fact when a deferred result-object commit fails; BDO diagnostic digests never stand in for observation bytes. Corrupt image replay preserves its known parent without invoking the adapter. General redacted projections and external descriptor resolution remain open. P090-004b2/004b2a and P090-007b add the ordinary text-controller intent/commit matrix and Assistant publication-sink/restart matrix without redispatch or duplicate accounting. |
| `inq-execution-provenance-acceptance` | Prove the complete local, non-local, mixed, unknown, redacted-provider, cache, artifact, and replay matrix. | `partial` | Unit and Schema Gate tests prove exact response binding, transport-independent unknown, explicit admitted local evidence, pre-dispatch non-execution, mixed parent composition, exact restart replay, and stripping/substitution refusal through a representative higher-layer Agent consumer. A real daemon capability regression verifies that a catalog-local deterministic runtime still yields realized `unknown` without an admitted characteristic. P090-004a adds executable operation-specific terminal, HTTP translation, cache/restart and four-passage Agent tests with non-local/withheld fixture evidence and no duplicate charge. The durable-producer inventory set now exercises real adapter dispatch in local deterministic HTTP/stub fixtures, committed-source restart and artifact sink/refusal matrices. This is not physical-model acceptance or the complete no-drop inventory. P090-008c adds deterministic local deployment evidence with real supervised modules and WSS/AD; P090-012a owns aggregate promotion, not physical or complete-producer acceptance. P090-004c owns the separate sixty-cell admitted-producer matrix and requires every fully qualified named proof in the same bounded run; missing, duplicate, truncated or wrong-owner evidence refuses promotion. New profiles and the complete cross-component inventory remain open. |
...
| `inq-python-remote-provider-adapters` | Add first middleware-hosted remote provider adapters while preserving adapter-instance/runtime-candidate stratification. | `done` | The OpenAI adapter maps neutral generation and host-authorized JSON Schema to Responses and neutral embedding to the native `/v1/embeddings` endpoint; the Anthropic adapter maps generation and the same schema subset to Messages and deliberately does not claim an embedding API. Both share `node/middleware-modules/lib/inquirium_adapter`, expose neutral and chat-compatible endpoints, read secrets from env/file config, and have fake-provider coverage. `model-runtime-http` starts each adapter as a managed process, exercises invoke and shutdown, and verifies one OpenAI instance serving two model bindings. `P089-013` closes the Python/private Rust DTO mismatch. `daemon_openai_embedding_path_accepts_provider_metadata_without_disclosing_it` exercises the real managed Python adapter against a fake `/v1/embeddings` provider, validates vectors and usage, and checks that provider/request metadata stays runtime-private. This is deterministic daemon integration evidence, not live external-provider acceptance. Host validation and rails remain authoritative after normalization. |
...
| `inq-embed-host-surface` | Expose direct embedding through the same host capability and audit model as text generation. | `done` | Daemon exposes `POST /v1/host/capabilities/inquirium.embed`, advertises it through host capability discovery when an implemented handler is routable, requires explicit inference grants for module callers, selects `embed` runtime candidates by host-owned model binding, and writes metadata-only traces under `trace/inquirium/embed` with host-keyed request digests and no input text or vector values. `node/inquirium-host` owns direct request validation, profile/classification/locality/trust eligibility, and model-binding admission; missing classification fails closed to Personal/local-only. Deterministic and lower HTTP/channel handlers plus vector revalidation are implemented. `P089-013` supplies the full managed OpenAI embedding daemon regression: success validates vectors/usage without exposing provider metadata, while provider loss retains typed failure provenance. Private DTO tests reject missing, malformed, or unexpected metadata. The provider is a controlled HTTP fake; Anthropic remains generation-only because its native API exposes no embedding operation. |
| `inq-direct-data-plane` | Add durable direct data-plane leases, artifact output persistence, and deferred long operations. | `done` | Daemon persists model-runtime leases in SQLite, exposes local lease create/read APIs, enforces canonical allowed-root containment and rejects symlinked roots/escapes, hides expired leases, restricts caller metadata with per-value and total caps, admits `batch.embed` through `DeferredOperationRegistry`, and verifies/writes pilot output artifacts through the object store with selected-binding and operation-bound write-lease provenance. `P089-012` replaces the transport-only heuristic with explicit, default-unknown `AdapterDataPlaneCharacteristics`. Host admission checks exact runtime/model binding, routability, host-local read-only access, both lease policies, strict-local policy, and managed-process no-egress enforcement; the registry independently checks caller/adapter/runtime roots and TTL. Unit and daemon endpoint regressions cover fail-closed admission, and batch-embed lifecycle tests cover deferred registration, verified artifacts, and lease/digest/conflict refusal. Pilot artifact output is now planned as an `ArtifactOutputIntent` and executed by the shared daemon artifact-output effect interpreter rather than as an inline endpoint write. P090-004b3/004b4 replace caller-output batch admission with bounded embedding execution and immutable producer/artifact commits. Lease/digest/context/conflict negatives now exercise the shared publication boundary directly; worker restart cannot authorize reinference. |
...
| `inq-agent-passage-profile-binding` | Bind every Agent inference passage to exact admitted P064 assembly and structured-output profiles without exposing prompt content in Agent state. | `done` | `agent.inference-flow-binding.v1` offers closed prompt-policy, output-schema, repair-profile, runtime, and visibility ref sets; every passage selects exact members before the local Inquirium call. The daemon requires the canonical caller request digest, selected runtime, host-owned 64-hex `prompt_assembly.instruction/hash`, and provider/model snapshot before committing a content-addressed product. Content-addressed contract digests remain base64url-no-pad and are not interchangeable with that hex instruction hash. Only the daemon-projected prompt-assembly metadata is accepted as instruction evidence. Token and provider-cost use is charged through the shared Inquirium usage normalizer. A host-local single-flight claim prevents concurrent local invocation for one passage, while provider-native idempotency remains required beyond the host. Durable Agent traces contain refs, digests, normalized usage, and closed decisions but no prompt, response body, private reasoning, or protected source content; a refused invocation records a durable trace and exact dirty-restart replay does not reinvoke or recharge the model. |
