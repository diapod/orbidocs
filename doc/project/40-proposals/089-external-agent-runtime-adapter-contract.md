...
claims are implemented and tested. The evidence does not yet claim
real-platform host isolation, routable active-turn cancellation, actuation, or
...
sidecar after a mid-passage Agent restart. Generic disclosure conformance and
higher-layer carriage remain open under Proposal 090.
...
1. Complete the partial `P089-011` projection with retained real-Codex evidence,
   disclosure conformance, and higher-layer no-drop gates. Preserve the
...
3. Implement `P089-008d` only through a bounded concurrency seam that makes the
