...
| `P085-003` | Freeze the P085 schema family and positive/negative fixtures | `partial` | In addition to the limit/resource-envelope and lifecycle families, accepted synchronized Schema Gate contracts now cover signed `capability-derived.v1`, `operator-guard-hook.v1`, `operator-attention-budget.v1`, `federated-envelope-declaration.v1`, and `node-extension-posture.v1` artifacts. The five families are registered in the complete-family Schema Gate drift guard. Operator-binding references accept the canonical generated `node-operator-binding:<time>:<suffix>` grammar rather than only a one-segment placeholder. The NSE offer/decision schemas also close the eleven Inquirium, Corpus, and Agent policy-hook identifiers and reject hook-class mismatch and arbitrary broadcast rewrite. Positive and negative fixtures are synchronized into both contract trees. Phase 2 semantic-registry and dispatch-classification contracts remain tracked separately. |
...
| `P085-012` | Implement operator attention budgets, grouping, and availability windows | `partial` | Signed budgets now cross Schema Gate, verify operator authority, persist and recover, require at least one explicit availability window, resolve real IANA time-zone windows, and drive the daemon's operator-question notification boundary. An empty window list is rejected instead of becoming an implicit permanent blackout. Rolling prompt/group caps, repeat suppression, overflow defer/deny, and a separate never-approving security lane use a bounded 4,096-event durable ledger with budget-scoped cleanup. Questions persist before notification projection, so suppression cannot become approval or erase required HIL. Current declaration metadata is visible in inspection and expired declarations are omitted. Request-expiry clamping, one aggregated group notification, and richer bucket-usage projection remain open. |
...
| `P085-016` | Add operator CLI/UI for effective policy and authority inspection | `partial` | `operator-extensionctl inspect <store-root>` remains a canonical owner-store, permission-checked, audited offline read. Its live mode mirrors the authenticated loopback lifecycle routes with bounded bodies and prompt-free refusal diagnostics. The accepted inspection projection now also lists at most 256 current signed derived-capability, guard, and attention declarations by kind/ref/revision/digest/operator/expiry without exposing payload or signature bytes; expired history is omitted and admission refuses before current projection capacity can be exceeded. Comparison views, attention usage, effective guard results, posture evidence, richer explanations, and graphical UI remain open. |
...
| `P085-018` | Add local and three-node acceptance plus trace-explorer evidence | `partial` | Existing lifecycle and refusal acceptance remains in place. New unit/golden coverage proves the five signed authority contracts and negative fixtures, sequential durable recovery and conflict refusal, bounded attention grouping across restart, per-budget cleanup, guard caps, inspection capacity/expiry, all eleven generic hooks, byte bounds, broadcast transform-profile containment, task/session narrowing, federated intersection, and refusal of fan-out/risk widening and arbitrary rewrite. A real daemon-process smoke admits a correctly signed local declaration, observes it through inspection, and refuses a foreign operator binding, an invalid signature, and a schema outside the local policy-admission family. Modified-posture acceptance, trace-explorer integration, and three-node federation remain open. |
...
| `P085-023` | Implement bounded runtime caches and fact-driven invalidation | `partial` | The lifecycle service owns a 64-entry generation-bound compiled-content cache and the daemon owns a 256-entry ephemeral NSE offer registry with bounded TTL/reuse and restart invalidation. The attention ledger is capped at 4,096 durable events and cleans only the evaluated budget's expired window; the signed policy projection refuses a 257th current declaration while retaining expired history outside the active view. Effective-envelope caching remains generation-bound in Inquirium. Staging, trust/sanction/restriction invalidation, aggregate-notification lifecycle, and complete TTL/index diagnostics remain open. |
