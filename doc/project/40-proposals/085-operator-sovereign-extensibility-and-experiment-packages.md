...
| `P085-003` | Freeze the P085 schema family and positive/negative fixtures | `partial` | In addition to the limit/resource-envelope family, accepted synchronized Schema Gate contracts now cover NSE offer/decision/table/middleware evidence, the daemon's prompt-free offer-resolution trace, the exact refusal vocabulary and projection, experiment package, durable/session activation, transition, revocation, conformance report, bounded operator inspection, loose signed import and receipt, and host-owned conformance-run request/result. Durable activation and revocation signatures share the closed `ed25519+domain-v1` shape. Positive/negative fixtures cover the new import and conformance contracts as well as changed offer digest, unknown table/evidence/refusal values, prompt content in a resolution trace, production session activation, package-owned effective priority, required-producer fallback, and semantically inconsistent inspection state. `capability-derived.v1`, `operator-guard-hook.v1`, `operator-attention-budget.v1`, federation, posture, and Phase 2 enum/dispatch contracts remain open. |
...
| `P085-016` | Add operator CLI/UI for effective policy and authority inspection | `partial` | `operator-extensionctl inspect <store-root>` remains a canonical owner-store, permission-checked, audited offline read. Its live mode accepts only literal loopback endpoints and mirrors daemon `status`, `import`, `run-conformance`, `activate`, `rollback`, and `revoke` routes with bounded request/response bodies and prompt-free refusal-code diagnostics. The accepted `operator-extension-inspection.v1` projection remains the canonical lifecycle output. Comparison, authority/attention/guard/posture views, richer bounded refusal explanations, and a graphical UI remain open. |
...
| `P085-018` | Add local and three-node acceptance plus trace-explorer evidence | `partial` | Unit/golden tests cover table/Rhai shared validation, supervised evidence containment and exact dispatch binding, host-owned offer expiry/restart/replay/caller/digest/producer/budget refusals, opaque Flow decision projection, prompt-free resolution trace, Flow branching, selecting disagreement, required/advisory failure, pre-execution producer-budget reservation, interrupted recovery, signatures, WAL durability, atomic revocation/audit rollback, pinned-runner replacement and unsafe-permission refusal, corpus/runtime substitution, revocation, invalidation, session restart discard, derived grants, guards, attention, and safe mode. Local process tests cover audited offline inspection, the reference evidence process, and the actual daemon HTTP lifecycle: signed allowlisted import remains inert, a host-selected bounded runner records conformance, signed activation and rollback advance generations, restart recovers generation two, status remains prompt-free, terminal revocation persists, and a later rollback refuses as `policy/revoked`. The common 28-code matrix has distinct reaching fixtures. Modified posture, federation, trace-explorer integration, and three-node acceptance remain open. |
| `P085-019` | Synchronize Capability Registry, Node ledger, solutions, FAQ/HOWTO, and readiness classification | `partial` | The Node ledger, crate READMEs, P085 tracker, generated schema documentation, and MVP readiness snapshot describe the implemented offer registry, decision resolver, supervised evidence process, signed loose import, daemon-owned lifecycle routes, conformance runner, CLI, exact refusal reachability, process smoke, and remaining federation boundaries. Formal daemon capability ids, cross-solution HOWTO/FAQ changes, and federation/posture documentation wait for their contracts. P085 remains post-MVP. |
...
| `P085-023` | Implement bounded runtime caches and fact-driven invalidation | `partial` | The lifecycle service owns a 64-entry compiled-content cache whose executable binding includes package ref and active generation. Conformance replacement, activation commit, revocation, package deactivation, and safe-mode entry invalidate synchronously; safe mode independently blocks insertion and lookup; restart restores no compiled entries. Effective-envelope caching remains independently generation-bound in Inquirium. The daemon additionally owns a 256-entry ephemeral NSE offer registry with bounded TTL/reuse and unconditional restart invalidation. Attention-window, staging, trust/sanction/restriction invalidation, and complete TTL/index diagnostics remain open. |
