...
node-signed evidence. Local trust policy decides whether self-declared posture is
sufficient or stronger attestation is required.
...
| `P085-016` | Add operator CLI/UI for effective policy and authority inspection | `partial` | `operator-extensionctl inspect <store-root>` remains a canonical owner-store, permission-checked, audited offline read, while live mode mirrors authenticated loopback lifecycle routes with bounded bodies and prompt-free refusals. `posture` prints the current node-signed publication and `peer-posture` reads a dedicated narrow route that returns only the persisted local/peer policy comparison from one admitted Corpus invitation, not the complete invitation inbox record. The bounded projection includes current declarations, rolling attention usage, package/lifecycle state, decisive restrictions, provenance, effective capability ids, policy-hook outcomes, cache/store occupancy, nearest expiry, and index health without payload, signature, prompt, answer, grouping-key, or model-output bytes. The accepted peer comparison exposes posture and declaration refs/digests, peer and local effective profile digests, and effective entry count; Trace Explorer projects the same admitted result as metadata only. Richer explanations and graphical UI remain open, so this item remains `partial` rather than overstating CLI completion as UI completion. |
...
3. Complete the graphical and richer refusal-explanation views in the operator
   inspection surface. CLI posture publication, persisted peer comparison, and
   prompt-free Trace Explorer evidence are implemented regression obligations.
4. Begin Phase 3 with its fixed-vocabulary vertical only after the production Flow
