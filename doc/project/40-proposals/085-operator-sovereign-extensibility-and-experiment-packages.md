...
- reusable inference-Flow registrations with exact prompt-policy, output-schema,
...
| `P085-OQ9` | Implement WASM only after table policy, shared validators, and refusal-corpus evidence; P087 now owns that future implementation contract and tracker. | Portable untrusted policy does not enlarge the first implementation slice or P085 closure. |
...
| `P085-016` | Add operator CLI/UI for effective policy and authority inspection | `done` | `operator-extensionctl inspect <store-root> [inspection-context-json]` remains a canonical owner-store, permission-checked, audited offline read. `inspect`, `explain`, and `graph` now consume one accepted `operator-effective-policy-inspection.v1` projection built from the audited lifecycle facts plus an optional validated owner-composed context. The context must repeat the exact audited extension view and must account for every Inquirium, five-domain registry, and federation source as populated or explicitly unavailable. Only the extension section is rechecked against this store; the host composes the remaining facts from their owning strata, and the projection digest identifies rather than independently certifies them. The initial text and Mermaid views expose at most twelve material/domain/federation details plus an omitted count; the JSON view retains bounded stable drill-down refs. Process acceptance composes all 38 Inquirium axes, Agent/Corpus/Arca/Dator/Inquirium registries, overlapping requested/effective values, federation evidence, and one decisive refusal; all three views agree on that refusal and omit prompt, secret, signature, and model-output content. Schema Gate carries positive and negative fixtures and semantic validation for both composition input and projection. `posture`, narrow `peer-posture`, and Trace Explorer retain their existing prompt-free local/peer comparison surfaces. Completed `P085-042` adds host-composed interactive attention plus bounded peer-mismatch and refusal-provenance navigation without changing the core inspection contract. |
| `P085-017` | Implement WASM NSE backend | `deferred` | Extracted into P087 as implementation-ready future work. P087 preserves the P085 authority boundary, keeps package V1 unchanged, specifies an import-free no-WASI ABI, deterministic fuel plus independent wall-time and resource fences, package lifecycle and current-use binding, refusal/inspection evidence, and a phased tracker. It remains non-blocking for P085 and hard-MVP. |
...
stable. The deferred WASM backend in `P085-017`, now specified by P087, is not a
...
`max/passages` is a ceiling of one exact inference-flow binding. The first daemon
slice admits one such binding per Agent, but the accounting remains binding-local so
that a future multi-binding Agent does not silently turn it into a global Agent
ceiling. Token and provider-cost usage are normalized through the shared Inquirium
usage projection before charging the Agent budget.

Invocation is single-flight only inside the host for one `passage/ref`; it is not a
global exactly-once claim. Provider-native idempotency remains required beyond the
host boundary. A refused invocation records a durable prompt-free refusal trace and
releases the ephemeral claim, permitting exact retry of the same passage without
allocating another passage number. A newly published CAS object is committed with
the product fact or compensated on commit refusal; cleanup failure is explicit.

The family intentionally uses full `classification.v1` values for concrete passage
facts and a tier-only `classification/ceiling` for binding policy. Likewise,
content-addressed contract digests use `sha256:<base64url-no-pad>`, while the P064
`instruction/hash` remains its established lowercase 64-hex SHA-256 representation.
These values are not interchangeable and must not share one permissive validator.
...
`P085-031`, and `P085-032`. The deferred WASM backend specified by P087 is not a
prerequisite.
...
The V1 Agent runtime admits exactly one current inference Flow binding per Agent.
Consequently, `AgentStatusResponse` exposes one singular `inference-flow` projection;
the package-level bound of 32 `inference-flows` is a catalog bound across reusable
registrations, not permission to attach several current bindings to one Agent.
...
eleven-check validator on `macos-vz-arm64.v1`. The report binds the image artifact
...
| `P085-044` | Make Corpus turn order operator-configurable and Flow-consumable | `done` | **Dependencies:** P085 `P085-005`, `P085-007`, `P085-009`, `P085-014`, `P085-039`, and `P085-040`; P069 `P069-TURN-001` and completed `P069-TURN-002`; P049 `P049-008`. The accepted `nse-select-turn-order-table.v1` contract is Schema-Gated and compiles one closed role-order table into the common `order` proposal. Ordinary package activation, restart recovery, terminal revocation, producer lookup, opaque Flow consumption, single-use authority, centrally registered host-local `agent.turn-order.resolve` dispatch, and durable Corpus decision replay are implemented. Unit and process evidence cover reference and alternate policy order, exact candidate containment, target-specific ordinary HIL and floor admission, exact duplicate resolution, changed-body and stale-authority conflict, node-A restart with durable replay, producer revocation without semantic fallback, and absence of floor authority in Flow output or scenario helpers. A separate no-producer three-node run records the exact distribution-policy source and completes floor assign/release through the same Chair and Room admission path. |
| `P085-045` | Resolve asset-pinned semantic entries for domain consumers | `done` | 2026-09-26, for P094 task profiles (`P094-005a`): the package stores no asset bytes, so `OperatorExtensionService::package_entry_activation` resolves a semantic entry whose payload the caller supplies. The registration must carry exactly the caller's digest for the exact domain, ref and revision; the answer is `not-installed`, `not-bound`, `inactive` (no current activation, expiry, revocation, a failed dependency or safe mode) or the single current activation with its generation, operator binding and operational class, and more than one current activation refuses as ambiguous. The domain owner defines the digest rule, and callers never cache the answer; unlike the code-backed projection, current capability grants and the current operator binding are left to the consumer, and a malformed clock is the caller's error, not an inactive package. Owner tests cover the exact tuple, restart, rollback to a new generation and revocation. |
