...
| `operator-extension-inspection.v1` | Bounded prompt-free local read model of safe mode, package identity, active producer composition, lifecycle state, conformance, session activation, and revocation. |
...
| `P085-003` | Freeze the P085 schema family and positive/negative fixtures | `partial` | In addition to the limit/resource-envelope family, accepted synchronized Schema Gate contracts now cover NSE offer/decision/table/middleware evidence, the exact refusal vocabulary and projection, experiment package, durable/session activation, transition, revocation, conformance report, and the bounded operator inspection read model. Durable activation and revocation signatures now share the closed `ed25519+domain-v1` shape and negative fixtures reject malformed or absent signatures. Other negatives cover changed offer digest, unknown table/evidence/refusal values, production session activation, package-owned effective priority, required-producer fallback, and semantically inconsistent inspection state. `capability-derived.v1`, `operator-guard-hook.v1`, `operator-attention-budget.v1`, federation, posture, and Phase 2 enum/dispatch contracts remain open. |
| `P085-004` | Refactor `nse` into versioned hook contracts with shared offer-bound validators | `partial` | `select-llm-model` now uses a host-built digest-bound offer, raw backend proposal, opaque admitted decision, typed backend failures, hook class, and one shared validator used by table and Rhai. Hook payload validation precedes offer digest assignment. The common selecting compositor validates the full activation set and reserves the whole producer budget before invoking any producer callback, verifies activation-owned producer identity/digest, requires exact agreement, and refuses hidden fallback. Tests prove zero producer calls when aggregate reservation fails. Completion still requires the remaining hook validators and migration of broadcast rewrite outcomes to offered transform-profile selection. |
...
| `P085-009` | Add JSON-e Flow `decision` step | `partial` | The executor has a typed `decision` step, an allowlist of hook ids, opaque `offer/ref` resolution through its host invoker, admitted/refused branching, bounded output validation, traces, and refusal-first tests. The production daemon still needs the host-owned ephemeral offer registry/resolver; the default invoker fails closed as `decision-invoker-unavailable`. |
| `P085-010` | Add bounded supervised middleware evidence input | `partial` | `nse-middleware-evidence.v1` closes annotations, scores, features, and candidate suggestions and rejects candidates outside the host set. A host-owned admission binding rechecks module, package digest, grants, hook/version, schemas, timeout, byte caps, causal ref, invocation, and offer fields exactly. Wiring this contract to a real supervised middleware dispatch and projecting admitted evidence into the daemon-built offer remain open; no Python backend can construct an admitted NSE decision. |
| `P085-011` | Implement derived capability-set overlay and invalidation | `partial` | The pure authority stratum recomputes every derived set from current base-grant snapshots on each use and fails on missing, inactive, quarantined, expired, duplicate, or wrong-operator components while intersecting restrictions. Persistent signed `capability-derived.v1` facts and daemon/P072 overlay wiring remain open; no route, passport, or advertisement is created by this implementation. |
| `P085-012` | Implement operator attention budgets, grouping, and availability windows | `partial` | A pure bounded ledger validates real IANA time-zone ids, enforces rolling prompt and group caps, minimum repeat interval, defer/deny outside-window policy, and a separately visible but never approving security lane. Durable buckets, configured availability-window resolution, restart rebuild, indexed cleanup, expiry, and operator projection remain open. |
| `P085-013` | Implement and register the non-delegable core | `partial` | A checked fresh `local-control` proof rejects Agent, peer, middleware, Flow, remembered consent, wrong binding, stale time, and future time. The lifecycle service persists extension safe mode across restart, blocks compiled executable bindings, and exposes forced session/package/all deactivation plus restore-defaults/rebuild actions without invoking NSE, package code, or middleware. Daemon capability ids/routes and append-only safe-mode transition facts remain open. |
| `P085-014` | Implement experiment package install/activate/revoke/rollback and composition lifecycle | `partial` | A new host-owned SQLite service implements inert install, exact passing-conformance binding, self-verifying domain-separated Ed25519 activation and revocation signatures, activation plan digest and current operator gates, `planned -> committed -> finalized` recovery, one authoritative generation, rollback only after a finalized generation, terminal revocation, synchronous cache invalidation, and exact idempotent replay with conflicting package/transition/revocation content refused. WAL, `synchronous=FULL`, bounded busy wait, schema versioning, and immediate write transactions serialize commit, recovery, and safe-mode changes across store connections. Effective priority, required/advisory mode, and advisory fallback live only in the local activation and must bind an exact package producer. NSE composition rejects digest substitution, disagreement, required failure, and unavailable aggregate budget. Current package trust/container import, daemon routes, compatibility/grant/context checks, and full rollback acceptance remain open. |
| `P085-015` | Require and execute package refusal corpora | `partial` | The package binds a refusal-corpus ref/digest and activation requires a current fully passing content-addressed conformance report with bounded positive/refusal totals. Schema and service tests reject stale package/report bindings. A real runner that executes the common 28-code matrix plus package-specific fixtures against the installed runtime remains open. |
| `P085-016` | Add operator CLI/UI for effective policy and authority inspection | `partial` | `operator-extensionctl inspect <store-root>` resolves only the canonical owner store, refuses unsafe Unix ownership or group/world permissions, appends a metadata-only audit fact, and exposes the accepted, Schema-Gated `operator-extension-inspection.v1` prompt-free projection of persistent safe mode, package/conformance identity, active generation, activation ref, effective producer composition, session activation, latest journal state, and revocation ref. Inspection does not perform daemon startup recovery. Mutation deliberately remains behind host-owned live-operator and signature APIs. Comparison, authority/attention/guard/posture views, daemon CLI/UI actions, and bounded refusal explanations remain open. |
...
| `P085-018` | Add local and three-node acceptance plus trace-explorer evidence | `partial` | Unit/golden tests now cover table/Rhai shared validation, evidence containment and exact dispatch binding, Flow decision/refusal branching, selection disagreement, required/advisory failure, producer-budget reservation, interrupted activation recovery, typed signature shape and domain/key substitution, WAL durability settings, terminal revocation, cache invalidation on conformance/revoke/safe mode, audited session restart discard, derived-grant invalidation, guard caps, attention exhaustion, and live-operator-only safe mode. A process test performs owner startup recovery and then proves canonical-root, permission-checked, audited CLI inspection; arbitrary file paths and unsafe modes refuse. Real supervised Python evidence, full local daemon causality, rollback, exact 28-code reaching coverage, modified posture, federation, and three-node acceptance remain open. |
| `P085-019` | Synchronize Capability Registry, Node ledger, solutions, FAQ/HOWTO, and readiness classification | `partial` | The Node ledger, crate READMEs, P085 tracker, generated schema documentation, and MVP readiness snapshot describe the implemented foundation and its remaining boundaries. Formal daemon capabilities, cross-solution HOWTO/FAQ changes, and federation/posture documentation wait for their routes and contracts. P085 remains post-MVP. |
| `P085-020` | Implement operator-declared guard hooks | `partial` | The pure authority layer closes anchors, axes, and operations to host-owned `restrict|narrow|raise-risk`, checks registered anchors, enforces per-anchor count and serialized-byte caps without truncation, rejects duplicate refs, and returns deterministic priority/ref ordering. Signed `operator-guard-hook.v1` facts, producer execution/budget reservation, daemon admission-anchor wiring, and synchronous invalidation remain open. |
...
| `P085-023` | Implement bounded runtime caches and fact-driven invalidation | `partial` | The lifecycle service owns a 64-entry compiled-content cache whose executable binding includes package ref and active generation. Conformance replacement, activation commit, revocation, package deactivation, and safe-mode entry invalidate synchronously; safe mode independently blocks insertion and lookup; restart restores no compiled entries. Effective-envelope caching remains independently generation-bound in Inquirium. Per-invocation, attention-window, staging, trust/sanction/restriction invalidation, and complete TTL/index diagnostics remain open. |
| `P085-024` | Implement expiring local session activation | `partial` | Session activation is unsigned, exact-plan/package/conformance bound, `research|experimental` only, capped at one hour, tied to the current operator binding, local in memory, removed by revocation/safe-mode recovery, and unconditionally discarded on owner restart. Metadata-only audit records start, explicit terminal removal, revocation, and restart discard without restoring session authority. Time-driven expiry sweeps, daemon routes, and federation refusal evidence remain open. |
| `P085-025` | Implement the closed refusal registry and diagnostic projection | `partial` | A closed 28-code Rust vocabulary has an exact JSON Schema enum and bounded prompt-free projection carrying producer/package/hook/anchor/field/invocation/offer/causal/declaration/omission metadata. Unknown codes and duplicate or oversized omission projections fail. NSE validators and composition emit only registered codes. This closes vocabulary parity, not runtime reachability: the CI gate proving that every code has a distinct reaching fixture and that no emitted code is dead remains open. |
...
12. After the production Flow decision resolver and applicable package lifecycle are
    stable, begin Phase 3 with its fixed-vocabulary vertical. Add code-backed role,
    sink, operation, or policy semantics only after their Phase 2 registries exist;
    never model private chain of thought as an Orbiplex contract.
