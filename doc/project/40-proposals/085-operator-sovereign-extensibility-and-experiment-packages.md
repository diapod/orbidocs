...
signed envelope.
The runtime sources task/session tightening, performs bounded signed-envelope widening
through the distribution safety-range map, and can compute a pure stricter federated
intersection. Network binding of the federated declaration remains separate work.
...
| `P085-006` | Add Inquirium resource envelopes and initial NSE hook expansion | `partial` | `inquirium-core` owns the 38-axis closed profile, complete distribution defaults, pure source-chain resolution, per-axis provenance, canonical digest, and distributor-owned per-axis safety ranges. The daemon validates every configured range against its distribution value during startup, persists, recovers, and rechecks signed envelopes, and shares immutable effective profiles. A signed envelope may widen an axis only inside its configured distribution range; an axis without a range refuses widening, while unsigned local configuration and task/session overlays remain tighten-only. Equal-valued local configuration does not claim false narrowing provenance, so an explicitly ranged envelope may still widen the unchanged distribution value; an actual local or task narrowing always wins. A pure federated intersection chooses the stricter cap or higher minimum and records both sources. Classify, rerank, embed, and batch-embed now validate requests and responses directly against the resolved profile, so an explicitly admitted widening reaches the owning runtime boundary instead of being re-rejected by compiled defaults. The daemon applies `score-candidate`, `select-output-schema`, `select-repair-profile`, and `assemble-prompt` at the owning generate boundary; the same prompt-assembly path is reused by summarize, transform, and Assistant Channel generation. Static producers still pass the ordinary package activation and offer-bound NSE admission path. Network binding of federated declarations and enforcement of the remaining vocabulary-only axes remain open. |
| `P085-007` | Add Corpus decision hooks and federated envelope binding | `partial` | The shared offer-bound validator covers `select-turn-order`, `weigh-bid`, `resolve-tie`, and `admit-participant`, and rejects removal of host-required roots or any widening beyond offered sets and bounds. Corpus now invokes them over current eligible participants, all admitted priced bids, the host-derived exact-price tied set, and the exact accepted role assignment respectively; ordinary Room, bid-state, settlement, membership, sanction, and role validators remain authoritative after the hook. At floor dispatch the order hook validates that the already HIL-approved, digest-bound target remains the effective head; it may refuse but cannot substitute another participant after approval. Signed invitation binding to an exact federated declaration digest and multi-node runtime acceptance remain open. |
...
| `P085-016` | Add operator CLI/UI for effective policy and authority inspection | `partial` | `operator-extensionctl inspect <store-root>` remains a canonical owner-store, permission-checked, audited offline read. Its live mode mirrors the authenticated loopback lifecycle routes with bounded bodies and prompt-free refusal diagnostics. The accepted inspection projection lists at most 256 current signed derived-capability, guard, and attention declarations, bounded rolling attention usage and active aggregate groups, plus the latest 256 content-addressed authority decisions whose refs are reverified against canonical payloads. Guard decisions expose artifact refs/digests, effective capability ids, fully resolved guard types, the decisive restriction, outcome, and typed refusal; policy-hook decisions expose the exact hook, effective decision class, causal subject, package/producer provenance, operator binding, and refusal without payload bytes. Runtime diagnostics expose bounded cache/store occupancy, inert versus active package counts, active-policy count, nearest known expiry, configured ceilings, and required-index health. A reached guarded boundary without active policy records an inert `policy-absent` decision without an operator binding or policy artifacts. Payload, signature, prompt, answer, grouping-key, and model-output bytes remain absent. Expired current policy and groups are omitted while bounded decision history survives restart. Comparison views, posture evidence, richer explanations, and graphical UI remain open. |
...
| `P085-018` | Add local and three-node acceptance plus trace-explorer evidence | `partial` | Existing lifecycle and refusal acceptance remains in place. Unit/golden coverage proves the five signed authority contracts and negative fixtures, sequential durable recovery and conflict refusal, bounded attention grouping across restart, quiet-window denial, security aggregation, DST and effective-deadline handling, live global-capacity refusal, stale-event reclamation under capacity pressure, more-than-one-batch restart cleanup, stale-group reconciliation, per-budget cleanup, guard caps and empty intersections, inspection capacity/expiry, all eleven generic hooks, byte bounds, broadcast transform-profile containment, task/session narrowing, federated intersection, all eleven owning local call sites, exact source-domain grant snapshots, and refusal of ambiguous grants, stale facts, fan-out/risk widening, and arbitrary rewrite. The daemon-process smoke admits signed local policy, activates one exact package-bound producer, proves both ordinary capability-use and Agent effect-admission guard paths, verifies their durable prompt-free decisions, evaluates the exact active-Flow derived capability before and after restart, and proves package revocation synchronously changes the guarded route to a typed refusal. It also covers attention timeout clamping and restart recovery plus foreign-binding, invalid-signature, and unsupported-family refusal. Modified-posture acceptance, trace-explorer integration, and three-node federation remain open. |
| `P085-019` | Synchronize Capability Registry, Node ledger, solutions, FAQ/HOWTO, and readiness classification | `partial` | The Node ledger, crate README, P085 tracker, generated schema documentation, and MVP readiness snapshot describe the signed policy store, attention gate, capability-use and Agent-effect guard anchors, prompt-free guard/derived-capability/policy-hook decisions, bounded runtime diagnostics, source-domain derived-capability adapters, package compatibility checks, distributor-bounded resource widening, all eleven local owning hook call sites, broadcast profile selection, and task/session and federated profile mechanics while retaining the post-MVP classification. Capability Registry overlay semantics beyond primitive eligibility, cross-solution HOWTO/FAQ material, and completed federation/posture guidance remain open. |
| `P085-020` | Implement operator-declared guard hooks | `partial` | The pure authority layer closes anchors, axes, and operations to host-owned `restrict|narrow|raise-risk`, checks registered anchors, and enforces per-anchor count and byte caps. Signed declarations cross Schema Gate, verify exact operator authority, persist as sequential revisions, and recover after restart. The daemon applies `restrict/grant-set` tables both to the shared `inquirium.generate` capability-use path and to Agent effect-proposal admission before any effect fact is committed. Every table is daemon-configured, content-addressed, bound to an exact active package producer, intersected once under one current-operator/service snapshot, capped at 32 capability ids, revalidated against current activation/operator/package/conformance/revocation state, and recorded as a content-addressed prompt-free decision. The decision projects only guard types that completed resolution; unsupported or failed later guards are not reported as applied. Empty intersections, stale operator binding, missing producer, digest substitution, and package revocation fail closed, while complete policy absence remains an explicit inert audit fact rather than a refusal. Additional anchors, guard operations/axes, and executable backends remain open. |
...
| `P085-027` | Freeze shared registry envelope, activation binding, and read-model mechanics | `partial` | `semantic-registry-core` and the matching Python middleware library now share canonical entry identity, revision/digest validation, provenance, exact monotone ceiling intersection, typed refusal, activation-binding and prompt-free causal inspection values. Optional ceilings use explicit `not-applicable|resolved|unresolved` states; unresolved authority refuses. Durable replay requires the complete binding and current generation, while entry-integrity and caller-expectation digest failures remain distinct; catalog corruption remains an integrity refusal rather than being masked as stale binding. The public selection and binding-replay APIs require one reusable resolved-ceiling value and borrow its effective set without per-selection cloning; the Python reference vertical atomically replaces an equivalent immutable cached projection after revocation. Property tests prove commutativity, idempotence, and non-growth, while allocation-reuse tests pin the hot-path contract. Corpus, Arca, Agent, Inquirium, and the Dator reference vertical now consume the shared resolver and preserve domain-owned entry schemas and closed semantic algebras. Accepted contracts, shared Rust/Python golden vectors, Schema Gate semantics, empty-set refusal, unavailable implementation and missing-capability tests are in place; the Dator and Arca Python verticals prove immediate deactivation and restart recovery, while Agent proves disabled-binding recovery remains inert. Signed package activation journals, rollback, and cross-domain lifecycle reuse remain open, so this common item is not yet `done`. No global semantic entry schema or mega-registry exists. |
...
| `P085-030` | Add Arca fulfillment, selection, and fan-in strategy registries | `done` | Arca owns code-backed selection, host-fan-out, and fulfillment entries above the shared Python registry library. Operator configuration narrows exact refs; preferred-provider selection, host fan-out, and step fulfillment all fail closed before their prior branches when an entry is disabled. The local lifecycle store preserves monotonic activation generation and exact deactivation across restart, channel inspection reports prompt-free registry state, and tests cover real execution paths rather than registry construction alone. No strategy entry can create a workflow action, bypass DAG validation, or widen the existing dispatch contract. |
...
1. Extend signed operator guards beyond the implemented `capability-use` and
   `agent-effect-admission` anchors and beyond the `restrict/grant-set` table backend.
   Preserve resolve-once prompt-free decisions, synchronous current-fact revalidation,
   and the ordinary capability/effect path at every added anchor.
2. Enforce the remaining vocabulary-only Inquirium resource axes at their owning
   runtime boundaries. Preserve distributor-owned per-axis safety ranges as the only
   path by which a signed operator envelope can widen a compiled default.
3. Publish and evaluate node posture and federated envelope declarations, bind them
   to the existing Corpus and Inquirium owning boundaries, then add
   modified-posture, trace-explorer, and three-node acceptance evidence.
4. Complete comparison, posture, and richer refusal views in the operator inspection
   surface. Guard, derived-capability, effective policy-hook, lifecycle occupancy,
   nearest-expiry, and index-health projections are implemented and must remain free
   of payload, signature, prompt, and output bytes.
5. Continue Phase 2 from the completed Dator, Corpus, Arca, Agent, and Inquirium
   domain migrations. Finish the signed package activation, current-grant,
   rollback, and federated portions of `P085-027`, then close cross-domain
   conformance and operator inspection in `P085-033`. Deferred WASM is not a
   prerequisite.
6. Begin Phase 3 with its fixed-vocabulary vertical only after the production Flow
