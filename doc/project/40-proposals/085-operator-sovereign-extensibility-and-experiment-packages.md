...
| `P085-003` | Freeze the P085 schema family and positive/negative fixtures | `partial` | In addition to the limit/resource-envelope family, accepted synchronized Schema Gate contracts now cover NSE offer/decision/table/middleware evidence, the daemon's prompt-free offer-resolution trace, the exact refusal vocabulary and projection, experiment package, durable/session activation, exact session deactivation, safe-mode action, transition, revocation, conformance report, bounded operator inspection with session expiry, loose signed import and receipt, and host-owned conformance-run request/result. Durable activation and revocation signatures share the closed `ed25519+domain-v1` shape. Positive/negative fixtures cover the new control contracts, import and conformance contracts, changed offer digest, unknown table/evidence/refusal values, prompt content in a resolution trace, production session activation, package-owned effective priority, required-producer fallback, and semantically inconsistent inspection state. `capability-derived.v1`, `operator-guard-hook.v1`, `operator-attention-budget.v1`, federation, posture, and Phase 2 enum/dispatch contracts remain open. |
...
| `P085-016` | Add operator CLI/UI for effective policy and authority inspection | `partial` | `operator-extensionctl inspect <store-root>` remains a canonical owner-store, permission-checked, audited offline read. Its live mode accepts only literal loopback endpoints and mirrors daemon `status`, `import`, `run-conformance`, `activate`, `rollback`, `revoke`, `session-activate`, `session-deactivate`, and `safe-mode` routes with bounded request/response bodies and prompt-free refusal-code diagnostics. Status preserves canonical JSON on stdout and reports session expiry plus remaining seconds on stderr. The accepted `operator-extension-inspection.v1` projection remains the canonical lifecycle output. Comparison, authority/attention/guard/posture views, richer bounded refusal explanations, and a graphical UI remain open. |
...
| `P085-018` | Add local and three-node acceptance plus trace-explorer evidence | `partial` | Unit/golden tests cover table/Rhai shared validation, supervised evidence containment and exact dispatch binding, durable and session binding-loss refusal, host-owned offer expiry/restart/replay/caller/digest/producer/budget refusals, opaque Flow decision projection, prompt-free resolution trace, Flow branching, selecting disagreement, required/advisory failure, pre-execution producer-budget reservation, interrupted recovery, signatures, WAL durability, unique session-event facts, atomic revocation/audit rollback, pinned-runner replacement and unsafe-permission refusal, corpus/runtime substitution, revocation, invalidation, expired-session replay refusal, restart discard, derived grants, guards, attention, and safe mode. Local process tests cover audited offline inspection, the reference evidence process, and the actual daemon HTTP lifecycle: signed allowlisted import remains inert, a host-selected bounded runner records conformance, signed activation and rollback advance generations, session activation replays exactly, restart discards it, the scheduler records expiry, explicit deactivation replays exactly, safe mode fences new activation and revokes session authority, restart recovers durable generation two, terminal revocation persists, and a later rollback refuses as `policy/revoked`. The common 28-code matrix has distinct reaching fixtures. Modified posture, trace-explorer integration, and three-node acceptance remain open. |
| `P085-019` | Synchronize Capability Registry, Node ledger, solutions, FAQ/HOWTO, and readiness classification | `partial` | The Capability Registry, Node ledger, crate READMEs, P085 tracker, generated schema documentation, and MVP readiness snapshot describe the implemented offer registry, decision resolver, supervised evidence process, signed loose import, daemon-owned durable/session/safe-mode lifecycle routes, conformance runner, CLI, exact refusal reachability, process smoke, and remaining federation boundaries. Cross-solution HOWTO/FAQ changes and federation/posture documentation wait for their contracts. P085 remains post-MVP. |
