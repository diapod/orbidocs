...
| `P085-006` | Add Inquirium resource envelopes and initial NSE hook expansion | `partial` | `inquirium-core` owns the 38-axis closed profile, complete distribution defaults, pure source-chain resolution, per-axis provenance, and canonical digest. The daemon persists, recovers, and rechecks signed envelopes and shares immutable effective profiles. A task/session seam now applies a sparse tighten-only overlay for one invocation without mutating the host baseline, while a pure federated intersection chooses the stricter cap or higher minimum and records both sources. The daemon applies `score-candidate`, `select-output-schema`, `select-repair-profile`, and `assemble-prompt` at the owning generate boundary; the same prompt-assembly path is reused by summarize, transform, and Assistant Channel generation. Static producers still pass the ordinary package activation and offer-bound NSE admission path. Bounded widening remains deliberately unavailable until a distributor-owned per-axis safety range exists; network binding of federated declarations and enforcement of the remaining vocabulary-only axes remain open. |
...
| `P085-008` | Add Agent decision hooks | `partial` | The shared validator covers `choose-next-step`, `shape-fanout`, and `classify-effect-risk`; fan-out may only narrow host bounds and risk may only increase. Agent now invokes those hooks at the controller-decision, child-fork, and effect-proposal boundaries. Existing lifecycle choices, monotonic fork validation, review policy, and host risk floor remain authoritative: a hook cannot invent completion, widen a child, lower risk, or repair a missing required HIL declaration. Operator inspection of effective hook decisions remains open. |
...
1. Extend signed operator guards beyond the first `capability-use` / `grant-set`
   table backend. Preserve resolve-once prompt-free decisions, synchronous current-fact
   revalidation, and the ordinary capability/effect path at every added anchor.
2. Define distributor-owned per-axis safety ranges before allowing bounded resource
...
4. Complete comparison, posture, and richer refusal views in the operator inspection
   surface. The first bounded guard-result and effective-capability-set projection is
   implemented and must remain free of payload, signature, prompt, and output bytes.
