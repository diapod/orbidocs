...
| `P085-042` | Add interactive attention and peer/refusal drill-down UI | `in-progress` | The render-only Node UI now consumes the accepted effective-policy projection, separates populated from explicitly unavailable sources, shows material differences, five domain registries, peer posture, and the decisive refusal, and follows only bounded projection-owned artifact refs with an explicit back-link. It links attention to the existing operator notification surface and never fetches protected artifact bytes. Remaining closeout is a host-composed production projection for the non-extension strata plus separate peer-mismatch and refusal-provenance views that link back to the same decisive fact. |
...
| `P085-044` | Make Corpus turn order operator-configurable and Flow-consumable | `in-progress` | **Dependencies:** P085 `P085-005`, `P085-007`, `P085-009`, `P085-014`, `P085-039`, and `P085-040`; P069 `P069-TURN-001` and `P069-TURN-002`; P049 `P049-008`. The accepted `nse-select-turn-order-table.v1` contract is Schema-Gated and compiles one closed role-order table into the common `order` proposal. Ordinary package activation, restart recovery, terminal revocation, producer lookup, opaque Flow consumption, single-use authority, centrally registered host-local `agent.turn-order.resolve` dispatch, and durable Corpus decision replay are implemented. Unit and local process evidence prove alternate valid ordering, exact candidate containment, restart invalidation of opaque offers, use-time authority revocation, recovered package producer authority, and terminal package revocation. Remaining closeout is the full Corpus process route proving explicit distribution fallback, stale offer/policy refusal, duplicate resolution, target-specific HIL ordering, and absence of floor authority in Flow output or scenario helpers. |
