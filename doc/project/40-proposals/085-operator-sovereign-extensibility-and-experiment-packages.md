...
For the restart-bound Dator reference vertical, activation generation is an explicit
operator/distributor configuration value rather than a database-migration counter.
The store persists its monotonic high-water: startup may retain or advance the value
but refuses a decrease, and rollback creates a newer generation rather than reusing
an older one. Legacy event migration is one atomic transaction and refuses conflicting
terminal facts without leaving a partially migrated table or index.
...
5. Continue Phase 2 from the completed classification inventory and Dator reference
   vertical. Finish the package activation, current-grant, rollback, and federated
   portions of `P085-027`, then migrate Corpus, Arca, Agent, and Inquirium in
   `P085-029` through `P085-032` before closing cross-domain conformance in
   `P085-033`. Deferred WASM is not a prerequisite.
