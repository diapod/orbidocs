...
1. Extend signed operator guards beyond the implemented five-anchor
   `restrict/grant-set` vertical to the remaining guard operations, axes, and backends.
...
2. Bind federated Inquirium resource-envelope declarations to the exact peer
   admission and runtime use that consumes them. Preserve distributor-owned per-axis
   safety ranges and monotone intersection; do not reinterpret local envelopes as
   network authority.
3. Complete the cognitively bounded operator projection, attention-specific graphical
   interaction, and richer peer/refusal drill-down. Prove the `P085-016` five-domain,
   38-axis, federated fixture and cross-view agreement; CLI posture publication,
   persisted peer comparison, and the render-only `explain`/Mermaid `graph` views are
   implemented regression obligations.
