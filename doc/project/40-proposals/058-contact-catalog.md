...
| P058-005 | Contact Catalog admission policy (attestation freshness, signature, expiry, purpose allowlist, TTL recommendation) | partial | Node `contact-catalog-core` now validates `contact-claim.v1` records and implements Day 2 typed evidence freshness / claim admission policy. Pending: participant-authorized signature verification and concrete `email-control` / `phone-control` passport profile verification. |
