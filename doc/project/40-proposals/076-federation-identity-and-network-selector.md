...
The projection is a federation authority set, not a claim that every node owns
every participant root key. Local readiness and local signing intersect the
participant-root projection with signing keys actually present in that node's
data directory. At least one local intersection is required when participant
roots are configured; absence fails closed. Remote root keys remain verification
authority only and must never be copied to a node merely to satisfy readiness.
...
- Root custody for `orbiplex-main` SHOULD be `threshold`, not
  ...
  reference. `DIA-ROOT-001` is adopted as the `orbiplex-main` root charter at
  version `0.1.0`; concrete custodian names, concrete signing keys, and the
  signed production pack remain activation deliverables and block treating
  `orbiplex-main` as production-trustworthy until documented and published.
...
Custodian keys may attest or authorize a separate operator identity through an
explicit binding or endorsement artifact, but they MUST NOT be converted,
promoted, or derived into a full operator identity. Operator identities remain
separately generated, separately revocable subjects. If tooling is added for
this boundary, prefer explicit flows such as `custodian-identity-init`,
`custodian-key-import`, `custodian-key-rotate`, and
`custodian-attest-operator`; do not hide the transition inside root ceremony
signing.
...
| P076-004 | Ship a signed `orbiplex-main` default federation pack | partial | Node ships a bundled `orbiplex-main` federation-root fixture that exercises the runtime path and populates the four surfaces, but it is not trusted by default. Treat it as a development/demo bootstrap fixture, not production root authority, until the adopted root charter is activated through concrete custodians, concrete keys, final signer roster, and a signed production pack. The charter is now an adopted initial implementing act: `doc/normative/50-constitutional-ops/en/ORBIPLEX-MAIN-ROOT-CHARTER.en.md` (`DIA-ROOT-001`, 0.1.0, with a PL twin); roster/keys remain deliberately unfilled until the activation ceremony. The threshold-root ceremony shape is frozen in this proposal, but the production roster and signed pack remain governance-authored activation work. |
...
| P076-014 | Provide hard-MVP federation-root ceremony tooling | done | Node-side tooling can derive public ceremony identities from node participant mnemonics, generate passphrase-encrypted PEM custodian keys by default, and sign root packs with encrypted or legacy plaintext PEM keys, mnemonics, plaintext participant key records, plaintext data-dirs, or encrypted participant signing keys opened through the narrow Rust unlock helper. The tool supports manifest digest verification, strict pre-assembly signature checks, per-root custody threshold verification, production `orbiplex-main` profile checks, and an end-to-end smoke test that covers encrypted PEM keygen/signing, encrypted data-dir signing, and production-profile positive/negative cases. Plaintext PEM output is an explicit disposable-fixture mode, not production ceremony storage. This remains MVP multisig custody, not FROST/DKG threshold signing. |
...
| P076-009 | Update Proposal 074's harness to pin distinct federation-root files per test node | done | Node acceptance seeders now write explicit signed `federation-root.v1` packs into local profile data dirs, refresh placeholder node ids to runtime `node:did:key` values after first boot where needed, and embed signed `federation-service-endorsement.v1` artifacts for Seed Directory bootstrap entries that claim `federation-endorsed`. Pre-first-boot roots may intentionally omit Seed Directory bootstrap entries until real runtime DID node ids exist, instead of giving official status to placeholders. Story-005 seeds one deterministic harness-only participant key per profile before first boot and includes the matching participant roots and signatures in both initial and refreshed packs; each private key remains on its own profile. Story-010 refreshes its root with the imported Marcin/Daniel participant roots and matching harness-only signatures; each node's readiness selects only the root key present locally, so no participant private key is copied between nodes. Story-011 additionally creates provider participants during first boot, inserts B/C participant attestation roots with matching self-signatures into the refreshed runtime root, restarts, asserts `/v1/seed-directory` active trust, and then exercises capability passport issue/publish through the active root projection. Story-005, Story-009, Story-010, and Story-011 no longer rely on loose trust-list shortcuts for official local Seed Directory bootstrap. These local roots are acceptance fixtures, not production org/threshold ceremony roots; Story-011 may include additional local participant roots only to exercise root-backed runtime projection. Broader multi-federation matrix polish remains a future P074/testnet extension. |
...
- **Do not**: reuse the `federation-root-ceremony-manifest.v1` schema string;
  ...
  preflight); auto-derive the endorser subject from whichever key signs; derive
  or promote an operator identity from a custodian key.
...
1. Complete the activation package under the adopted `orbiplex-main` root
   charter (`doc/normative/50-constitutional-ops/en/ORBIPLEX-MAIN-ROOT-CHARTER.en.md`,
   `DIA-ROOT-001`, 0.1.0): name custodians, generate keys, fill the roster,
   confirm the ≥2-of-3 threshold, and record the appeal process before treating
   the default federation as production-trustworthy.
2. Replace the bundled hard-MVP fixture with a real `orbiplex-main` signed pack
   once the activation ceremony is complete, then set production packaging to
