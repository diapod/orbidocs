...
| P076-009 | Update Proposal 074's harness to pin distinct federation-root files per test node | done | Node acceptance seeders now write explicit signed `federation-root.v1` packs into local profile data dirs, refresh placeholder node ids to runtime `node:did:key` values after first boot where needed, and embed signed `federation-service-endorsement.v1` artifacts for Seed Directory bootstrap entries that claim `federation-endorsed`. Pre-first-boot roots may intentionally omit Seed Directory bootstrap entries until real runtime DID node ids exist, instead of giving official status to placeholders. Story-005, Story-009, Story-010, and Story-011 no longer rely on loose trust-list shortcuts for official local Seed Directory bootstrap. These local roots are single-participant/single-secret fixtures, not production org/threshold ceremony roots. Broader multi-federation matrix polish remains a future P074/testnet extension. |
...
| P076-027 | Optional TLS pin for `seed_directory_bootstrap[]` entries | todo | Decided 2026-07-03: add an optional `tls_certificate_sha256` field to `SeedDirectoryBootstrap` (same `sha256:<base64url-43>` shape as `BootstrapSeedPeer`'s pin) for deployments that do not want to rely on DNS/WebPKI alone. Symmetry note: directory responses are signed artifacts, so the pin adds transport privacy/integrity, never authority. Compat: another optional field in an `additionalProperties: false` schema — older loaders reject packs carrying it, so coordinate schema+loader+fixtures in one change and note the pack-compat impact. |
