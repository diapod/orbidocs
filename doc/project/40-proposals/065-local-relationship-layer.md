...
relationship-class.v1
  ...
  retention/profile-ref?  # optional ref; absent resolves to local-relationship/default
...
1. `relationship-class.v1` retention horizon inherits from the host profile when
   ...
   contract.
2. `pairwise-nym-binding.v1` uses a registry-driven context-kind set seeded
   with `messaging`, `ad-direct`, `agora-topic`, and `inquirium-session`.
   Unknown contexts fail closed; future context kinds require an explicit
   schema/registry update instead of ad-hoc caller spelling.
...
| P065-D3 | Public/federated protocol capability | `false` | deferred | Local-authenticated host API remains the MVP boundary. A federated relationship capability needs a separate threat model, disclosure policy, remote audit semantics, and revocation/freshness contract before implementation. |
| P065-D4 | Registry-driven pairwise nym context kinds | `false` | done | `context/kind` is now seeded from the local registry (`messaging`, `ad-direct`, `agora-topic`, `inquirium-session`) and exposed as the schema enum for `pairwise-nym-binding-fact.v1` / `pairwise-nym-binding.v1`. `local-relationship-core` validates both facts and projections against that seed registry, so unknown context kinds fail closed. Future context kinds require an explicit schema/registry update rather than ad-hoc caller spelling. |
| P065-D5 | Relationship retention profile inheritance | `false` | done | `relationship-class.v1` now treats absent `retention/profile-ref` as inheritance of the explicit `local-relationship/default` profile and validates any present ref as a narrow host-owned profile pointer. `local-relationship-core` rejects inline `retention/*` lifecycle policy fields, daemon class writes and vault replay fail closed for unknown profile refs, reserved class overrides are disabled by default, store status reports `all-profiles-resolved` only when every ref is known, Node UI displays inherited/explicit retention refs, and tests cover inheritance, replay refusal, unknown-profile refusal, reserved override refusal, and known custom profile admission. |
| P065-D6 | Richer multi-operator UX layer | `false` | deferred | Post-MVP operator layer for filters, comparison views, owner-scope explanations, and safer bulk review. Current UI remains sufficient for hard-MVP class, membership, predicate, package trust queue, and decision audit workflows. |
| P065-D7 | Relationship cardinality performance profile | `false` | deferred | Post-MVP profiling under realistic relationship cardinalities, including projection rebuild time, HMAC index lookup behavior, pairwise nym history growth, and group expansion cost. Current hard-MVP tests prove correctness and fail-closed behavior, not production-scale envelopes. |
