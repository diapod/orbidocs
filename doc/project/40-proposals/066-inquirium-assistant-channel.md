...
| `assistant-model-story-012-powerdns-acceptance` | Compose two real local Bielik runtimes with the Story 012 vfkit/Corpus/Agent vertical. | `done` | The additive profile and schema-gated closed report validator are implemented. B and C each supervise a direct `http_local` `llama-server`, select the exact runtime from their Agent spec, and expose the actual content-addressed Agent product as inert Corpus text only after digest, stable provider-alias, and response-shape verification. The model alias is stable and no materialization path enters model identity. The retained 2026-07-23 macOS arm64 run proves two distinct inference refs, exact package and guest-image pins, deterministic HIL/P083 actuation, the localhost-only listener, and all three `localdomain` records within budget. Evidence: `node:tools/acceptance/story-012-shared-chair-terminal/reports/2026-07-23.story-012-powerdns-full-system.macos-arm64.json`, file digest `sha256:2dd351f4df724a57c5ed3c68c98fbd20da2349038dd5d40af21212bc1a3f70d8`. Package-lifecycle install/rollback/removal evidence stays owned by the existing local-model acceptance rather than being duplicated here. |
