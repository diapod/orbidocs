...
| `assistant-model-story-012-powerdns-acceptance` | Compose two real local Bielik runtimes with the Story 012 vfkit/Corpus/Agent vertical. | `in-progress` | The additive profile and schema-gated closed report validator are implemented. B and C each supervise a direct `http_local` `llama-server`, select the exact runtime from their Agent spec, and expose the actual content-addressed Agent product as inert Corpus text only after digest, stable provider-alias, and response-shape verification. The model alias is stable and no materialization path enters model identity. A retained real run with the exact PowerDNS guest image remains required; package-lifecycle install/rollback/removal evidence stays owned by the existing local-model acceptance rather than being duplicated here. |
