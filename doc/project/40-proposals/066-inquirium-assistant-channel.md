...
| `assistant-model-llama-server-linux-acceptance` | Prove the same full lifecycle with real `llama-server` and Bielik bytes on Linux x86_64/CPU. | `in-progress` | The acceptance runner now selects a Linux x86_64/CPU profile, builds the same pinned source with curl/OpenSSL/Metal/native-host specialization disabled, checks executable x86_64 ELF identity plus system-library-only dependencies, uses the same package/lifecycle implementation, and emits the same accepted report family with `platform/ref = linux-x86_64-cpu`. Python and Schema Gate tests pin the profile and report shape. A full run on an actual Linux x86_64 host and its retained metadata-only report remain required; macOS tests or emulation do not count as that evidence. |
