...
2. **Apple Silicon companion candidate.** The macOS arm64 profile should evaluate
   [`mlx_lm.server`](https://github.com/ml-explore/mlx-lm/blob/main/mlx_lm/SERVER.md)
   first among MLX-family servers. Upstream describes it as an OpenAI-like API,
   but explicitly does not recommend production exposure and provides only basic
   security checks. The host must therefore treat it as a private, untrusted
   worker: supervise its pinned Python environment and model bytes, bind it only
   to loopback or host-private local IPC, deny undeclared egress and ambient host
   authority, and never expose it as a public service. The existing `http_local`
   OpenAI-compatible adapter should communicate directly with that worker where
   conformance permits; an MLX-specific normalization limb is justified only for
   incompatibilities in streaming, errors, usage, tool calls, or structured
   output, and must not create an HTTP-to-stdio proxy or leak MLX semantics into
   Inquirium. Runtime capabilities for streaming, batching, prompt cache, KV-cache
   quantization, tool calls, JSON-Schema structured output, and speculative
   decoding are separate, versioned, conformance-backed facts; unsupported or
   unproven capabilities remain false. The macOS arm64 distribution policy
   prefers a fresh, passing MLX candidate, while `llama-server` remains the
   portable fallback and required cross-platform baseline. Model Runtime receives
   no additional Inquirium authority from this preference.
...
7. **Distributor trust.** Official artifacts require a distributor-signed
   ...
   The reference host additionally requires an exact host-owned release-catalog
   entry binding `release/ref`, `package/ref`, `manifest/digest`, and the set of
   active allowed release-key ids. When an allowed key exists only through a
   federation-root provider-key endorsement, the same entry also binds its
   exact `provider/ref`; only a manually configured trust root is an explicit
   host-wide override. A valid signature from a generally trusted key does not
   authorize an uncatalogued, cross-provider, or substituted manifest.
   Distributor key generation and signing are offline ceremonies; private
   release keys are neither daemon configuration nor node data.
   A federation root may explicitly endorse a provider release public key for
   the domain-separated purpose `model-package-distribution`. This is a
   separate, expiring and revocable authority artifact: ordinary federation
   membership, node endorsement, or service authority does not imply it. The
   verified projection supplies the exact `(provider/ref, key/id, public/key)`
   tuple to the distributor trust resolver, but it does not bypass the exact
   release catalog, source trust, digest verification, conformance, or
   operator-controlled activation. Root rotation, endorsement expiry/revocation,
   provider-key revocation, a provider mismatch, or a purpose mismatch
   invalidates the projection fail closed. Operator diagnostics preserve only a
   bounded set of at most 32 endorsement refs, provider refs, key ids, and
   inactive reason codes plus an omitted count; they do not copy signatures or
   public keys. A release entry without `provider/ref` intentionally admits only
   manually trusted keys. Inactive states such as expiry, root mismatch, and
   revocation are isolated per endorsement, while a malformed artifact, invalid
   signature, or unknown sovereign subject rejects the complete effective-key
   snapshot. This atomic failure is deliberate: trust-config integrity takes
   precedence over availability of otherwise unrelated providers.
...
12. **Release gate.** Contract conformance, source and digest verification,
    ...
    hard release gates. `offline_ok` is backed by a host-enforced process
    isolation mode rather than egress metadata alone: the managed child may bind
    and serve loopback HTTP, including replies on accepted connections, but may
    not initiate outbound connections or create non-stream Internet sockets.
    Exact restart adoption requires durable evidence that the same
    process incarnation was launched under that mode; unknown platforms and old
    unrestricted records fail closed. Performance and resource measurements are
    operator- and NSE-visible diagnostics in v1, not universal correctness thresholds.

    The reference macOS mechanism currently uses Apple's deprecated
    `sandbox-exec` interface. Its absence must refuse launch rather than degrade
    to unrestricted execution; replacing it with a host-owned macOS isolation
    mechanism is a platform evolution concern outside the hard-MVP denominator.
    The Linux acceptance profile is explicitly x86_64 and uses an
    architecture-specific seccomp audit/syscall table. Linux architectures
    without their own verified table remain unsupported fail-closed. The filter
    also denies `io_uring_setup`, including io_uring-backed disk I/O, because
    preventing asynchronous network bypass takes precedence for current managed
    runtimes, which do not require that optimization.

Implementation status (2026-08-26): the shared package/runtime bridge now
registers `mlx_lm_server` as a closed macOS arm64/Metal family rather than
special-casing it in Inquirium. It launches one pinned self-contained executable
directly over the existing `http_local` OpenAI-compatible mapping, clears the
ambient environment, supplies only host-rendered `PEX_ROOT`, uses the immutable
model directory as the child working directory, and retains the common no-egress,
health, conformance, activation, restart-adoption, rollback, and removal strata.
The asset store builds that flat model directory atomically from separately
verified Model/Auxiliary assets and re-verifies exact contents before reuse.
Managed candidates are offered to NSE and direct Inquirium generation in the
same explicit selection order only after they become routable. The host-owned
platform policy makes MLX the default on the admitted Apple Silicon profile and
keeps managed llama as its deterministic fallback; Linux x86_64/CPU keeps llama
as the default. The generator, pure manifest validator, Schema Gate,
provider-neutral acceptance-report v2, full local-import lifecycle runner, and
additive two-runtime Story-012 Qwen profile are implemented. The resulting
metadata-only Story report v2 is a registered Schema Gate family rather than a
harness-local JSON shape.

The native macOS arm64/Metal package lifecycle is now qualified for the exact
Qwen profile. A strict hash-locked builder produces a byte-reproducible
self-contained MLX-LM launcher with embedded portable CPython and confines both
scie and PEX state below the host-rendered `PEX_ROOT`. The retained v2 report
binds launcher digest `sha256:dUNHOrNxan1C-hFfxfeWKN_c9GFkncz050QXu7AIH_I`,
manifest digest `sha256:j-Pjw7r69GoW5T-57OG2OYLizcaBDK2lVesGMdnZFN4`,
and the exact nine-file Qwen revision. It passes all ten lifecycle checks,
including no-egress activation, real inference, dirty-process adoption,
upgrade/rollback, removal, and generation 3; measurements are install 36,913 ms,
cold warmup 58,150 ms, and first inference 589 ms. The mapped structural matrix
also proves that malformed empty text-message lists are rejected before worker
I/O. This contains, but does not repair, the bare upstream endpoint defect.
The exact Qwen descriptor and Story harness
reject llama-server fallback, rely on the existing typed host validators rather
than claiming native grammar support, and bind package, manifest, lifecycle,
model, memory, and timing evidence into the Story report. The lifecycle report is
retained at
...
(file SHA-256 `086fd22b0cb3abcf93be98e79003642e7c7d4d91bcb04ee56f84ef1fa1cdea99`).
The MLX profile tracker remains `in-progress` because no independent retained
proof promotes streaming, batching, prompt cache, KV-cache quantization, tool
calls, JSON-Schema output, or speculative decoding; every such fact remains
`not-probed`. The Story tracker now waits only on the retained two-runtime vfkit
run and calibrated resource/timing evidence. `llama-server` remains the required
portable baseline.
...
### 8.5.3 Inference Execution And Product Identity

An `inference/ref` identifies one host-admitted model invocation. It is minted by
the host/controller path when that invocation is materialized and is carried by
the corresponding Agent step as prompt-free causal evidence. A `step/ref`
identifies the bounded controller transition that consumed or produced the
invocation, while a `product/ref` and its digest identify the resulting immutable
bytes. None of these references grants authority to execute, publish, or accept
the product.

Execution identity and content identity are deliberately distinct. Two admitted
invocations may produce byte-for-byte identical output, especially under a
deterministic model and equal effective parameters. Their `inference/ref` values
must still differ, while their product digests may legitimately be equal. An
acceptance report that claims two model executions therefore proves cardinality
with distinct host-issued inference refs and uses product refs or digests only to
bind the exact returned bytes.

Distinct execution is not the same claim as epistemically independent judgment.
Neither equal outputs disprove independence nor different outputs prove it. A
profile that needs independent first judgments must define context isolation,
model/seed policy, and admission evidence explicitly rather than use digest
inequality as a proxy.

Retained evidence may join `step/ref`, `inference/ref`, and product identity for
audit, but it must not retain prompt content merely to make that join possible.
The references remain scoped trace handles under host retention policy; they are
not provider-generated authority and must not be inferred from output diversity.
...
1. Preserve the delivered P090-007a inline Assistant path and extend the remaining
   producer/disclosure variants without weakening exact-context pre-I/O model
   acceptance, independent acknowledgement or uncertain-attempt refusal.
...
| :--- | :--- | :--- | :--- |
| `assistant-model-story-012-powerdns-mlx-acceptance` | Compose two pinned Qwen2.5-Coder 7B MLX runtimes with the Story 012 vfkit discovery vertical. | `in-progress` | The closed runtime descriptor, additive profile, preflight, neutral Story harness seam, and Schema-Gate-registered v2 report validator are implemented. B and C render separate supervised `mlx_lm_server` processes through the existing OpenAI-compatible mapping; the profile refuses all llama-server/GGUF inputs, assumes no native grammar, fails malformed CandidatePlan/review output at the existing host validators, and binds exact runtime, model revision, package manifest, lifecycle report, memory, and timing evidence. The prerequisite native lifecycle is retained and passing. Promotion now awaits the two-runtime Story run with measured memory, warmup, turn, deliberation, and end-to-end budgets; no fallback or legacy Qwen/llama evidence can satisfy this row. |
...
| `assistant-model-egress-provenance` | Carry the Proposal 090 realized inference execution provenance through assistant responses, terminal transcript facts, trace records, replay, export/import, and Activity projections. | `partial` | Every inference-derived result and every terminal outcome after possible dispatch carries or references a schema-valid provenance value; a proven pre-I/O refusal is `not-dispatched`/`not-applicable`/`none`, while insufficient evidence after possible dispatch is explicit `unknown`. The host stamps the value from the admitted execution path and evidence rather than request metadata or model output; cache, transcript, bundle import/export, and rebuild preserve it; no consumer can downgrade known non-local or mixed execution to local. P090-007a (2026-09-06) implements the inline result/refusal, transcript-owned replay, authoritative attempt fence and UI/feed path; broader external resolution and producer variants remain open. P090-007b now repairs finalized synchronous-text publications after trace, charge and transcript projection failures; absent completion remains fenced. Source, HTTP class, charge and excision checks survive restart. |
| `assistant-remote-disclosure-ui` | Show preflight egress disclosure and post-result execution provenance as two distinct operator-facing states. | `partial` | Before every admitted non-local inference I/O, Node UI shows the host-admitted `inference-execution-posture.v1` value bound to the assertion owner, exact runtime/profile generation, turn/context scope, validity, and processing-boundary ref, plus destination/provider class when local disclosure policy permits it. Where local classification/context policy requires acknowledgement — including the current remote-turn slice — the UI also shows its ceiling and scope. The posture remains an input to existing authority gates, not a grant. After completion, the rendered answer and Activity item show realized locality or `unknown` plus an optional provider ref. Provider redaction preserves the non-local badge, and UI tests prove that request policy is never presented as execution fact. P090-007a (2026-09-06) implements exact-context preview/execute, independent acknowledgement, and separate declared/realized UI and Activity projections with deterministic HTTP/UI tests; broader provider-disclosure variants remain open. |
...
| `assistant-model-source-trust` | Add operator-managed HTTPS-origin and canonical-local-root trust. | `done` | `ModelPackageAuthorityStore` verifies domain-separated Ed25519 trust artifacts against a host key resolver and an exact prior approved-decision projection, persists one-shot/durable rules, reserves one-shot authority by exact operation and manifest digest, and consumes it only after the lifecycle claim is durable. Active competing reservations, changed-manifest replay, expiry, revocation, and malformed durable artifacts fail closed; expired unconsumed reservations can be reclaimed. Canonical real local roots, contained regular files, and HTTPS redirects bounded to signed origins remain mandatory. Each HTTPS hop resolves to an exclusively public address set pinned into that request, so loopback/private/link-local/special-use results and DNS rebinding fail closed. A total large-file timeout and rolling minimum-progress window bound slow sources independently. The daemon now creates the bounded Confirm question and notification, requires its durable affirmative answer and bound operator identity, signs the exact trust artifact through `HostSigner`, and persists it before planning or install admission. Callers submit only the source target, mode, expiry, question ref, and manifest bindings; they cannot construct the admitted authority DTO. |
| `assistant-model-operator-endorsement` | Permit operator-owned models through detached manifest signatures. | `done` | The contract exposes canonical signature bytes that bind the manifest digest while excluding recursive signatures. The authority registry verifies the detached endorsement against the exact manifest, node, profile, operator, question, decision, and timestamps, and persists exact replay. The daemon renders the bounded package/profile review as a Confirm operator question, derives the decision and operator from durable host state, signs through `HostSigner`, and records the admitted endorsement idempotently. The reference generator computes the manifest and digest without signing it, and no wire caller can supply an admitted endorsement projection. |
| `assistant-model-llama-server-package` | Ship the canonical managed `llama-server` profile for macOS arm64/Metal and Linux x86_64/CPU. | `in-progress` | `node/tools/inquirium-model-package.py` produces an operator-endorsed manifest for either supported platform around an already reviewed local `llama-server` and the revision-pinned Apache-2.0 `speakleash/Bielik-1.5B-v3.0-Instruct-GGUF` Q8_0 asset (1,699,568,288 bytes, below the 5 GiB reference ceiling). Python and Rust share a golden manifest/digest fixture plus a Unicode, escaping, nested-value, and safe-integer JCS parity corpus; floating-point package values are rejected by the generator. The offline `inquirium-model-release` tool creates a dedicated Ed25519 release key, signs the exact canonical distributor manifest, verifies it independently, protects private-key files, and documents additive rotation followed by revocation. The daemon requires an exact approved release-catalog entry and active allowed key before signature verification; tests reject an uncatalogued release, revoked key, substituted key, and substituted manifest. The daemon materializes verified assets, creates a dynamic `http_local` model-runtime candidate, supervises a real process, runs conformance while the candidate remains quarantined, and exposes it to routing only after the journaled active-profile commit. Bounded fake-process smoke plus real macOS arm64/Metal and retained real Linux x86_64/CPU acceptance prove that boundary. Publication of a production-key-signed distributor manifest remains open. No model or runtime is silently downloaded. |
| `assistant-model-llama-server-macos-acceptance` | Prove the full managed package lifecycle with real `llama-server` and Bielik bytes on macOS arm64/Metal. | `done` | The opt-in acceptance runner builds a pinned self-contained `llama-server`, installs the revision-pinned Bielik Q8_0 GGUF through approved HTTPS authority, verifies CAS reuse, activates only after conformance, and performs real generate plus Assistant Channel inference. It asserts that the provider model snapshot equals the stable alias rather than a materialization path, adopts the exact surviving process after daemon `SIGKILL`, upgrades to a distinct runtime build, rejects stale rollback, rolls back by generation, refuses active and rollback-pinned removal, removes an unreferenced package idempotently, and emits the schema-gated metadata-only `inquirium.local-model-acceptance-report.v1`. The recorded run completed install in 507,889 ms, warmup in 35,605 ms, and first inference in 1,088 ms, ending on active generation 3. |
| `assistant-model-llama-server-linux-acceptance` | Prove the same full lifecycle with real `llama-server` and Bielik bytes on Linux x86_64/CPU. | `done` | A native Debian x86_64 run built the pinned source with curl/OpenSSL/Metal/native-host specialization disabled, verified x86_64 ELF identity and system-library-only dependencies, installed the pinned Bielik Q8_0 bytes, passed conformance and real generate/Assistant inference, adopted the exact process after dirty restart, upgraded A -> B, rejected stale rollback, rolled back to A as generation 3, and proved protected plus idempotent removal. The retained metadata-only nine-check report is `node:tools/acceptance/inquirium-local-model/reports/2026-08-24.inquirium-local-model-acceptance.linux-x86_64-cpu.json`, file digest `sha256:3643c46550ca6bc4044920eb1b69c43bd58e22a928b78a2528d6f80134c58ef3`; measurements are install 727,463 ms, warmup 334,839 ms, and first inference 21,677 ms. The 420,000 ms operational warmup ceiling was calibrated from two clean native observations and does not weaken conformance or authority. Python and Schema Gate validate the retained report. |
| `assistant-model-story-012-powerdns-acceptance` | Compose two real local Bielik runtimes with the Story 012 vfkit/Corpus/Agent vertical. | `done` | The additive profile and schema-gated closed report validator are implemented. B and C each supervise a direct `http_local` `llama-server`, select the exact runtime from their Agent spec, and expose the actual content-addressed Agent product as inert Corpus text only after digest, stable provider-alias, and response-shape verification. The model alias is stable and no materialization path enters model identity. The retained 2026-07-24 macOS arm64 run passes the exact 17-check validator and proves two distinct inference refs, exact package and guest-image pins, separate solver/reviewer roles, two round-robin cycles, failed and corrected experiments after fresh terminal feedback, Agent `propose`, admitted HIL, P083 `claim -> invoke -> release`, no effect derived directly from Room prose, the localhost-only listener, and all three `localdomain` records within budget. Evidence: `node:tools/acceptance/story-012-shared-chair-terminal/reports/2026-07-24.story-012-powerdns-full-system.macos-arm64.json`, file digest `sha256:706e1c78f069a2277cb86dde79919880e6450a1505d2aa8df08d26ab0a739de4`. Package-lifecycle install/rollback/removal evidence stays owned by the existing local-model acceptance rather than being duplicated here. |
| `assistant-model-provider-key-federation-endorsement` | Allow a federation root to endorse a provider release public key for model-package distribution. | `done` | The accepted `inquirium.model-package.provider-key-endorsement.v1` contract is closed, domain-separated, expiring, revocable, and bound to one exact federation-root version and digest plus purpose `model-package-distribution`. The installer reuses the canonical sovereign participant/org signature-threshold verifier and projects the exact `(provider/ref, key/id, public/key)` tuple. The daemon requires an endorsed-only release entry to retain the same `provider/ref`, intersects that scope with the exact approved-release catalog, and re-evaluates it at activation, rollback, and planned-transition recovery; manually trusted keys remain the only explicit host-wide override. Bounded metadata-only denial diagnostics retain at most 32 inactive reasons plus an omitted count without signatures or public keys. Tests prove equal/reversed validity-window refusal, cross-provider and unscoped-release denial, mixed active/inactive aggregation, diagnostic truncation, whole-snapshot refusal for one integrity-invalid endorsement, root mismatch, expiry, endorsement revocation, key revocation, bad signatures, purpose mismatch, service-endorsement substitution, missing active root, key conflict, and endorsement-without-catalog denial. Federation membership remains non-authorizing. |
| `assistant-model-mlx-profile` | Evaluate and, if conformant, ship an MLX-family Apple Silicon companion profile. | `in-progress` | The host-owned family registry, Apple Silicon platform gate and MLX-first default, shared direct-Inquirium/NSE selection order, deterministic managed-llama fallback, direct supervised `http_local` mapping, provider-native `default_model` selector below the semantic model identity, `/health` readiness, cleared child environment with exact `PEX_ROOT`, immutable flat multi-asset model materialization, activation/conformance quarantine, memory-bound manifest generator, pure Rust manifest validation, provider-neutral acceptance-report v2, and complete local-import lifecycle runner are implemented. A strict hash-locked PEX/scie builder pins portable CPython 3.12.14, MLX-LM 0.30.2, MLX/MLX-Metal 0.30.1, and produces byte-identical launchers; the managed stderr sink now survives daemon loss so the exact worker can be adopted after dirty restart. Linux x86_64/CPU retains managed llama as its default. The retained 2026-08-26 native report passes all ten lifecycle checks on the exact nine-file Qwen revision, generation 3, no-egress transport, install 36,913 ms, cold warmup 58,150 ms, and first inference 589 ms; file SHA-256 is `086fd22b0cb3abcf93be98e79003642e7c7d4d91bcb04ee56f84ef1fa1cdea99`. The mapped structural matrix proves missing, non-array, empty, non-object, and blank-role/content messages are refused before HTTP I/O, containing rather than repairing the bare upstream defect. The row remains `in-progress` only because independent retained evidence for streaming, batching, prompt cache, KV-cache quantization, tool calls, JSON-Schema output, and speculative decoding is still absent; all seven report facts remain `not-probed`, no optional capability is advertised, and `llama-server` remains the required portable baseline. |
| `assistant-model-package-release-gate` | Add supply-chain, rollback, security, and platform acceptance gates. | `in-progress` | The host-side gate binds exact manifest/receipt/profile values, current manifest authority, and exact duplicate-free asset sets, then checks platform, verified asset descriptors, the configured model-size ceiling, loopback-only binding, empty egress, offline capability, and fresh passing conformance. Activation re-reads every referenced CAS object and re-resolves current distributor/operator authority before creating a profile transition and immediately before durable commit; install-time receipt authority cannot survive provider-key endorsement expiry, revocation, federation-root rotation, or exact-catalog removal. The same revalidation runs for rollback and planned-transition recovery. The daemon derives OS, architecture, backend, bind host, egress, offline policy, and asset ceiling from trusted runtime/config state; activation and rollback requests cannot supply these values or conformance evidence. A stale `expected-current` is rejected before candidate startup and checked again by the lifecycle transaction. Candidate publication and retirement of the prior runtime share one supervisor lock; a prior process that cannot stop remains quarantined and its cleanup failure is reported without turning the already committed activation into a false conflict. Dynamic candidates remain non-routable until process health and conformance pass and the profile transition commits. Managed package transports require `no_egress`; macOS Seatbelt and Linux x86_64 seccomp tests prove outbound connection refusal while retaining loopback bind/listen, conformance asserts the launch mode, and dirty-restart adoption requires exact durable isolated-launch evidence. Manual HTTPS redirect handling rejects downgrades, protocol-relative targets, and unapproved changed origins while accepting an explicitly allowlisted HTTPS origin; each hop retains public-address pinning. Exact release-catalog and offline signing tests reject substituted manifests/keys and revoked keys. Real lifecycle evidence now covers both supported platforms, with the Linux report retained under the acceptance pack. Publication of the production distributor artifact remains open. |
