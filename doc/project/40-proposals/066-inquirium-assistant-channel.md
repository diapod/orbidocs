...
7. **Distributor trust.** Official artifacts require a distributor-signed
   ...
   active allowed release-key ids. A valid signature from a generally trusted
   key does not authorize an uncatalogued or substituted manifest. Distributor
   key generation and signing are offline ceremonies; private release keys are
   neither daemon configuration nor node data.
   ...
   verified projection may supply the provider public key to the distributor
   trust resolver, but it does not bypass the exact release catalog, source
   trust, digest verification, conformance, or operator-controlled activation.
   Root rotation, endorsement expiry/revocation, provider-key revocation, or a
   purpose mismatch invalidates the projection fail closed.
...
| `assistant-model-provider-key-federation-endorsement` | Allow a federation root to endorse a provider release public key for model-package distribution. | `todo` | Freeze a domain-separated, expiring and revocable provider-key endorsement contract; verify sovereign signatures and purpose against the active federation-root snapshot; project only the exact provider `key/id` and public key into distributor trust; invalidate on root/key/endorsement revocation or expiry; and prove that federation membership, service endorsements, key endorsement alone, and uncatalogued manifests cannot authorize install or activation. |
