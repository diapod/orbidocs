...
No unresolved questions remain for this proposal's MVP contract. One post-MVP
storage-lifecycle question remains open below and does not weaken expiry
enforcement at authorization time.
...
11. What durable lifecycle should expired or explicitly revoked assistant
    context grants follow after they stop authorizing reads? The active registry
    already fails closed on expiry, but retaining every record indefinitely
    creates unbounded storage while hard deletion loses authorization history.
    Candidate policies are: move terminal grants into an append-only audit/archive
    projection and prune the active table in bounded batches (sensible default),
    retain all terminal grants indefinitely, or hard-delete them after a declared
    class-keyed retention period. The decision must preserve enough provenance to
    explain a historical context decision without making the active grant table
    the audit source of truth.

## Next Actions

1. Continue conformance expansion after the landed
   `generate`/`embed`/`classify`/`rerank` runner slice when image/audio/transform
   contracts graduate into `inquirium-core`.
...
5. Add explicit context-grant revocation and future remote-route-derived egress
   acknowledgement on top of the landed classified source resolvers and
   participant/session-bound grant registry; resolve Open Question 11 before
   adding a retention sweep.
...
| `assistant-history-per-participant` | Scope assistant history to the participant. | `in-progress` | Daemon derives principal/participant from the authenticated host boundary; fallback stream refs, transcript projection/search, feedback lookup, and context grants are participant-scoped, while request-body `participant/ref` is not authority. Memarium facts carry principal/transcript refs and classification-derived spaces; cross-device projection rebuild and retrieval remain later work. |
...
| `assistant-transcript-projection` | Search + semantic/thematic tagging as a projection over Memarium streams. | `in-progress` | Daemon owns a participant-scoped SQLite read model with bounded full-text substring and exact-tag filters, content-ref visibility, excision projection, and local-control search capability. It is updated after Memarium-first/local-fallback writes and stores descriptors rather than externalized bulk. Explicit rebuild-by-Memarium-replay tooling and semantic embedding tags remain future work. |
...
| `assistant-context-source-grants` | Require explicit operator grants per context source and session. | `in-progress` | `inquirium.context-grant.issue.request.v1` and local-control `inquirium.context-grant.issue` persist grants in SQLite, bind them to host-derived participant, session, source kind/ref, classification ceiling, expiry, and exact egress acknowledgement, and reject caller-declared participant authority. Explicit revocation and richer approval UI remain future work. |
| `assistant-context-grant-retention` | Bound the active context-grant registry without erasing authorization provenance. | `todo` | Authorization already fails closed at expiry. Open Question 11 must choose archive projection, indefinite retention, or class-keyed hard deletion before a sweep is implemented; the sensible default is an append-only audit/archive projection plus bounded pruning of the active table. |
| `assistant-model-acceptance-policy` | Add classification-aware model acceptance policy. | `in-progress` | `ContextAcceptancePolicy` evaluates class-keyed include/drop/fail-closed decisions after assembly; grant ceilings are enforced and egress acknowledgement is represented and checked. The assistant remains structurally strict-local, so remote-candidate derivation of egress requirements and declassification remain later work rather than caller metadata. |
...
| `assistant-interaction-notification-projection` | Derive notification actions from operator-question interactions. | `in-progress` | Pending questions project idempotently to durable notifications with expiry and typed input; answers route back before handled state. Cancel/supersede transitions close the paired notification, and replay repairs a prior notification-update failure. Richer widget-specific UI rendering remains future work. |
...
| `assistant-feedback-loop` | Add `inquirium.inquiry-feedback.v1` (verified/refuted/amended). | `in-progress` | Core validates classified verified/refuted/amended facts, bounded metadata/grounding/amendments, and required human correction shape. Local-control append binds the active participant, requires an existing non-excised assistant result, persists an append-only participant-scoped projection, and attempts a Memarium fact with deterministic idempotency. Training-candidate marking requires an explicit flag and grant ref, but actual training consumption must still verify that separate grant before admitting data. |
