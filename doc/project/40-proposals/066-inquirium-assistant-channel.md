...
5. Add explicit context-grant revocation and future remote-route-derived egress
   ...
   participant/session-bound grant registry. Explicit revocation and Decision
   12 archive-before-prune maintenance are landed; remote-route-derived egress
...
| `assistant-local-runtime-target` | First target: OpenAI-compatible local HTTP over Ollama/`llama.cpp`-class server; baseline anchored to `llama.cpp`-class runtime. | `done` | Daemon coverage proves profile pinning, required conformance, and direct local `http_local` chat-completions with host-owned model binding and no Python proxy. `node/config/examples/60-inquirium-baseline-ollama.json` is the reference catalog; `node/tools/inquirium-baseline-profile.py` renders operator configuration for loopback Ollama or managed `llama-server`, verifies executable/model files, pins the model digest, atomically activates a reviewed profile under the Node data directory, and reports active runtime refs while leaving installation to the operator/distributor. |
...
| `assistant-transcript-projection` | Search + semantic/thematic tagging as a projection over Memarium streams. | `in-progress` | Daemon owns a participant-scoped SQLite read model with bounded full-text substring and exact-tag filters, content-ref visibility, excision projection, and local-control search capability. `inquirium.assistant.transcript.rebuild` performs a bounded, participant-filtered Memarium replay and atomically replaces only that participant's projection after every embedded fact validates; empty, saturated, malformed, or unavailable source reads leave the prior projection intact. Semantic embedding tags remain future work. |
...
| `assistant-escalation-policy` | `ModeKeyed` escalation threshold (feed → notification). | `in-progress` | `inquirium-core` now has typed escalation reasons and a configurable monotone `ModeKeyed<ClassKeyed<AssistantEscalationLevel>>` floor. Daemon config owns the active `assistant_mode`, validates the policy at startup, rejects model-supplied mode metadata, and host-shapes model-proposed blocking questions, rejecting an operator-question control unless the resolved level is actually `operator-prompt`. Registered widgets are delivered through P057 notifications. Automatic notification creation for sustained baseline loss and boundary-risk feed events remains later work. |
