...
2. **Apple Silicon companion candidate.** The macOS arm64 profile should evaluate
   ...
   unproven capabilities remain false. A conformant MLX profile may be preferred
   by the macOS arm64 distribution policy, while `llama-server` remains the
...
| `assistant-model-mlx-profile` | Evaluate and, if conformant, ship an MLX-family Apple Silicon companion profile. | `todo` | Evaluate `mlx_lm.server` first as a pinned, supervised, host-private Python worker over loopback HTTP or natively supported private local IPC, with offline materialized model assets, denied undeclared egress, no public listener, and no additional Inquirium or host authority. Reuse the existing `http_local` OpenAI-compatible adapter where conformant; keep any required MLX normalization below Model Runtime and cover streaming, errors, usage, tool calls, and structured output without adding an HTTP-to-stdio proxy. Selection evidence compares operational/API parity with `llama-server` and independently proves versioned flags for streaming, batching, prompt cache, KV-cache quantization, tool calls, JSON-Schema structured output, and speculative decoding; absent or unproven server capabilities remain false. A fresh passing MLX candidate may be the macOS arm64 preference, while `llama-server` remains the portable fallback and required cross-platform baseline. Failure to qualify does not block the canonical `llama-server` package. |
