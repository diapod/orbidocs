...
| P083-015 | Reuse the coordinator for an Agent-executed Corpus experiment passage | done | The composed Story 012 technical acceptance profile validates a signed, retained-turn-bound experiment proposal, binds its signer node to the author's canonical Room invite, and compiles the inert `CandidatePlan` under a host-owned operation allowlist before a local Agent receives one fresh latest-state passage. The requester verifies the content-addressed Inquirium product and accepts only a closed `propose|no-effect` decision for the compiled flow node; `no-effect` reaches neither claim nor invoke. The Agent effect remains lease-free through inference and requires an admitted operator-question decision. Before dispatch the daemon rechecks proposal expiry and exact passage constraints for the actuation interface, grant, generation, operational context, method, input schema, payload digest, classification, and lease ceiling. Only the existing P083 coordinator may then claim the exact interface generation, invoke one descriptor-admitted method, emit its receipt, and release immediately. Unsupported remote runtime execution still fails admission until a passage adapter exists; a remote participant may nevertheless author the attributed inert plan while requester node A retains all effect authority. The local in-process adapter avoids an extra HTTP hop but preserves grant, generation, lease epoch, sequence, idempotency, and receipt checks. The runtime slice is complete, but the digest-pinned delegated Story 012 profile remains `partial`: its retained source proves requester-owned lifecycle records, correction feedback, and zero direct Room-prose effects, while CandidatePlan author/signature, per-effect lease refs, and HIL-before-effect ordering require a fresh evidence run. This is a non-defining consumer proof: it neither restricts Corpus to technical deliberation nor turns this profile's terminal grammar into general P083 semantics. |
