...
2. ~~Add broader operation wrappers beyond `generate`/`embed`, starting with
   ...
   conformance fixtures. Image/transform surfaces remain later operation
...
| P063-03 | Define Inquirium capability vocabulary | partial | `model-runtime` now carries structured operation classes including generate/embed/batch-embed/classify/rerank and routing capabilities; the workflow-facing Inquirium vocabulary still needs the remaining helper names and operator-facing phrasing. |
| P063-04 | Define request/result schemas | partial | Operation-specific contracts are implemented in `inquirium-core` for `inquirium.generate.{request,response}.v1`, `inquirium.embed.{request,response}.v1`, `inquirium.batch-embed.{request,response}.v1`, `inquirium.classify.{request,response}.v1`, and `inquirium.rerank.{request,response}.v1`. Generate includes optional `profile/ref` and a narrow request-parameter allowlist; embedding contracts cover inline vectors plus lease-backed batch artifact output; classify/rerank contracts cover bounded text, simple classification label tokens, closed label/candidate sets, normalized confidence/relevance, duplicate-free results, request/response pair invariants, and `top_k`/`top_n` ceilings. Summarize/transform/image contracts remain future work. |
| P063-05 | Implement Inquirium Core wrapper over model-runtime | partial | The daemon exposes `inquirium.generate`, `inquirium.embed`, `inquirium.classify`, and `inquirium.rerank` host surfaces over routable runtime candidates and host-owned model bindings. Generate supports optional `profile/ref`, NSE advice, HTTP adapter invocation, and Story 005 simulator acceptance; embed has a deterministic local handler; classify/rerank have deterministic and local HTTP/simulator handlers with request/response pair validation. Transform/image wrappers and provider-backed embedding adapters remain future work. |
...
| P063-07 | Add host capability gate and audit | partial | `inquirium.generate`, `inquirium.embed`, `inquirium.classify`, and `inquirium.rerank` require local control-plane auth or an explicit JSON-e/module inference grant; `allowed_calls` alone is not authority, and grants bind request size plus optional runtime/profile refs. Discovery advertises only routable candidates backed by implemented handlers. Each operation writes a dedicated metadata-only trace with a host-keyed/domain-separated HMAC request digest and no prompt/input, generated text, vectors, classification labels returned, or reranked content. Future operation families must add the same gate and audit shape. |
...
| P063-11 | Deterministic stub runtime | done | Implemented as an explicit `deterministic_stub` adapter-instance transport with daemon generate execution, local deterministic embedding vectors, closed-label classification, and deterministic reranking for contract/conformance/smoke coverage; it is opt-in catalog data, not an ambient production fallback. |
