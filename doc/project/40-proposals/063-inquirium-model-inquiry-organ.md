...
2. ~~What is the minimal schema set for MVP: only `inquirium-request.v1` and
   ...
   `AdapterImplementation` declares supported operation classes. MVP started with
   `generate`, then `embed` and `batch-embed`; `classify` and `rerank` now exist as
   operation contracts in `inquirium-core`, while summarize/transform/image follow as
   their own contracts.
...
2. ~~Add broader operation wrappers beyond `generate`/`embed`, starting with
   ...
   HTTP/simulator execution, metadata-only traces, budget charging, and
   conformance fixtures. Summarize/transform and deterministic artifact-only
   image generate/edit verticals now repeat the same shape. The first live
   provider path maps the neutral image contract through the supervised OpenAI
   adapter directly to the provider image endpoints while retaining host-owned
   source-lease resolution and artifact publication.
3. Keep `train.adapt` behind the landed bounded deferred worker, evaluation
   gate, and publication lifecycle over the content-addressed dataset manifest
   and exact grant-rechecked admission. A concrete trainer may replace the
   deterministic reference behavior without changing the worker contract.
   Semantic transcript tagging, canonical
...
| P063-13 | Data-plane lease schema + pilot | done | Daemon persists model-runtime leases under `storage/model-runtime-leases.sqlite`, exposes local create/read APIs, supports artifact/object-store/query scopes plus canonical/allowlisted `file://` leases, hides expired leases, and restricts caller metadata. `P089-012` closes raw-file eligibility: `AdapterDataPlaneCharacteristics` is explicit and default-unknown; host admission requires the exact runtime/model binding, current routability, host-local read-only access, both lease policies, strict-local policy, and managed-process no-egress enforcement. The lease registry independently enforces canonical caller/adapter/runtime roots and TTL limits. Unit and daemon endpoint regressions cover admission and fail-closed refusal; transport kind alone never grants file access. `batch.embed` and grant-rechecked local `train.adapt` use `DeferredOperationRegistry` with operation/runtime/model-binding-bound read/write leases. Batch artifacts and evaluation-passing training artifacts flow through the shared `ArtifactOutputIntent`, object-store digest/size verification, and provenance descriptor path. The bounded training worker restart-recovers private continuations through operation-kind/status-scoped bounded pages, rechecks authority, reports progress, caps decoded adapted-model artifacts at 96 MiB, and uses an atomic compare-and-set status transition so cancellation cannot race publication; event-log tests prove the losing transition is not appended. Training manifest windows use canonical UTC second timestamps. P090-004b3/004b4 now separate immutable execution commits from publication. Bounded batch accepts immutable JSON input and invokes the embedding adapter; caller-supplied output is refused. The shared producer worker replays verified source bytes and artifact sidecars, with BDO remaining a projection. H-1–H-14 review bounds shutdown, compensates refused pending admission and acknowledges completed recovery without relaxing execution fences. |
| P063-14 | Emit and preserve realized inference-execution provenance | accepted | Inquirium owns a provider-neutral descriptor for every inference-derived result and post-dispatch terminal outcome, while pre-dispatch refusals remain explicitly `dispatch = not-dispatched` with `locality = not-applicable`. Requested locality and egress policy are not execution proof. Exact contract semantics are governed by Proposal 090; implementation and acceptance remain open in Proposal 064. P090-004b2/007b now cover ordinary text-controller and finalized Assistant publication recovery; the broader producer capability remains partial. P090-004b3/004b4 add the scoped durable batch/training/image checkpoint and source-image ancestry. P090-004c adds the ten-operation terminal matrix, intrinsic ingress separation, sanitized pre-dispatch refusals, exact terminal traces, sink-failure/restart preservation and a same-run proof manifest. Generalized producer/effect profiles and the full P090 inventory remain open; this is deterministic local evidence, not a new physical acceptance passage. |
