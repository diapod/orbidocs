...
2. ~~Add broader operation wrappers beyond `generate`/`embed`, starting with
   ...
   image generate/edit verticals now repeat the same shape. Live image-provider
   execution remains a later adapter layer.
3. Continue with the deferred `train.adapt` worker, evaluation, and publication
   lifecycle after the landed content-addressed dataset manifest and exact
   grant-rechecked admission. Semantic transcript tagging, canonical
...
| P063-05 | Implement Inquirium Core wrapper over model-runtime | partial | The daemon exposes inference-grant-gated host surfaces for generate, embed, classify, rerank, summarize, transform, image-generate, and image-edit over routable runtime candidates and host-owned model bindings. Summarize/transform lower into the hardened generate substrate while retaining their own contracts, budgets, and traces. Image operations use explicit runtime operation declarations, deterministic raster stub execution, operation-bound read/write leases, `ArtifactOutputIntent`, object-store persistence, and artifact-only responses; image conformance runs without persisting fixture bytes. Generate/embed also have real HTTP/provider paths, while classify/rerank have deterministic and local HTTP/simulator handlers. Live image provider adapters and the deferred train-adapt worker remain future implementation layers. |
