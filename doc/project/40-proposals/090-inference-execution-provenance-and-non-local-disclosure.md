...
cross-node consumer matrix. The remaining Inquirium operations, scoped posture,
consumer policies, the guided provider registry, operator surfaces, and complete
cross-node provenance acceptance remain implementation work. Existing locality policies,
...
| `P090-004` | Bind Inquirium pre-execution posture and produce realized provenance for every inference-derived result and post-dispatch terminal outcome. | `P090-003` | `partial` | `inquirium-core` owns an exact response-bound `inquirium.result-provenance.v1` sidecar, and `inquirium-host` has pure conservative posture and result-binding functions. The real daemon Agent-passage `generate` path now stamps a completed dispatch with exact runtime/model/model-snapshot bindings and explicit `unknown` locality/egress when the selected runtime lacks admitted realized evidence; its V2 response is schema-gated and replay preserves the same descriptor. `LocalityMode` and `TrustMode` Rust defaults are `Unknown`, while malformed current catalog entries missing required fields still fail deserialization. Completion still requires every registered operation, admitted realized runtime characteristics, pre-I/O refusal, direct result/trace/effect carriers, cache, artifact, and deferred replay coverage. |
...
1. Implement `P090-003a` as a separate operator-facing layer over the open
   ...
   from a URI or DID.
2. Complete producer coverage: extend the implemented Inquirium `generate`
   vertical to every inventoried operation and admitted realized-runtime
   evidence without deriving execution facts from transport or catalog state;
   complete generic external-runtime disclosure conformance beyond the retained
   real Codex path.
3. Extend the implemented ordinary-controller bridge through repository-wide
   translator no-drop gates, then add scoped posture and Assistant disclosure,
