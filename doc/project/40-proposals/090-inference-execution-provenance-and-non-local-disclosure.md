...
batch/training execution and artifact/deferred preservation still require their
own durable boundaries. Ordinary synchronous-text Inquirium Agent controller
recovery is now covered by P090-004b2/004b2a.
...
1. Complete scoped posture exposure beyond the implemented operator inspection
   of exact runtime/profile catalog bindings: expose Agent and external-runtime
   scopes, bind invocation policy independently, and integrate the pre-I/O gate.
   The completed `P090-003a` registry and its CLI/UI are display/configuration
   vocabulary, not proof of execution or authority to dispatch.
2. Complete producer coverage beyond the direct-success traces: add typed
   post-dispatch terminal outcomes, compatible V2 response successors,
   batch-embed and train-adapt results, parent-preserving summarize/transform
   joins, and cache/artifact/deferred/effect-intent replay without deriving
   execution facts from transport or catalog state. Generic external-runtime
   disclosure conformance is complete under P090-005, beyond the retained real
   Codex path; broader Assistant disclosure remains P090-007.
3. Extend the implemented ordinary-controller bridge and executable inventory
   into every remaining repository-wide translator no-drop gate, then add
   scoped posture and Assistant disclosure,
   followed by service-offer
