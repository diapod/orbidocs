...
P091 reuses sealed-registry, bounded carrier, conservative projection and
inventory-checker idioms, not inference-provenance semantics. Configuration
describes intended/resolved behavior; it cannot prove realized locality or egress.
The configuration contract keeps its own identities and types. This reciprocal
reference does not add a P091 dependency to P090's existing completion gates.
...
and layering guards are implemented by `P090-003`. A first producer-to-Agent
vertical is now partial: Inquirium `generate` and External Agent Runtime
terminal products/outcomes emit durable compatible successors, while both an
Inquirium passage and an ordinary External Agent Runtime controller step
preserve a monotone join through trace, restart replay, and `agent.outcome.v2`.
Real three-host Story-012 evidence now also retains the exact ordinary-controller
sidecar for the final Codex Reviewer product after a mid-passage Agent restart;
this is a bounded producer-to-Agent acceptance slice, not completion of the
cross-node consumer matrix. The admitted provider registry, guided CLI/UI, and
runtime/profile operator inspection are implemented. Direct successful
`generate`, `embed`, `classify`, `rerank`, `summarize`, `transform`, image-generate,
and image-edit executions now bind exact result sidecars into durable traces from
explicit runtime characteristics; absent characteristics stay `unknown`, and a
typed pre-I/O generation refusal is recorded as `not-dispatched`. The executable
translator inventory runs its named verification sets in CI. P090-004a now
carries bound generate/embed results, sanitized failure dispatch evidence,
generate-backed translation, current embedding cache/restart and exact Agent
passage ancestry through a compatible shared V2 envelope. Optional scoped
Agent/external-runtime invocation preflight precedes new I/O and does not block
historical replay. Other operation terminals, general public V2 negotiation,
batch embedding, training, artifact/deferred/effect carriage, higher-layer
consumer policies and the complete cross-node provenance matrix remain
implementation work. Existing locality policies,
...
| Semantic Index embedding row/projection | Planned rebuildable consumer of Inquirium embeddings; no current Node runtime carrier exists | Require a content-bound descriptor/ref in the first durable row schema rather than introducing a legacy provenance-free row. Deferred until a Node implementation exists; see [the conditional carrier decision](#conditional-semantic-index-carrier-2026-09-25). | `orbidocs:doc/project/60-solutions/022-semantic-index/022-semantic-index.md`; no implementing Node crate exists as of the audit |
...
For the ordinary Agent controller path, the compatible migration uses an
immutable `agent.controller-execution-provenance.v1` fact. It binds the exact
Agent binding, controller step, external request, retained product ref and
digest, and the provider-neutral realized descriptor. The controller execution
value carries that fact only between daemon strata; `agent.step-trace.v1`
references the durable fact and `agent.outcome.v2` names its generic
`execution-binding/ref`. Neither the step nor the outcome reconstructs execution
characteristics from a runtime name, transport, model snapshot, or product ref.
Recovery must validate the fact against both the exact external-runtime V2
commit and the controller step before repairing a missing trace or outcome, and
must reject substitution, stripping, or a conflicting replay.
...
### Conditional Semantic Index carrier (2026-09-25)

The operator made the Semantic Index carrier conditional. Solution 022 is still
`planned`, and no Node crate, ledger row, or durable embedding row exists, so P090
has nothing to migrate or gate there. Requiring that carrier would make P090
completion, and with it the `alpha-node` completeness blocker, depend on building a
component outside P090's scope.

The requirement itself is unchanged and moves to the moment the owner appears: the
first durable Semantic Index embedding row schema must bind a content-bound
provenance descriptor or ref instead of introducing a provenance-free row. Until a
Node implementation of Solution 022 exists, Node's translator inventory records the
carrier as `deferred`, with this decision and its condition cited. A deferred entry
carries no sources, tests, or verifications, is skipped by `--promote`, and can
never satisfy a scoped `--require` claim. When an implementation lands, the entry
returns to `planned` and becomes part of P090 completeness again. This decision
does not defer any other carrier.
...
| `P090-003` | Define separate canonical `inference-execution-posture.v1` and `inference-execution-provenance.v1` contracts in a pure `inference-provenance-core`, plus comparison/join/projection rules, error vocabulary, and Schema Gate corpus. | `P090-002` | `done` | `inference-provenance-core` binds posture and realized provenance to exact subjects and processing boundaries, conservatively migrates the complete legacy locality/trust matrix, and owns domain-separated `JcsV1` source and projection identities. One `inference-provenance-common.v1` schema owns the shared ref, digest, provider, disclosure, evidence, and extension vocabulary; posture requirements deliberately exclude observation-only `unknown` and `derived` bases. The canonical 16 KiB inline and 64 KiB descriptor limits, content-addressed external form, bounded lineage, hidden uncertainty components, and provider-neutral error vocabulary are mirrored in both schemas and Schema Gate. Exhaustive tables cover all 25 locality, 9 dispatch, 16 egress, and 16 provider-disclosure pairs; unit and property tests cover identity, permutation, associativity, idempotence, replay, redaction, missing ancestry, no-downgrade, and unrelated-boundary policy. Positive, schema-negative, and semantic-negative fixtures are synchronized into Node; every schema conditional has a non-empty local discriminator requirement, and a mechanical test refuses copied common definitions. Mechanical dependency tests protect the pure core and the five currently inventoried semantic consumers; adding another consumer requires extending that explicit inventory and guard. Generated schema docs, full Orbidocs schema validation, crate tests, full Schema Gate tests, and Clippy with warnings denied pass. No provider-specific enum enters the shared core. |
| `P090-003a` | Add the admitted provider-ref registry and guided operator configuration surface. | `P090-003` | `done` | The canonical `inference-provider-registry.v1` contract and pure registry own bounded refs, aliases, display metadata, disclosure defaults, generation and JCS identity. The shared daemon configuration service validates raw JSON before set normalization, resolves exact admitted names, previews URI/DID-derived refs without fetching or retaining the source identifier, and atomically installs only the confirmed preview against the current registry digest under a nonblocking lock. The offline CLI accepts hand-authored drafts, lists/resolves entries and requires typing `yes`; Node UI lists exact refs for configuration and has separate preview and initially unchecked confirmation forms. Tests cover canonical fixture/Rust identity agreement, malformed inputs, collisions, stale previews, duplicate raw aliases, concurrent writes, CLI process restart, escaped UI output, and the real authenticated daemon HTTP path. This installs display vocabulary, not runtime routing or inference authority. |
| `P090-004` | Bind Inquirium pre-execution posture and produce realized provenance for every inference-derived result and post-dispatch terminal outcome. | `P090-003` | `partial` | `inquirium-core` owns an exact response-bound `inquirium.result-provenance.v1` sidecar, and `inquirium-host` has pure conservative posture and result-binding functions. Daemon configuration now admits explicit provider-neutral execution characteristics independently of transport and catalog posture, validates their runtime and registered provider refs at startup, and defaults missing characteristics to explicit `unknown`. Successful direct `generate`, `embed`, `classify`, `rerank`, `summarize`, `transform`, image-generate, and image-edit executions commit exact response-bound sidecars into their durable metadata traces before returning V1 success; a typed pre-I/O `generate` refusal is represented as `not-dispatched`, `not-applicable`, and no egress. The Agent-passage `generate` V2 response remains schema-gated and replay preserves its descriptor. `LocalityMode` and `TrustMode` Rust defaults are `Unknown`, while malformed current catalog entries missing required fields still fail deserialization. P090-004a now adds bound generate/embed results, sanitized failed/ambiguous terminals, the compatible shared V2 envelope, generate-backed translation and current embedding cache restart. P090-004b1 and P080-044/045 provide explicit V2 response selection, and P090-004b2 closes the scoped ordinary text-controller recovery matrix. P090-004b3/004b4 now close bounded batch/training source commits and image/artifact publication recovery. P090-004c closes terminal/refusal and sink-recovery coverage for the ten currently admitted operations, with a common same-run proof gate. Additional producer profiles and general effect carriage remain open; this scoped closeout does not promote the complete P090 inventory. |
| `P090-005` | Bind External Agent Runtime posture and make its products and terminal outcomes produce the same provider-neutral provenance. | `P090-003`, provider-neutral P089 runtime contract | `done` | The host layer derives posture only from an explicit host-owned context, not transport. V2 product and terminal-outcome contracts bind exact canonical V1 bytes. The daemon durably writes their V2 commit sidecar before V1 compatibility, recovers an interrupted V2-first bundle without duplicate charging or driver I/O, records `not-dispatched` plus `not-applicable` locality and `none` egress after a host-local pre-turn refusal with zero turn-driver calls, records ambiguous dispatch only after an uncertain dispatch boundary, and leaves the generic fake at `unknown`. The Codex composition root explicitly admits `non-local` plus occurred egress with provider identity withheld; neither `stdio` nor provider-specific session/account data enters the generic contract. Three-host Story acceptance now retains a real Codex-backed descriptor through its ordinary Agent controller sidecar. The 2026-09-06 generic disclosure matrix closes conformance for complete, partial, withheld and unknown identities in both completed and cancelled execution: posture, product and terminal descriptors retain exact provider refs and non-local/egress facts, and restart under a fresh unknown characteristic preserves the entire V2 commit without driver I/O, extra facts or duplicate charging. |
| `P090-006` | Expose scoped posture and propagate/join realized provenance through Orbiplex Agent products, passages, lineage, and `AgentOutcome`. | `P090-004`, `P090-005` | `partial` | `agent-core` owns compatible V2 passage product, successful invoke response, terminal selection, passage trace, and outcome contracts. The host distinguishes current execution from inherited Agent products, requires exact parent refs for a complete join, preserves explicit incomplete ancestry as `unknown`, and refuses substitution or stripping. Memarium facts write stronger V2 values first, repair V1 compatibility projections after interruption, and replay a four-passage Inquirium chain without reinvocation or duplicate charging. An ordinary External Agent Runtime action writes `agent.controller-execution-provenance.v1` before its controller step/trace, deterministically rebinds the external product descriptor to the retained Agent artifact, exposes the sidecar ref as trace evidence, and emits `agent.outcome.v2`; dirty recovery recomputes the exact bridge and appends nothing. Real three-host Story-012 run `federation-run:story-012-physical-three-host-three-node-codex-reviewer:20260904T160513Z` then interrupted and resumed the Codex Reviewer Agent and retained the exact final sidecar ref `agent-controller-execution-provenance:agent:story-012-reviewer:11` in its passing Story report. Operator inspection now reuses a pure five-axis scope matcher and checks runtime/profile catalog and registry fingerprints with half-open validity; a real HTTP regression proves missing, matched and wrong-generation outcomes. P090-004a adds independently scoped optional Agent/external-runtime policy preflight before new I/O, rechecks current fingerprints and preserves durable replay after policy change. Inquirium passages consume the received bound result rather than reconstructing evidence. P090-004b2/004b2a now cover the ordinary text-controller journal; P090-007b and P090-009b/c cover finalized Assistant and signed Corpus publication/receiving paths. Guided policy UX, general parent/effect joins and broader Room/remote projections remain open. |
| `P090-006a` | Gate every inventoried Inquirium/Agent result translator and higher-layer carrier against metadata loss or downgrade. | `P090-003`, `P090-004`, `P090-006` | `partial` | Schema Gate refuses stripped provenance across the representative Inquirium result sidecar, Agent/external-runtime V2 product, invoke-response, selection, trace, controller-execution sidecar, and outcome chain. Durable partial-write and restart tests cover the representative generate passage, external-runtime terminal commit, pre-turn refusal, and ordinary-controller bridge through terminal Agent outcome. Story-012 now binds each external Reviewer evidence check to the exact observed controller step, permits a later status projection without confusing value with observation time, rejects an older unrelated sidecar, and retains the final sidecar ref in its report. Node owns a 27-entry inventory with source/test pointers plus operation-enum and translator discovery across Inquirium Rust modules. Its reusable named verification sets use bounded argument vectors rather than a shell; CI executes the deduplicated checks for every `done` or `partial` entry, and `--require`/`--promote` run the selected checks after refusing unfinished or unlisted claims. P090-004a adds operation-specific checks for bound results, terminal translation, current embedding cache/restart, invocation preflight and the daemon Agent passage. The current embedding-cache row is done; other carrier rows retain honest partial status. P090-012a adds a promotable scoped durable-consumption aggregate, including real supervised role/WSS-AD/Arca and controller/Assistant sink matrices. This proves only the named checks, not runtime completion of remaining rows. Completion still requires the full `P090-001a` inventory: every operation, aggregate, cache/deferred/artifact replay, Assistant, Corpus, Room, offer, and procurement translator plus promotion-time inventory enforcement. The Semantic Index carrier is `deferred` under the [2026-09-25 decision](#conditional-semantic-index-carrier-2026-09-25) and rejoins this requirement when a Node implementation exists; promotion skips it and never counts it as done. |
| `P090-007` | Add Assistant preflight and post-result disclosure. | `P090-004`, `P090-003` | `partial` | Current checkpoint implements authenticated preview/execute, independent sensitive-input acknowledgement, V2 response/refusal carriage, transcript-owned exact sidecars, restart replay without producer I/O, and UI/feed projection; P090-007a/007b and P090-012a close the inline synchronous text and finalized publication gate. Broader provider-disclosure variants and external resolution remain outside that scoped evidence. Acceptance target: before every admitted non-local inference I/O, UI shows posture bound to the exact context and profile generation; acknowledgement remains a separate gate required according to local classification/context policy. Response, transcript, trace, activity feed, and UI then show realized locality/egress and conservative unknown; route changes remain visible. |
...
### Workflow and Whisper translator slice

Reconcile historical inventory notes before extending the remaining translators.
P090-004c already closes admitted producer refusals; P090-006b/006c/009d already
close the scoped controller input and synthesis joins. Their generalized parent
rows remain partial, but those checkpoints are not outstanding blockers.

| ID | Work item | Status | Completion gate |
| :--- | :--- | :--- | :--- |
| `P090-006d` | Preserve host-owned result ancestry outside Workflow/JSON-e/Flow values. | `in-progress` | A content-bound immutable sidecar survives projection, call failure and retained-result replay. Host adapters supply evidence independently of template values. Unknown calls remain incomplete; the parent budget refuses before further I/O. Canonical join remains in inference-provenance-core. |
| `P090-006d1` | Recover exact committed Flow call sources before aggregation. | `done` | Host-scoped recovery key pins exact configuration/input/evidence and clock. Conditional intent precedes dispatch; source commit precedes verification/join. Restart reuses committed sources, rechecks admission before new effects, and fences unresolved dispatch. The five-command `generic-workflow` gate passed on 2026-09-21: partial writes, mixed/unknown parents, substitution/stripping, source retained on aggregate overflow, concurrent claims, no repeat inference/charge/publication and real local HTTP restart. Does not close deferred continuation, decision-authority adapters or the all-port parent task. |
| `P090-006e` | Carry redaction ancestry through Whisper preparation and retained drafts. | `in-progress` | Opt-in observed preparation verifies the exact wrapper and commits the private draft/result pair atomically, fences the whole source snapshot, bounds retention and verifies SQLite reopen. Manual writes clear the source association. Private sync seals the retained wrapper under a descriptor-addressed key; deterministic replay verifies exact payload and key reuse. Determinism of the transformer does not prove inference-free inputs. Actual Sensorium source admission, terminal recovery and full Memarium transport/recovery qualification remain open. |
| `P090-012d` | Gate the Workflow/Whisper carriers. | `in-progress` | The five-command inventory set covers mutation, stripping, missing parents, mixed sources, failure, bounds, SQLite reopen and real local daemon HTTP extraction. Remaining current-port and deferred/publication cases must be added before promotion. Retained-result replay is not workflow redispatch or recovery of arbitrary external effects. |

Operator scope decision (2026-09-20): cover **all currently available Flow
capabilities**, not only a small admitted redaction or Inquirium profile.
The shared collector already reserves evidence independently of values for
every call/decision. Typed extraction covers direct Inquirium, nested Flow and
Agent passage/controller sources; unintegrated ports remain explicitly unknown.
This is an implementation checkpoint, not permission to mark the generic
translator complete while known source metadata still awaits an adapter.
The `workflow.step.completed.publish` adapter now atomically retains the exact
request and host-owned sidecar under the dispatch key, with a minimal read
projection and restart regression. Nested dispatch carries exact input evidence
outside template values. The prospective collector checks both parent count
and inline composition limits before further calls; a larger received proof
still refuses binding rather than dropping or weakening prior evidence.
The P090-006d1 checkpoint adds a separate, bounded host-owned source journal
before aggregation. It preserves the source when the aggregate cannot fit;
it does not promise that an oversized aggregate becomes representable.

Recovery decision (2026-09-21): an explicit recovery key is scoped by caller
and Flow identity, with immutable configuration/input/evidence binding and
clock. A committed source is replayable; an unresolved intent is not permission
to dispatch again. Current authority applies before new effects. A retained
decision verdict cannot become current authority, so decision-offer recovery
requires a dedicated adapter. Deferred observations remain deferred. Private
source retention conveys no export authority and is not an exactly-once claim
for arbitrary effects. Node owns the mechanism and executable matrix in
`docs/integration/middleware/MIDDLEWARE-RESULT-PROVENANCE.md`.

Review checkpoint (2026-09-21): Node review 282 hardened malformed publication
readback and safe recovery diagnostics, and qualified exact replay at the fixed
Whisper journal capacity. Capacity and time retention remain independent;
diagnostics do not authorize redispatch or expose private provider errors.
Disposition and executable evidence are recorded in
`node:docs/evidence/inference-provenance/P090-WORKFLOW-WHISPER-REVIEW-282.md`.
This does not promote the still-open translator parent rows.

Whisper now pins draft provenance by an explicit journal association, cleared
even by an identical manual write. Its product sync seals the retained wrapper
into a separate Personal fact keyed by descriptor identity. Deterministic tests
cover reopen, exact payload/key reuse and refusal before any host I/O; complete
Memarium transport/recovery acceptance remains open.

The remaining work includes other publication/source adapters, broader source
resolvers, deferred continuation and terminal/restart matrices. The current private
append-only result journal and Whisper projection do not claim a complete
temporal migration of the older intake store.
...
| ID | Work item | Depends on | Status | Done criteria / evidence |
| :--- | :--- | :--- | :--- | :--- |
| `P090-004a` | Carry scoped generate/embed execution results through response projection, translation, cache/restart and Agent invocation preflight. | `P090-003a`, current `P090-004` and `P090-006` foundations | `done` | Runtime-owned dispatch facts travel with their exact result; the compatible shared `inquirium.response.v2` envelope preserves operation-specific V1 payloads. Generate-backed summarize/transform retains inference parents and rebinds failure operation identity. Embedding cache separates historical execution/usage from retrieval/charge and refuses corrupt bound entries. The operation-specific inventory gate passes terminal dispatch-state tests, actual HTTP translation/trace checks, scoped Agent/external-runtime preflight and session-free refusal, L1/reopened-SQLite corruption negatives, and a four-passage daemon restart with exact non-local/withheld ancestry and no duplicate Agent charge. This is local deterministic evidence, not a new physical Story-012 run. Parent P090-004/006/006a remain partial for all-operation/public/consumer coverage. |

The shared response envelope is a carriage contract, not a universal output
ontology: the operation owner still validates the enclosed result. This reuses
...
| ID | Work item | Status | Completion gate |
| :--- | :--- | :--- | :--- |
| `P090-004b` | Complete producer terminal coverage, public V2 selection, ordinary Inquirium Agent carriage, batch/training and artifact/deferred preservation. | `done` | Closed for the ten currently admitted operations and existing producer profiles by P090-004b1/004b2/004b2a/004b3/004b4 plus the P090-004c terminal gate. Exact source evidence, ordinary text-controller accounting and artifact publication remain separate durable transitions; replay never reexecutes or charges twice. Existing V1 success projections remain available, while explicit V2 never silently falls back. The final local gate covers refusal, received/ambiguous failure, sinks and recovery; generalized profiles/effect joins remain in P090-004/006, not this scoped aggregate. |
| `P090-007a` | Implement Assistant preflight disclosure and realized result carriage through response, transcript, trace and UI. | `done` | Exact authenticated preview/execute, independent sensitive-input acknowledgement, under-lock revalidation, V2 success/refusal and transcript-owned replay are implemented. A durable keyed attempt fence precedes model I/O and refuses automatic redispatch after uncertain completion, including after restart; it is not a rebuildable index. Metadata-only preflight refusals do not invent execution. HTTP, UI/feed and executable inventory cover this inline consumer path. Finalized synchronous-text repair is now covered separately by P090-007b; physical acceptance is not claimed. |
| `P090-009a` | Implement offer/catalog posture, generic procurement carriage and Corpus result validation in dependency order. | `done` | Shared host policy filters before pagination; selected declarations and actual evidence remain distinct. Generic result V2 and Corpus signed V2 enforce buyer policy and frozen offer comparison before accountable closeout/admission, retaining exact evidence through restart. P090-008c now supplies real supervised role/WSS-AD/Arca deployment proof for inline text. External descriptor resolution remains broader P090-008a work and is not certified by this checkpoint. |
| `P090-010a` | Preserve Room contribution provenance and derive participant read models. | `done` | Explicit V3 content-bound contribution evidence, minimal disclosure, four-boundary Schema Gate, WSS delivery/exact replay, evidence-aware sequence recovery and durable observation restart are implemented. Agent effects attach explicitly referenced owned durable sources, retaining incomplete ancestry instead of guessing. Independent receiving policy and bounded sender-relative participant badges preserve unknown and peer-attested strength; readiness frames do not count as inference. Scoped declarations and general federation projection remain P090-010/011; no new physical passage is claimed. |
| `P090-009b` | Sign provenance-bearing Corpus answer/publication V2 and persist it with the legacy view. | `done` | Exact answer, admitted draft lineage and publisher policy assessment share one signed envelope and one durable round transition. Shared V1 signature verification and all-four-boundary Schema Gate reject signature/content/sidecar substitution. The local daemon process passage covers V1 and explicit V2 publication, restart without the product, exact replay after a stricter policy change, and refusal of a new publication under that policy. Historical publications are not enriched. This is deterministic local acceptance, not physical federation evidence. |
| `P090-009c` | Apply the receiving Corpus policy to both procurement answers and local Agent publications. | `done` | Local publication and receiving-host policies are independent of signed producer assertions. Bid admission freezes the catalog-admitted declaration before ranking; named unresolved sources refuse, and later offer expiry or removal cannot invalidate the retained commitment; known violations override incomplete evidence and peer claims remain peer-attested. AD preserves the signed V2 within a 128 KiB materialization budget. Forty-nine Corpus host tests pass in the 2026-09-06 review, including receiving-daemon restart after stricter policy, exact replay and stripping refusal. Evaluation is durably recorded before refusal or admission. Existing candidate discovery remains bounded at 256, not an exhaustive catalog claim; no new physical federation acceptance is asserted. |

V2 publication implementation decision (2026-09-06): use an outer
`corpus-reasoning-answer.v2` envelope around the unchanged signed V1 answer,
with content-bound provenance and its own domain-separated signature. The outer
signature binds the complete inner answer and evidence; an unsigned neighboring
sidecar is insufficient. The receiving host still authenticates the accountable
responder independently. New Agent publication consumes the previously admitted
draft evidence, not current model configuration. Historical V1 publication
remains V1 and is not retroactively enriched. Representation and receiving
policy are explicit inputs, never inferred from topology or provider identity.
The publication assessment exports only the exact policy digest and decision,
not private provider allow/deny sets. Its signature makes the publisher
accountable for the assertion; it cannot authorize consumption on another host.
Full policy values remain in the publisher's durable local evidence. Receiving
policy is independently configured and evaluated before admission or settlement.

Producer subcheckpoints keep the remaining durable work explicit:

Consumer review checkpoint (2026-09-06): historical Assistant V1 results retain
principal/session/key-only replay without fabricated provenance; explicit V2
selection refuses unavailable historical evidence. Modern replay requires the
exact request binding and sidecar together. A source-log-backed index replaces
the fixed history ceiling; bounded catch-up commits progress, unknown unrelated
record families are excluded before parsing, and owned corruption or excision
still refuses. New preview/execute requests require an explicit turn id, and
durable disclosure traces retain token bindings rather than credentials. Unknown
and declared possible non-local execution have distinct consent wording.
...
normalization. P090-008c closes the inline deployment role/AD admission gate;
external descriptor resolution still keeps P090-008a partial.
...
Ordinary-controller recovery checkpoint: an intent fences dispatch before I/O;
one atomic Memarium completion then binds the exact Inquirium source, Step,
trace and budget. Recovery validates and projects this committed value without
reconstructing decisions or repeating inference. P090-004b2/004b2a close the
scoped synchronous-text gate with a 36-case journal matrix and separate daemon
restart evidence. Intent-only and legacy source-only histories remain refused:
neither contains a complete decision from which safe replay can be derived.
...
Ordinary synchronous-text Inquirium Agent controller recovery is covered by
P090-004b2/004b2a. The 2026-09-09 P090-004b3/004b4 checkpoint adds durable
batch/training execution and image/artifact publication, with an executable
restart/refusal gate. Generalized producer profiles, wider admission/refusal
coverage and general effect joins remain separate open work.

The same-day H-1–H-14 review hardens this checkpoint: producer shutdown has a
bounded drain, queue refusal compensates only unclaimed pending operations, and
recovery completion is acknowledged by the worker rather than inferred from
enqueue. Six additional restart cases distinguish context-binding refusal from
ambiguous execution. A known image terminal survives in the typed error/V2
...
Review hardening: immutable signed offer admission validates before append and
distinguishes exact replay from conflicting signed bytes at the same sequence
(HTTP 409), including unchanged catalog projections. Assistant completion checks
keyless session excision without synthetic keys and reports bounded replay-index
catch-up as retryable HTTP 503, separately from publication failure. Dator's
committed-publication polling backs off to 60 seconds on idle/error passes and
logs once per continuous failure episode. These corrections preserve the scoped
completion criteria below; they do not close broader producer/disclosure work.

Operator-approved scope: synchronous text/prose results through the existing
Inquirium and external Agent runtime paths, inline descriptors, Assistant and
the offer/procurement/Corpus publication path. No new model, thematic interpreter
or Sensorium capability is required. Assistant has no dependency on Corpus
publication. Earlier scoped `done` checkpoints remain valid; the following work
adds recovery and deployment guarantees rather than reopening their contracts.

| ID | Work item | Status | Completion gate |
| :--- | :--- | :--- | :--- |
| `P090-004b2a` | Reconcile committed ordinary-controller results and interrupted step/accounting projections. | `done` | Done for synchronous text. A 36-case matrix crosses completed/denied/invalid/unsafe/unavailable/failed results with intent/commit write refusal and lost acknowledgement. Exact Agent, binding, actor, request, source, step and budget survive restart; contradictory source refuses, terminal artifacts are not publishable, and replay appends no new charge or fact. Intent-only and uncommitted execution remain fenced. |
...
inference or making an uncertain external call exactly-once. A missing result
commit after possible execution requires explicit reconciliation/refusal. A
receiver's refusal can withhold order closeout but cannot erase the cost or fact
of producer execution. Persist repairable intents in the owning fact/journal
plane; never infer original authorization, timestamps or evidence from current
configuration. Tests and code review precede each status promotion.
...
### Current producer terminal closure (P090-004c)

The next bounded checkpoint covers the ten currently admitted operations:
generate, embed, batch.embed, classify, rerank, summarize, transform,
image.generate, image.edit and train.adapt. It does not admit new batch sources,
trainer profiles, models or descriptor resolution. Authentication and malformed
request ingress are not inference outcomes. A valid invocation refused before
dispatch records proven non-dispatch; an attempted invocation with no trustworthy
response retains uncertainty. Known result evidence must survive a later sink
failure in the returned terminal without claiming successful persistence.

| Task | Scope | Status | Completion evidence |
| :--- | :--- | :--- | :--- |
| `P090-004c1` | Inventory admitted operations, terminal boundaries and exclusions independently of downstream consumers. | `done` | `inquirium-core` owns the admitted-operation list; the daemon test-only conformance module owns the six-boundary vocabulary. A new operation variant fails coverage until it declares its boundaries. The daemon matrix names one proof, one stated exclusion, or one named gap for each of the sixty cells, and its audit accumulates findings instead of stopping at the first. Negative controls prove the audit refuses a dropped operation, a dropped boundary, a duplicate row, an unnamed exclusion and a renamed proof. Request-layer outcomes are recorded separately and may not be spelled like a boundary. |
| `P090-004c2` | Preserve sanitized pre-dispatch refusals without inventing execution or authority. | `done` | The supervisor binder covers all ten operation identities with not-dispatched/not-applicable/none, withholding private causes and preserving an existing terminal. Intrinsic shape validation precedes operational limits and admission; malformed HTTP input never mints an inference outcome. Actual selection-refusal endpoints retain exact terminal/provenance traces and stable image replay. Generate-backed summarize/transform preserve non-dispatch through relabelling. Resource-profile, current preflight, lease/grant admission and deferred worker refusal/restart tests pass. The two fully qualified constructor sites remain a structural guard, not an alias-proof authority theorem. |
| `P090-004c3` | Preserve terminal observations across partial persistence/publication failures. | `done` | All eight synchronous HTTP trace-sink failures preserve the exact known V2 result. Image byte/commit faults and corrupt committed-byte replay retain execution ancestry without reinference. Pre-dispatch image-edit refusal does not merge unconsumed source ancestry; unresolved post-dispatch ancestry preserves the known component and marks the aggregate incomplete. Both deferred workers commit and replay valid refusals. Preparation precedes running and dispatch intent; a pre-dispatch refusal uses one atomic refusal-intent/result pointer, so interruption before BDO projection does not invent possible dispatch. A one-byte result-object budget proves that failed source retention records a separate sanitized owner-journal fact, survives restart and leaves execution fenced; BDO diagnostic digests alone do not retain observation bytes. The 24-state source/projection recovery matrix and artifact refusal/replay proofs pass. No failure fact grants publication. |
| `P090-004c4` | Run the common producer conformance gate and synchronize component evidence. | `done` | The 2026-09-13 scoped promotion command --require admitted-inquirium-producers passes: sixty cells and thirty-one fully qualified named proofs, all executed in the same bounded run. The manifest refuses missing/truncated output, duplicates and wrong-owner tests; nineteen Python negative controls pass. Core (159 tests), host (74 tests), supervisor and actual HTTP/deferred/image regressions are green. All-target Clippy for core/host/daemon passes with -D warnings; canonical schema validation, mirror equality and documentation build pass. P063/P064, Solution 044, the Node ledger and readiness snapshot reflect this local checkpoint. An empty gap list alone still does not establish completion; the remaining P090 inventory is not promoted. |
| `P090-004c5` | Withhold ambient model-store authority from library-test daemons, following the existing acceptance-child and process-test isolation pattern. | `done` | The explicit for_test_data_dir fixture selects a test-owned model root through ordinary configuration precedence, without changing production semantics under cfg(test). The explicit-input regression covers both an operator {slot} template and an ordinary model path without accessing either; the ordinary constructor still receives the actual environment. The gate forces a nonempty environment template for this exact test, so build-time environment suppression cannot pass vacuously. Tests neither mutate the shared process environment nor give Rust template-expansion authority. Explicit model-store tests can supply isolated roots. Resolution remains in acceptance: configured_slot_path validates the single placeholder and root_from_environment refuses a conflicting supplied slot. Production runtime paths remain literal. |

Review checkpoint: 2026-09-13. Statuses above describe the verified local
worktree, not a released implementation or a new physical acceptance passage.
`P090-004c5` addresses test-infrastructure hygiene discovered during this slice,
not an inference semantic requirement. Keeping its discovery reference here does not
...
1. Explicitly inventory any additional admitted batch/training profiles before
   ...
   reopening those bounded producer tasks. The bounded batch/training journal
   and image/artifact recovery checkpoints
   `P090-004b3/004b4` are complete; do not repeat them or completed registry,
   scoped preflight, synchronous-controller, Assistant, offer-selection and
...
2. Preserve the completed bounded `P090-006b/006c/009d/012b` controller
   ...
   Complete the approved all-current-Flow scope in `P090-006d/006e/012d`:
   finish remaining source/effect and decision-authority adapters, deferred
   continuation and cross-transport recovery selection, then qualify Whisper
   terminals and full Memarium transport/recovery. Completed `P090-006d1`
   preserves exact sources when aggregation fails, including inline-budget
   overflow, and replays committed calls without redispatch. It does not make
   an oversized aggregate representable or authorize retry of an uncertain
   effect. Atomic workflow-step publication and sealed private
   observation sync now have deterministic regression coverage. The new
   sidecar/collector and private result-reopen checkpoint are implemented;
   neither an unknown placeholder nor rereading committed bytes closes these
...
4. Design and implement bounded, authenticated external descriptor resolution
   ...
   their own owning rows.
5. Extend `P090-012` acceptance only as the corresponding producers and
   consumers pass their gates. Synchronize component trackers, the Node ledger
   and readiness evidence, distinguishing deterministic local tests from
   retained physical multi-host runs. Neither this passage nor the completed
   `P090-005/008` closes all of P090.
