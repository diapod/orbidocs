...
| P081-006 | Implement pure causal-context derivation, child, fan-in join, canonicalization, and receipt validation | done | `orbiplex-node-horizontal-protocol-core` implements substrate-free root/child/join helpers, byte-canonical causation refs, deterministic digests, exact `causal-context.v1` schema and major-version refusal, distinct non-canonical-ref and operation-conflict failures, an operation-context conflict registry, bounded predecessor validation, host-actor comparison, a one-way receipt transition graph, a shared component-domain-separated receipt-id builder, and known ref-family validation. Unit/property tests cover permutation invariance, unknown schema/version refusal, conflicts, overflow, actor spoofing, receipt-id separation, invalid transitions, and missing/unknown evidence. |
