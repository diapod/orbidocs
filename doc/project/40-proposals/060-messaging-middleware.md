...
4. Complete runtime revocation detection for
   `messaging.passport-revoked.v1`; current fact writing covers
   membership, passport persistence, retention, and crisis marks.
5. Document the implemented `capability.passport.lookup` host capability
   surface in Capability Binding + Solution 019 Host Capability Bridge.
6. Harden outbound contact-request/message signing fixtures with a full
...
7. Promote the durable local recovery mirror into sealed `pseudonym-vault.v1`
   record replay at daemon startup.
8. Keep MVP decision rows and tracker evidence in sync while implementing
    schemas and host capabilities.
...
| P060-009 | Inbound acceptor: three messaging-specific scope checks + `contacts`-policy gate + recipient mailbox resolution | partial | `messaging-service` exposes `POST /v1/artifact-delivery/accept`, schema/domain validation, digest idempotency, Maildir + SQLite writes, passport scope matching, `contacts` membership projection from presented receive passports, generic `contacts-policy-denied` refusal, host `local-recipient-mailbox.resolve`, and inbound notifications. Remaining hardening: full receiver-side revocation-view integration and broader cross-node tests. |
...
| P060-013 | Messaging service implementation (compose + outbound queue + Layer 1 Maildir + Layer 2 SQLite + inbox projection + acceptor + Layer 3 fact writes) | partial | Node has `messaging-core` and `messaging-service` crates with host capability client, status, inbound accept, outbound enqueue/outbox/retry/process, contact-lookup-result promotion, mailbox/message read endpoints, Maildir message and draft storage, SQLite `PRAGMA user_version` migrations, signer-derived contact-request/message sender identity, Pseudonym Vault reply-route creation, contact-request dispatch through AD, passport lookup promotion, signed message-envelope private-direct delivery, pending Memarium facts, retention/crisis fact endpoints, and reindex. Remaining hardening: full mock-host integration coverage, full Story-010 cross-node acceptance, and revocation-triggered passport-revoked facts. |
...
| P060-016 | Recovery: `contacts` membership + issued `messaging-receive@v1` passports persisted in `pseudonym-vault.v1` | partial | `messaging-service` mirrors contacts membership records through the daemon host capability `identity.messaging-recovery.mirror`; the daemon validates accepted record kinds and persists them in a durable local recovery mirror table. The local contact store now records active/rotated pairwise `contact-nym` mappings for continuity, but sealed vault record replay at startup remains follow-up work. |
| P060-017 | Recovery: Layer 2 SQLite index rebuild procedure (replay Layer 3 → walk Maildir → repopulate rows → rebuild FTS5) | partial | `messaging-service` exposes `POST /v1/messaging/reindex` and daemon/UI proxies; the implementation rebuilds SQLite message rows from Maildir and exposes `reindexing`. Layer 3 replay and FTS5 are still future hardening. |
...
| P060-033 | Local contact and contactability bridge for Story-010 | partial | Node adds `local-contact.v1` schema-gate import/export validation, local contact labels and metadata, `/v1/local-contacts/resolve`, daemon contactability draft/options/attest/publish endpoints, and a Node UI contactability panel. The current publish path records a local draft; production Contact Catalog publication and contact-control evidence binding remain part of the Contact Catalog / attestation seam. |
| P060-034 | Contact attestation acquisition dependency for email/phone control | partial | Proposal 061 defines the separate attestation role. Node worktree adds `attestation-core`, `attestation-service`, bundled disabled `attestation-service` middleware config, `contact-attestation-request.v1` / `contact-attestation-result.v1` schemas and examples, schema-gate validators, and `email-attestation` / `phone-attestation` capability ids. Production SMTP/SMS delivery and full operator policy are still outside the messaging service itself. |
| P060-035 | Story-010 two-node acceptance scaffold | partial | `node/tools/acceptance/story-010-operator/` contains profile generation, Node A/B daemon launchers, UI launch/stop helpers, and a runbook for the two-node Seed Directory + Contact Catalog + Messaging topology. The pack is scaffolding only; the executable end-to-end smoke driver remains to be implemented. |
