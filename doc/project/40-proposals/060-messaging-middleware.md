...
| P060-035 | Story-010 two-node acceptance scaffold | done | `node/tools/acceptance/story-010-operator/` contains profile generation, Node A/B daemon launchers, UI launch/stop helpers, a runbook, `story-smoke`, and self-contained `ad-smoke` for the two-node Seed Directory + Attestation Service + Contact Catalog + Messaging topology. Strict `ad-smoke` now exercises attestation challenge/redeem, contactability publish with passport-bound signed contact claims and supervised Contact Catalog admission, shared Contact Catalog lookup, contact request delivery, operator accept, `messaging-receive@v1` passport handoff, private-direct `message-envelope.v1` delivery, and delivered inbox/outbox state. |
