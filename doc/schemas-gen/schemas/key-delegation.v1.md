...
| [`grants`](#field-grants) | `yes` | object | Open grant map from grant type to non-empty list of target identifiers. MVP recognizes `signing/capability` whose targets are capability IDs (e.g. `network-ledger`, `escrow`) or the wildcard `"*"` that grants all capability IDs. Additional grant types (`signing/org`, `signing/proxy`, ...) are reserved for future proposals and MUST be ignored by verifiers that do not recognise them (open-world extension semantics). The wildcard `"*"` is operationally convenient but semantically equivalent to full capability-signing authority; operators SHOULD prefer explicit capability lists. |
...
Open grant map from grant type to non-empty list of target identifiers. MVP recognizes `signing/capability` whose targets are capability IDs (e.g. `network-ledger`, `escrow`) or the wildcard `"*"` that grants all capability IDs. Additional grant types (`signing/org`, `signing/proxy`, ...) are reserved for future proposals and MUST be ignored by verifiers that do not recognise them (open-world extension semantics). The wildcard `"*"` is operationally convenient but semantically equivalent to full capability-signing authority; operators SHOULD prefer explicit capability lists.
